...
| **C3a** | ✅ **Antecipado** — a maquinaria de leitura dupla já está de pé, porque era também a causa dos conectores não aparecerem no desktop. A nuvem arquiva em `<tenant>:global` e o motor local perguntava por `global` nu; o `connector-bootstrap` já devolvia o id certo e a casca deixava-o cair. Agora a casca guarda-o (`W4Y_CONNECTOR_USER_ID`) e o motor lê os dois formatos — só fora do motor partilhado, onde ler o id nu devolveria a fronteira que o prefixo comprou. `_connector_user_ids()` é o ponto onde o C3 entra: acrescentar o id opaco à lista de leitura e mudar o canónico. | `work4you_cli/web_server.py`, `electron/w4y-composio.cjs` | Feito, com testes de adopção, leitura dupla, troca de conta e recusa de dica inválida. |

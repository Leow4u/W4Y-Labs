# BACKEND-MAP — secções legado (SPA web + casca fina)

> **Arquivado a 29/07/2026.** Estas secções saíram do `docs/BACKEND-MAP.md`
> porque descrevem duas arquitecturas que já não são o produto: a **SPA web**
> (`web/src`, `RightDock.tsx`, `NativeChatPage`, `OperationsPage`) e a **casca
> fina** `apps/desktop-shell`, que carregava o `web_dist` da nuvem por `loadURL`.
>
> **Não decidas nada a partir daqui.** O desktop actual é `apps/desktop`
> (renderer React nativo). Para o que existe hoje, ver
> [`../INVENTARIO-SUPERFICIES.md`](../INVENTARIO-SUPERFICIES.md); para o produto,
> [`../PRODUTO.md`](../PRODUTO.md); para contratos do motor,
> [`../BACKEND-MAP.md`](../BACKEND-MAP.md).
>
> **Duas armadilhas específicas neste texto**, para quem o ler à procura de
> história:
>
> - A secção *"Desktop: casca fina × Hermes desktop completo"* está **invertida**.
>   Chama "nossa casca" ao `desktop-shell` e diz que o `apps/desktop` está
>   "PARQUEADO no repo". Hoje é ao contrário.
> - A mesma secção afirma que o auto-update por electron-updater é "CONSTRUÇÃO,
>   não port" e que por isso saiu do plano. Foi construído desde então, e está
>   em produção.
>
> O texto está **verbatim**, sem correcções. Os factos duráveis que estavam
> enterrados nele — os três elos dos conectores, as paredes do Composio, o
> incidente da tag `p3`, a lição do proxy — foram salvos para o `BACKEND-MAP.md`
> vivo **antes** do corte, na secção "Factos duráveis salvos do pivô do desktop".

...
- **Conectores (MCP) — popular o catálogo** — integração credenciada (Notion, Google, Airtable, Slack, GIF/Tenor, e-mail…) é **conector = servidor MCP**, não skill (a plataforma nem deixa `SKILL.md` autenticar — "Footprint Ladder" do AGENTS.md). Hoje o catálogo `optional-mcps/` só tem `linear`/`n8n`/`unreal-engine`; a lib importada implementou essas integrações como "skill-com-curl+API-key" só como atalho. Workstream: adicionar os servidores MCP oficiais (reuse-first) em Conectores. **Já feito (2026-07-06):** tiradas as skill-conectores (notion/airtable/google-workspace/gif-search/himalaya) do *featured* de Habilidades — Habilidades agora = só capacidade/método pura (`FEATURED_SKILLS` em `SkillsPage.tsx`).
- **Dependências das skills no container (auditoria 2026-07-07)** — descoberto ao testar: a skill `powerpoint` falha porque **`python-pptx` E LibreOffice (`soffice`) não estão no container**. Depois ampliada para **as 72 skills** → ver **[SKILLS-AUDITORIA.md](./SKILLS-AUDITORIA.md)** (17 funcionam hoje · 6 pip-leve · 11 pesadas · 12 são conectores · 27 incompatíveis/nicho; plano em ondas A–D). Auditadas as skills featured. Container = Debian 13, **`uv` presente** (dá pra instalar pip num overlay fino tipo `Dockerfile.ui`, persistente, sem rebuild da base); Pillow + cryptography já no venv; **ausentes**: pip, tesseract, pandoc, LibreOffice, LaTeX/pdflatex, chromium. Plano em 2 fases:
  - **Fase 1 (pip leve, ~50–80 MB, overlay `uv pip install --system`):** `openpyxl` (excel), `python-pptx` (criar pptx), `youtube-transcript-api` (youtube), `nano-pdf`, `pymupdf`+`pymupdf4llm` (texto de PDF), `SciencePlots`+`arxiv` (rascunho de paper), `markitdown[pptx]`. Destrava a maioria dos fluxos de "criar". Barato.
  - **Fase 2 (apt pesado, ~0,6–2 GB cada, decisão skill a skill):** LibreOffice (miniatura/editar deck existente), tesseract/marker-pdf-ML (OCR de scan), LaTeX/texlive (compilar PDF de paper).
  - **Já feito (2026-07-07):** enquanto as deps não são instaladas, `FEATURED_SKILLS` reduzido às **7 que rodam hoje** (architecture-diagram, baoyu-infographic, claude-design, excalidraw, maps, obsidian, web-research-competitive-intelligence) — evita o usuário abrir skill que falha. As demais continuam no `?full=1`.

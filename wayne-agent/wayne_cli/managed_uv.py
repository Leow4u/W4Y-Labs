...
Wayne owns its own uv binary at ``$WAYNE_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$WAYNE_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Wayne keeps *its* uv binary.

    ``$WAYNE_HOME/bin/uv`` on POSIX, ``$WAYNE_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``wayne update`` runs
    the call site from the *old*, already-imported ``wayne_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``wayne update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$WAYNE_HOME/bin/`` instead of ``~/.local/bin/``.
    """

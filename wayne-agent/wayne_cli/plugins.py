...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with wayne-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.wayne/plugins/<name>/``
3. **Project plugins** – ``./.wayne/plugins/<name>/`` (opt-in via
   ``WAYNE_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``wayne_agent.plugins``
   entry-point group.
...
from wayne_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``WAYNE_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("WAYNE_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``WAYNE_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.wayne/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("WAYNE_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When WAYNE_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Wayne loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Wayne' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by wayne_cli.kanban_db when a task
    ...
    # separate `wayne -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `wayne kanban dispatch`),
...
ENTRY_POINTS_GROUP = "wayne_agent.plugins"

_NS_PARENT = "wayne_plugins"
...
class PluginManifest:
    ...
    #              in ~/.wayne/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``wayne plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Wayne profile name (e.g. ``"default"``).

        Derived from ``WAYNE_HOME`` via
        :func:`wayne_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.wayne/profiles/<name>``, or ``"custom"`` when
        ``WAYNE_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Wayne core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``wayne honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``wayne <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`wayne_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from wayne_cli.dashboard_auth import (
    ...
    def register_slack_action_handler(
        ...
        Wayne' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``wayne model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: wayne_cli.main imports plugins indirectly
        from wayne_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.wayne/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to wayne_cli/. Two layouts are
        ...
        # 2. User plugins (~/.wayne/plugins/)
        user_dir = get_wayne_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.wayne/plugins/)
        if _env_enabled("WAYNE_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".wayne" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with wayne and must
            ...
            # all ~20 of them added several seconds to every `wayne`
            # invocation — including plain `wayne chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Wayne ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `wayne plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``wayne plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`wayne plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `wayne plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``wayne_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``wayne_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="wayne-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``wayne tools`` TUI so plugin-provided toolsets appear

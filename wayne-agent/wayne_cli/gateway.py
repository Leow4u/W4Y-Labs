"""
Gateway subcommand for wayne CLI.

Handles: wayne gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_wayne_home is imported lazily at call sites to avoid ImportError
# when wayne_constants is cached from a pre-update version during `wayne update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "wayne CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``wayne gateway status`` from falsely counting the ``wayne`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``wayne gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_wayne_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"wayne_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. WAYNE_HOME
        ...
        # WAYNE_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "wayne_home=" in command_lc
            and f"wayne_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``wayne update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Wayne profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Wayne profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "wayne_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m wayne_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose WAYNE_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``wayne update`` users
    # with no running gateway until they re-invoke ``wayne gateway``
    ...
    from wayne_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from wayne_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / WAYNE_HOME) the windowless
        # base interpreter needs to import wayne_cli.  Empty on POSIX, where
...
def _sync_wayne_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``WAYNE_HOME``.

    Under ``sudo``, ``WAYNE_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_wayne_home` falls back to ``/root/.wayne`` — the wrong
    profile. The unit file pins ``WAYNE_HOME`` for the actual gateway
    ...
    unit_home = env.get("WAYNE_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("WAYNE_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["WAYNE_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from wayne_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``wayne gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from wayne_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (wayne_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (WAYNE_HOME-scoped).
...
from wayne_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Wayne Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``wayne gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``WAYNE_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("WAYNE_GATEWAY_DETACHED", "").strip().lower()
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current WAYNE_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (WAYNE_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from wayne_constants import get_default_wayne_root

    home = get_wayne_home().resolve()
    default = get_default_wayne_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary WAYNE_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(wayne_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when WAYNE_HOME is a named profile.

    For ``~/.wayne/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        wayne_home: Optional explicit WAYNE_HOME path. Defaults to the current
            ``get_wayne_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Wayne root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_wayne_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from wayne_constants import get_default_wayne_root

    home = Path(wayne_home or str(get_wayne_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_wayne_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(wayne_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".wayne"
    try:
        Path(wayne_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(wayne_home, default_root=target_root)
    except ValueError:
        return _profile_arg(wayne_home)
...
def has_legacy_wayne_units() -> bool:
    """Return True when any legacy Wayne gateway unit files exist."""
    return bool(_find_legacy_wayne_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Wayne gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_wayne_units()
    if not legacy:
        return
    print_warning("Legacy Wayne gateway unit(s) detected from an older install:")
...
def remove_legacy_wayne_units(
    ...
    legacy = _find_legacy_wayne_units()
    if not legacy:
        print("No legacy Wayne gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Wayne gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Wayne often sets ``HOME`` to a profile-scoped directory, but
...
def _wayne_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current WAYNE_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_wayne_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.wayne                    → /home/alice/.wayne
      /root/.wayne/profiles/coder     → /home/alice/.wayne/profiles/coder
      /opt/custom-wayne               → /opt/custom-wayne  (kept as-is)
    """
    current_wayne = get_wayne_home().resolve()
    current_default = (Path.home() / ".wayne").resolve()
    target_default = Path(target_home_dir) / ".wayne"

    # Default ~/.wayne → remap to target user's default
    if current_wayne == current_default:
        return str(target_default)

    # Profile or subdir of ~/.wayne → preserve the relative structure
    try:
        relative = current_wayne.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.wayne) — keep as-is
        return str(current_wayne)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    wayne_node = wayne_home / "node" / "bin"
    if _is_dir(wayne_node):
        candidates.append(str(wayne_node))
    wayne_nm = wayne_home / "node_modules" / ".bin"
    if _is_dir(wayne_nm):
        candidates.append(str(wayne_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m wayne_cli.main``, so module
    ...
    dir, or a clone that ``wayne update`` later relocates/removes — the path
    ...
    ``WAYNE_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if WAYNE_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        wayne_home = _wayne_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(wayne_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's WAYNE_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(wayne_home) if wayne_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m wayne_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="WAYNE_HOME={wayne_home}"
Restart=always
RestartSec=5
...
"""
    ...
    wayne_home = str(get_wayne_home().resolve())
    profile_arg = _profile_arg(wayne_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir WAYNE_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved WAYNE_HOME
    ...
    exported a throwaway ``WAYNE_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``wayne gateway restart``
    with ``WAYNE_HOME=/tmp/wayne-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="WAYNE_HOME=..."`` lines and launchd
    ``<key>WAYNE_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'WAYNE_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>WAYNE_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp WAYNE_HOME."""
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only WAYNE_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp WAYNE_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../wayne_test`` path into
    # ``Environment="WAYNE_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/wayne_test"' in new_unit
        or "/wayne_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # WAYNE_HOME into the unit (manual E2E homes like /tmp/wayne-e2e-NNN
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("WAYNE_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    if has_legacy_wayne_units():
        ...
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_wayne_units(interactive=False)
            print()
...
# process invocation (each ``wayne gateway start/stop/status`` call).
...
#      background process (the `nohup wayne gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_wayne_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_wayne_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m wayne_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active WAYNE_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "wayne_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup wayne gateway run --replace`
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.wayne/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See wayne-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_wayne_home() / "logs" / "launchd-reload.log"
        try:
...
def launchd_stop():
    ...
    # `wayne gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Wayne PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Wayne PID file).
    ...
    if deep:
        log_file = get_wayne_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``WAYNE_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("WAYNE_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``wayne gateway run`` is always a misconfiguration, so we
    ...
    try:
        from wayne_constants import get_default_wayne_root
        default_root = get_default_wayne_root()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``wayne gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``wayne gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other WAYNE_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("WAYNE_ALLOW_ROOT_GATEWAY")):
        return
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `wayne gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set WAYNE_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `wayne gateway restart` which already
    ...
    # `wayne gateway start/restart` — leaving the gateway vulnerable to
    ...
    # WAYNE_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("WAYNE_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. wayne) and copy the bot token",
...
def _all_platforms() -> list[dict]:
    ...
    ``wayne setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run wayne under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.wayne/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_wayne_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from wayne_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_wayne_home())))
    except KeyboardInterrupt:
        print()
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``wayne_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from wayne_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.wayne/plugins/
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`wayne_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from wayne_cli.service_manager import (
    ...
    if profile is None:
        ...
        # WAYNE_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``wayne gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``wayne gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``wayne gateway run`` are unaffected.
      2. ``WAYNE_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``wayne gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``WAYNE_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("WAYNE_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("WAYNE_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${WAYNE_HOME}/logs/gateways/<profile>/current,
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from wayne_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
        ...
        # slot — start them individually via `wayne -p <name> gateway
        ...
        # Stop, disable, and remove legacy Wayne gateway unit files from
        # pre-rename installs (e.g. wayne.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_wayne_units(interactive=not yes, dry_run=dry_run)

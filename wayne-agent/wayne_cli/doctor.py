...
from wayne_cli.config import get_project_root, get_wayne_home, get_env_path
from wayne_cli.env_loader import load_wayne_dotenv
from wayne_constants import display_wayne_home
from wayne_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
WAYNE_HOME = get_wayne_home()
_DHH = display_wayne_home()  # user-facing display path (e.g. ~/.wayne or ~/.wayne/profiles/coder)

# Load environment variables from ~/.wayne/.env so API key checks work
_env_path = get_env_path()
load_wayne_dotenv(wayne_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from wayne_cli.colors import Colors, color
from wayne_cli.models import _WAYNE_USER_AGENT
from wayne_constants import OPENROUTER_MODELS_URL
...
from wayne_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.wayne/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("WAYNE_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from wayne_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """
    try:
        from wayne_cli import __version__ as init_version
    except Exception:
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from wayne_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the WAYNE_MANAGED_DIR override (rather than the system
...
def run_doctor(args):
    ...
    # Detect drift between pyproject.toml and wayne_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.wayne/.env (primary location for user config)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from wayne_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from wayne_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from wayne_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        ...
        # Detect stale WAYNE_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives WAYNE_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from wayne_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Wayne' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = wayne_home / subdir_name
    ...
    memories_dir = wayne_home / "memories"
    ...
    state_db_path = wayne_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from wayne_state import _db_opens_cleanly, repair_state_db_schema
    ...
    wal_path = wayne_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "wayne"
        ...
        _cmd_link = _cmd_link_dir / "wayne"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(wayne not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/wayne → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/wayne points to wrong target",
                ...
                check_fail(
                    f"{_cmd_link_display}/wayne not found",
                    "(wayne command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/wayne → {_venv_bin}")
                    fixed_count += 1
    ...
    try:
        from wayne_constants import is_container as _is_container
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable WAYNE_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Wayne one.
                        check_info(
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = WAYNE_HOME / "config.yaml"
    ...
    try:
        from wayne_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "wayne -p" in content:
                            _m = _re.search(r"wayne -p (\S+)", content)

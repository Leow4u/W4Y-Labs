...
the tool registry or launching Wayne (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``wayne-cli`` to the non-existent ``wayne``. ``resolve_toolset('wayne')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``wayne-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``wayne`` instead of ``wayne-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `wayne tools` to reconfigure."
        )
    return warnings

"""``wayne project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$WAYNE_HOME/projects.db`` store
(see :mod:`wayne_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Wayne Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``wayne project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: wayne project <action> [options]\n"
                "Run 'wayne project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `wayne project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `wayne project add-folder`.",
            file=sys.stderr,
        )
        return 1

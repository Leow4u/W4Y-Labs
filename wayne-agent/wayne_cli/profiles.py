"""
Profile management for multiple isolated Wayne instances.

Each profile is a fully independent WAYNE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.wayne/profiles/<name>/`` by default.

The "default" profile is ``~/.wayne`` itself — backward compatible,
zero migration needed.

Usage::

    wayne profile create coder          # fresh profile + bundled skills
    wayne profile create coder --clone  # also copy config, .env, SOUL.md, skills
    wayne profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    wayne -p coder chat                 # or via flag
    wayne profile use coder             # set as sticky default
    wayne profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See wayne_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `wayne profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `wayne
...
# `wayne skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Wayne infrastructure directories that only the default profile
         (``~/.wayne``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_wayne_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.wayne) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "wayne-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "wayne", "default", "test", "tmp", "root", "sudo",
})

# Wayne subcommands that cannot be used as profile names/aliases
_WAYNE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the wayne root, NOT to the current WAYNE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where WAYNE_HOME points outside
    ``~/.wayne``, profiles live under ``WAYNE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_wayne_home() / "profiles"


def _get_default_wayne_home() -> Path:
    """Return the default (pre-profile) WAYNE_HOME path.

    In standard deployments this is ``~/.wayne``.
    In Docker/custom deployments where WAYNE_HOME is outside ``~/.wayne``
    (e.g. ``/opt/data``), returns WAYNE_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_wayne_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its WAYNE_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, wayne subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _WAYNE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a wayne subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            wayne_exe = shutil.which("wayne") or "wayne"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(wayne_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Wayne. If we
    ...
    try:
        ...
        from wayne_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `wayne doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``wayne -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``wayne profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``wayne -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``wayne profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``WAYNE_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Wayne config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_wayne_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, wayne_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own WAYNE_HOME.
    ...
    The returned ``wayne_home`` is the path to pass to
    ``set_wayne_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_wayne_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``wayne update`` skips re-seeding this profile's
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.wayne/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `wayne -p <name> auth add`) had no file until first
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`wayne profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Wayne (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `wayne profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `wayne -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches WAYNE_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``wayne profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "WAYNE_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_wayne_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Wayne *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Wayne backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``WAYNE_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `wayne -p <canon> profile
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Wayne process: either an entrypoint marker in argv, or
            # a resolved executable named `wayne`.
            joined = " ".join(argv)
            ...
            # ...or by WAYNE_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("WAYNE_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_wayne_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``WAYNE_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from wayne_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set WAYNE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("WAYNE_HOME")
    try:
        os.environ["WAYNE_HOME"] = str(profile_dir)
        from wayne_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["WAYNE_HOME"] = old_home
        elif "WAYNE_HOME" in os.environ:
            del os.environ["WAYNE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.wayne/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from WAYNE_HOME.

    Returns ``"default"`` if WAYNE_HOME is not set or points to ``~/.wayne``.
    Returns the profile name if WAYNE_HOME points into ``~/.wayne/profiles/<name>``.
    Returns ``"custom"`` if WAYNE_HOME is set to an unrecognized path.
    """
    from wayne_constants import get_wayne_home
    wayne_home = get_wayne_home()
    resolved = wayne_home.resolve()

    default_resolved = _get_default_wayne_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.wayne itself — its parent is ~/ and its
        # directory name is ".wayne", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.wayne itself — disallow
    ...
    with tempfile.TemporaryDirectory(prefix="wayne_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"wayne_{old_name}"
    legacy_old_host = f"wayne.{old_name}"
    new_host = f"wayne_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_wayne_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("wayne_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a WAYNE_HOME path string.

    Called early in the CLI entry point, before any wayne modules
    are imported, to set the WAYNE_HOME environment variable.
    """

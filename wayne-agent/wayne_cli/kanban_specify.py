...
Used by ``wayne kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``wayne_cli/goals.py`` — same aux
...
WAYNE_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("WAYNE_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)
...
def _profile_author() -> str:
    """Mirror of ``wayne_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("WAYNE_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=WAYNE_KANBAN_SPECIFY_MAX_TOKENS,

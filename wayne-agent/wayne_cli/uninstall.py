"""
Wayne Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.wayne/ (configs, sessions, logs)
"""
...
def remove_wrapper_script():
    """Remove the wayne wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "wayne",
        Path("/usr/local/bin/wayne"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains wayne_cli reference)
                content = wrapper.read_text()
                if 'wayne_cli' in content or 'wayne-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(wayne_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``wayne`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Wayne home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (wayne_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``wayne gateway run`` processes
    ...
    try:
        from wayne_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from wayne_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``WAYNE_HOME`` and ``WAYNE_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\wayne\git\cmd``, ``%LOCALAPPDATA%\wayne\git\bin``,
#      ``%LOCALAPPDATA%\wayne\git\usr\bin``, ``%LOCALAPPDATA%\wayne\node``.
...
def _wayne_path_markers(wayne_home: Path) -> list[str]:
    """Path-entry substrings that identify Wayne-owned User-PATH entries."""
    root = str(wayne_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare wayne-agent install dir.
    markers = [root + "\\wayne-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if WAYNE_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "wayne".  We don't want to catch
    # unrelated entries like "cwayne-foo" or "ephermeral", so we look for
    # backslash-wayne as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(wayne_home: Path) -> list[str]:
    """Strip Wayne-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _wayne_path_markers(wayne_home)
            kept: list[str] = []
...
def _is_default_wayne_home(wayne_home: Path) -> bool:
    """Return True when ``wayne_home`` points at the default (non-profile) root."""
    try:
        from wayne_constants import get_default_wayne_root
        return wayne_home.resolve() == get_default_wayne_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m wayne_cli.main` so we don't depend on a `wayne`
    #    wrapper that may be half-removed mid-uninstall.
    wayne_invocation = [_sys.executable, "-m", "wayne_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                wayne_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's WAYNE_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    """
    from wayne_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(wayne_home)
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.wayne/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.wayne/ for future reinstall
    """
    project_root = get_project_root()
    wayne_home = get_wayne_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie WAYNE_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_wayne_home(wayne_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.wayne data); otherwise keep-data. Named profiles
    ...
    print(f"  Config:  {wayne_home / 'config.yaml'}")
    print(f"  Secrets: {wayne_home / '.env'}")
    print(f"  Data:    {wayne_home / 'cron/'}, {wayne_home / 'sessions/'}, {wayne_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their WAYNE_HOME dirs. Otherwise
...
def _perform_uninstall(
    *,
    ...
    ``wayne`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$WAYNE_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in wayne_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\wayne\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(wayne_home))))
    ...
    #     step-5 rmtree(wayne_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE WAYNE_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from wayne_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(wayne_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.wayne/, just remove the wayne-agent subdir
            if wayne_home in project_root.parents or project_root.parent == wayne_home:
    ...
    #     under WAYNE_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(wayne_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(wayne_home)
    ...
    # 5. Optionally remove ~/.wayne/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile WAYNE_HOME dirs live under
        ...
        try:
            if wayne_home.exists():
                shutil.rmtree(wayne_home)
                log_success(f"Removed {wayne_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {wayne_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {wayne_home}")
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m wayne_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import wayne_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``wayne_constants`` + ``wayne_cli.colors``
    (and lazily ``wayne_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m wayne_cli.uninstall")
    parser.add_argument(
        "--mode",

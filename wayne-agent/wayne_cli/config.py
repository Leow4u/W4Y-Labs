...
- ~/.wayne/.env         - API keys and secrets

This module provides:
- wayne config          - Show current configuration
- wayne config edit     - Open config in editor
- wayne config set      - Set a specific value
- wayne config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``wayne config set`` (which rewrites
    ...
    ``config.yaml`` in place: wayne never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.wayne/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``wayne logs``
...
#   the next ``subprocess.run([...])`` Wayne makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Wayne itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``wayne update``, the TUI build.
...
#   on every plugin install / ``wayne update``.
...
# * ``WAYNE_HOME`` / ``WAYNE_PROFILE`` / ``WAYNE_CONFIG`` /
#   ``WAYNE_ENV`` — Wayne runtime location flags. Writing these into
...
#
# IMPORTANT: ``WAYNE_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (WAYNE_LANGFUSE_PUBLIC_KEY,
# WAYNE_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Wayne runtime location — never via dashboard env writer.
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "WAYNE_TOOL_PROGRESS", "WAYNE_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `wayne plugins enable
    # observability/langfuse` or `wayne tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "WAYNE_LANGFUSE_ENV",
    "WAYNE_LANGFUSE_RELEASE",
    "WAYNE_LANGFUSE_SAMPLE_RATE",
    "WAYNE_LANGFUSE_MAX_CHARS",
    "WAYNE_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("WAYNE_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_wayne_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Wayne is running in package-manager-managed mode.

    Two signals: the WAYNE_MANAGED env var (set by the systemd service),
    or a .managed marker file in WAYNE_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``wayne_cli/`` — i.e. the git checkout for source
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Wayne was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$WAYNE_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. WAYNE_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.wayne``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$WAYNE_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.wayne:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$WAYNE_HOME``, a
    ...
    ``docker`` and ``wayne update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $WAYNE_HOME.
    try:
    ...
    #    host install whose shared $WAYNE_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``wayne update``.
    try:
        method = (
            (get_wayne_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``wayne_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$WAYNE_HOME``, so that two installs sharing one data directory
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Wayne lives in a ``uv tool`` layout.

    ``uv tool install wayne-agent`` places the install at
    ``.../uv/tools/wayne-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``wayne-agent`` happens to be uv-tool-installed on the machine while
    the *active* Wayne is a regular pip/venv install, causing
    ``wayne update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/wayne-agent/" in norm + "/"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from WAYNE_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, wayne_bin
    or None if container mode is not active, we're already inside the
    container, or WAYNE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("WAYNE_DEV") == "1":
        return None
    ...
    container_mode_file = get_wayne_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "wayne-agent")
    exec_user = info.get("exec_user", "wayne")
    wayne_bin = info.get("wayne_bin", "/data/current-package/bin/wayne")
...
# Re-export from wayne_constants — canonical definition lives there.
from wayne_constants import get_wayne_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_wayne_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_wayne_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_wayne_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the WAYNE_UID / WAYNE_GID env vars set by Docker deployments.

    Docker containers running Wayne commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level WAYNE_HOME once, but
    subdirectories created at runtime by ``ensure_wayne_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("WAYNE_UID", "").strip()
    gid_str = os.environ.get("WAYNE_GID", "").strip()
    try:
...
def _chown_to_wayne_uid(path) -> None:
    """Chown ``path`` to ``WAYNE_UID:WAYNE_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_wayne_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_wayne_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the wayne group can
    share state with the gateway service.

    The mode can be overridden via the WAYNE_HOME_MODE environment variable
    (e.g. WAYNE_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse WAYNE_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``WAYNE_UID``/``WAYNE_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("WAYNE_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_wayne_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Wayne runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("WAYNE_CONTAINER") or os.environ.get("WAYNE_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set WAYNE_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def ensure_wayne_home():
    """Ensure ~/.wayne directory structure exists with secure permissions.
    ...
    # silently mkdir-ing the old WAYNE_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_wayne_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Wayne-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Wayne
        ...
        # identity slot (SOUL.md). Empty by default. The WAYNE_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Wayne adds a coding operating brief
    ...
    "terminal": {
        ...
        #             WAYNE_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force WAYNE_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Wayne auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Wayne sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Wayne
        ...
        #  "/home/user/.wayne/cache/documents:/output"]
        ...
        # bundled Wayne image, which drops to the `wayne` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Wayne sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Wayne should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``wayne chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.wayne/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: wayne sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .wayne.md, or .cursorrules before Wayne applies
    ...
    # single read_file page exceeds these limits, Wayne truncates the
    ...
    # See tools/kanban_tools.py and wayne_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``wayne dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``wayne kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``wayne kanban decompose`` and the kanban
        ...
        # ``wayne profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `wayne model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `wayne` (and `wayne chat`) launches by default:
        ...
        # REPL and `--tui` (or WAYNE_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `wayne --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `wayne -c` muscle memory.  Default off so existing
        # users aren't surprised.  WAYNE_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `wayne --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/wayne`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `wayne pets`. Disabled until
        ...
        "pet": {
            ...
            # get_wayne_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``WAYNE_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``WAYNE_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``WAYNE_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``WAYNE_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``WAYNE_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/wayne``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Wayne appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.wayne/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.wayne/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.wayne/cache/delegation/
    ...
    # response. If not, Wayne feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Wayne auto-pauses the goal and
    ...
    "moa": {
        ...
        # <wayne_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    "skills": {
        ...
        # accepted too, for agentskills.io ecosystem skills).  Lets skill
        # authors reference bundled scripts without the agent having to join
        # paths.
        "template_vars": True,
    ...
    # cron daemon.
    #
    # See `wayne curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `wayne curator run --consolidate` overrides
        ...
        # normally restored on every `wayne update`, so pruning them only
        ...
        # skipped), snapshot ~/.wayne/skills/ into
        # ~/.wayne/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `wayne curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for wayne-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (WAYNE_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Wayne' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.wayne/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or WAYNE_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `wayne doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``wayne_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Wayne to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $WAYNE_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via WAYNE_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `wayne -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `wayne` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `wayne kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     wayne-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Wayne tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.wayne/logs/.
    ...
    # update model picker lists without shipping a wayne-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `wayne model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal WAYNE_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # the WAYNE_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.wayne.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.wayne/.env,
        ...
        # behavior: files must live under the Wayne cache, under
        ...
        # user. Bridged to WAYNE_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Wayne
        # cache (~/.wayne/cache/{documents,images,audio,video,screenshots})
        ...
        # to WAYNE_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # WAYNE_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to WAYNE_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.wayne/state.db.
    ...
    "sessions": {
        ...
        # default of ``wayne sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.wayne/sessions/session_{sid}.json`` on every turn
    ...
    # ``wayne update`` behaviour.
    "updates": {
        # Run a full ``wayne backup``-style zip of WAYNE_HOME before every
        # ``wayne update``.  Backups land in ``<WAYNE_HOME>/backups/`` and
        # can be restored with ``wayne import <path>``.  Off by default:
        # zipping a large WAYNE_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``wayne update
        ...
        "backup_keep": 5,
        # What `wayne update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<WAYNE_HOME>/lsp/bin/`` on first use.
    ...
    # `wayne tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.wayne/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.wayne/.env (or your shell) and never in config.yaml.
            ...
            # ~/.wayne/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Wayne disables it for our users unless they
        # explicitly opt in here. When false (default), Wayne sets
    ...
    # Wayne Desktop (Electron app) launch options. These only affect
    # `wayne desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the WAYNE_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @wayne:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #wayne)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: wayne-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'wayne-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # WAYNE_TOOL_PROGRESS and WAYNE_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.wayne.config`` entries, then checks
    ...
    try:
        ...
        # should never break `wayne update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Wayne versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``wayne_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "wayne update / wayne -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("WAYNE_TOOL_PROGRESS")
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$WAYNE_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_wayne_home() / "plugins"
    ...
    # settings in their `config.yaml`, and `wayne curator status` has no
    ...
    #   3. Creates `~/.wayne/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_wayne_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_wayne_home() / "logs" / "curator"
    ...
    # behavior set it to true explicitly via `wayne config set`.
    ...
    try:
        ...
        from wayne_cli.toolset_validation import validate_platform_toolsets
    ...
    # `wayne update`, but only the version bump is persisted.
    ...
    # metadata.wayne.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``wayne config set``
    ...
    backends) — while display paths (``wayne status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _wayne_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.wayne/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.wayne/config.yaml.
    ...
    (which change ``WAYNE_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/wayne/config.yaml invalidates the
...
#   openai-codex (OAuth — wayne auth) — OpenAI Codex
#   nous         (OAuth — wayne auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.wayne/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Wayne writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.wayne/.env.
    ...
    interactive menu render (`wayne tools`, `wayne setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `wayne tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Wayne env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.wayne/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.wayne/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.wayne/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.wayne/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Wayne — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Wayne vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.wayne/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.wayne/.env`` over ``os.environ``.

    Used for Wayne-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from wayne_cli.auth import get_anthropic_key
    ...
    # Warn on stale WAYNE_MAX_ITERATIONS ghost in .env that disagrees with
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `wayne config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `wayne config set model.api_key cfut_...` routes to config.yaml
...
# ``_inject_profile_env_vars`` surfaces them in ``wayne config`` UI so users

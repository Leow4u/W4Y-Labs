...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to WAYNE_INFERENCE_MODEL
...
def _run_agent(
    ...
    # Imports are local so they don't run when wayne is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from wayne_cli.config import load_config
    from wayne_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("WAYNE_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from wayne_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("WAYNE_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                WAYNE_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via WAYNE_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via WAYNE_YOLO_MODE=1

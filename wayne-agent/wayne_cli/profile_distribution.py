"""Profile distributions — shareable, packaged Wayne profiles via git.

A distribution is a Wayne profile published as a git repository (or
...
* ``wayne profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``wayne skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``wayne profile``, not a parallel tree):

    wayne profile install <source> [--name N] [--alias] [--force] [--yes]
    wayne profile update  <name>  [--force-config] [--yes]
    wayne profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "wayne-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            wayne_requires=str(data.get("wayne_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.wayne_requires:
            out["wayne_requires"] = self.wayne_requires
        if self.author:
...
def check_wayne_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Wayne {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Wayne distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``wayne profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Wayne profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Wayne distribution."
        )

    # Version check up-front so we fail fast
    check_wayne_requires(manifest.wayne_requires, wayne_version)
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="wayne_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `wayne profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`wayne profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="wayne_dist_update_") as tmp:
        plan = plan_install(

"""``wayne slack ...`` CLI subcommands.

Today only ``wayne slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ wayne slack manifest > slack-manifest.json
    # or:
    $ wayne slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Wayne. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Wayne deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from wayne_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``wayne_cli/main.py``):

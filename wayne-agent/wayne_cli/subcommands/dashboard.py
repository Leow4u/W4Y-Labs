"""``wayne dashboard`` / ``wayne serve`` subcommand parsers.
...
``wayne_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `wayne dashboard` / `wayne serve` cmdlines and SIGTERM them directly —
    # the same path `wayne update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Wayne desktop app shells (<= 0.15.x) spawn the
    # backend as `wayne dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Wayne couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Wayne Desktop app (and headless remote
    ...
    # `wayne dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.wayne/.env.
    # Nested subparser so bare `wayne dashboard` keeps launching the server

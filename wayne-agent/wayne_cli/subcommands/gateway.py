"""``wayne gateway`` and ``wayne proxy`` subcommand parsers.
...
from wayne_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy wayne.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Wayne gateway unit files "
            "(e.g. wayne.service) left over from older installs. Profile "
            "units (wayne-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `wayne proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,

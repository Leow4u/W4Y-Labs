...
# display_wayne_home imported lazily at call sites (stale-module safety during wayne update)
...
from wayne_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from wayne_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``WAYNE_NONINTERACTIVE=1`` (see ``wayne_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("WAYNE_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``WAYNE_NONINTERACTIVE=1`` or a closed/redirected
...
def _print_setup_summary(config: dict, wayne_home):
    ...
    # Video generation — opt-in via `wayne tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from wayne_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    # Spotify (OAuth via wayne auth spotify — check auth.json, not env vars)
    try:
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {wayne_home}/cron/, sessions/, logs/"
    )
    print()
...
# between `wayne tools` and `wayne setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``wayne model``)
    ...
    provider added to ``wayne model`` is automatically available here.
    ...
    # Delegate to the shared wayne model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from wayne_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``wayne model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Wayne auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'wayne setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `wayne setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `wayne setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into WAYNE_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("WAYNE_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # WAYNE_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from wayne_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_wayne_home(Path(get_wayne_home()))
    except Exception:
        pass
...
def _profile_name_from_wayne_home(wayne_home) -> str | None:
    """Return the active profile name when WAYNE_HOME is a profile dir."""
    if wayne_home.parent.name == "profiles":
        return wayne_home.name
    return None
...
def _setup_qqbot():
    ...
    from wayne_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_wayne_units():
            print_legacy_unit_warning()
            print()
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `wayne setup tools` and `wayne tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``wayne_cli.auth`` — lists every supported
    ...
    """
    try:
        from wayne_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from wayne_cli.config
    ...
    if section_key == "model":
        ...
        from wayne_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_wayne.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_wayne migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_wayne", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(wayne_home: Path) -> bool:
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=wayne_home.resolve(),
    ...
    # Execute the migration — overwrite=False so existing Wayne configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Wayne config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from wayne_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      wayne setup           — full or quick (auto-detected)
      wayne setup model     — just model/provider
      wayne setup tts       — just text-to-speech
      wayne setup terminal  — just terminal backend
      wayne setup gateway   — just messaging platforms
      wayne setup tools     — just tool configuration
      wayne setup agent     — just agent settings
    """
    from wayne_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, wayne_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(wayne_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, wayne_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, wayne_home, is_existing)
            return
    ...
    # Tune later with `wayne setup agent`.
    if not is_existing:
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``wayne tools`` (which
    ...
    try:
        ...
        from wayne_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``wayne setup agent`` /
    ``wayne config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, wayne_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `wayne update` runs don't re-inject them.
        try:
    ...
    _blank_slate_walkthrough(config, wayne_home)


def _blank_slate_walkthrough(config: dict, wayne_home):
...
def _run_quick_setup(config: dict, wayne_home):

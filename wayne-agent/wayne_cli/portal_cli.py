...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `wayne model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `wayne portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    """
    from wayne_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    ...
    print("Run `wayne portal -h` for usage.", file=sys.stderr)
    return 1
...
def add_parser(subparsers) -> None:
    ...
    portal_parser = subparsers.add_parser(
        ...
        description=(
            "Run `wayne portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `wayne auth add "
            "nous --type oauth`, identical to `wayne setup --portal`). "

"""
Wayne Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Wayne.app`` or ``~/Applications/Wayne.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Wayne`` (NSIS per-user)
...
the app name ("Wayne"), separate from ``$WAYNE_HOME``:
  - macOS:   ``~/Library/Application Support/Wayne``
  - Windows: ``%APPDATA%\\Wayne``
  - Linux:   ``$XDG_CONFIG_HOME/Wayne`` (default ``~/.config/Wayne``)
...
``wayne uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(wayne_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return wayne_home / "wayne-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Wayne"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Wayne"
...
def source_built_gui_artifacts(wayne_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``wayne desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``wayne-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(wayne_home)
    ...
    return [
        ...
        wayne_home / "desktop-build-stamp.json",
    ]
...
def agent_is_installed(wayne_home: Path) -> bool:
    """Return True when a usable Python agent install exists under WAYNE_HOME.
    ...
    if (agent_root / "wayne_cli").is_dir():
        return True
...
def gui_is_installed(wayne_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(wayne_home):
        if p.exists():
            return True
...
def gui_install_summary(wayne_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = wayne_home if wayne_home is not None else get_wayne_home()
...
def uninstall_gui(wayne_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``wayne-agent/wayne_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$WAYNE_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove wayne' or "
            "'sudo dnf remove wayne'). AppImage builds are a single file you "

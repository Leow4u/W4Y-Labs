...
Used by ``wayne profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``wayne_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Wayne Agent kanban board.
...
Rules:
  ...
  - Never write "Wayne Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_wayne_home())
        else:

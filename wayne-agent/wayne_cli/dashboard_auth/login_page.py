...
``tests/wayne_cli/test_dashboard_auth_401_reauth.py``.
"""
from __future__ import annotations

import html

from wayne_cli.dashboard_auth import list_session_providers
...
<title>Entrar — Work4You</title>
<style>
  /* Cascadia Mono — fonte de marca W4Y (SIL OFL, vendorada em /fonts). */
  @font-face {{
    font-family: 'Cascadia Mono';
    font-style: normal;
    font-weight: 200 700;
    font-display: swap;
    src: url('/fonts/CascadiaMono.woff2') format('woff2-variations');
  }}
  ...
  /* Paleta W4Y — alinhada ao tema "White" (padrão da plataforma). */
  :root {{
    --background-base: #ffffff;
    --background: #ffffff;
    --midground: #171717;
    --foreground: #171717;
    --hairline: color-mix(in srgb, #171717 14%, transparent);
    --hairline-strong: color-mix(in srgb, #171717 30%, transparent);
  }}
  ...
  *, *::before, *::after {{ box-sizing: border-box; }}
  ...
  /* Logo Work4You — a palavra escrita em Cascadia Mono (branding W4Y):
     sem uppercase, sem tracking largo. */
  .brand {{
    text-align: center;
  ...
  .field-input {{
    ...
    background: color-mix(in srgb, #000000 4%, var(--background-base));
    color: var(--foreground);
...
{provider_buttons}
  ...
  <footer>
    <span class="sep"></span>Acesso p&uacute;blico &middot; Autentica&ccedil;&atilde;o obrigat&oacute;ria<span class="sep"></span>
  </footer>
</main>
{password_script}
</body>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Login indisponível — Work4You</title>
<style>
  @font-face {
    font-family: 'Collapse';
    font-style: normal;
  ...
  function handle(form) {
    form.addEventListener('submit', function (ev) {
      ...
      fetch('/auth/password-login', {
        ...
        if (err) { err.textContent = 'Erro de rede. Tente novamente.'; err.hidden = false; }
        if (btn) { btn.disabled = false; }
      });
    });
  }
    ...
    for p in providers:
        if getattr(p, "supports_password", False):
            ...
            buttons.append(
                ...
                f'Entrar com {html.escape(p.display_name)}</a>'
            )
...
def _render_password_form(provider, next_path: str) -> str:
    ...
    return (
        ...
        f'autocomplete="on">\n'
        f'        <div class="form-title">Entrar com {plabel}</div>\n'
        f'        <input type="hidden" name="next" value="{safe_next}">\n'
        f'        <label class="field">\n'
        f'          <span class="field-label">Usu&aacute;rio</span>\n'

...
Profile-aware location: ``$WAYNE_HOME/logs/dashboard-auth.log``.
...
from ``wayne_constants`` or other wayne_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$WAYNE_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``wayne_constants.get_wayne_home`` semantics: env var wins,
    else ``~/.wayne``. A local copy avoids an import cycle with the
    middleware which lives below ``wayne_cli``.
    """
    home = os.environ.get("WAYNE_HOME") or str(Path.home() / ".wayne")

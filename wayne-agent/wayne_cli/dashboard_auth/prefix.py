...
prefix (e.g. ``mission-control.tilos.com/wayne/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /wayne`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``WAYNE_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``wayne.example.com`` instead of
    ``https://wayne.example.com``) — the single most common cause of
    "I set WAYNE_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "wayne.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/wayne"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# WAYNE_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``WAYNE_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("WAYNE_DASHBOARD_PUBLIC_URL", "")

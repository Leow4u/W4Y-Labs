...
1. The Wayne venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.wayne/plugins``
...
def _discover_plugins(wayne_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.wayne/plugins``.
...
def run_audit(
    *,
    ...
    home = wayne_home or Path(get_wayne_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, wayne_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(wayne_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `wayne security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, wayne_home=home
    )
    if total == 0:

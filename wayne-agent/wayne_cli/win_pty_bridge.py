"""Windows ConPTY bridge for the `wayne dashboard` chat tab.

Drop-in counterpart to ``wayne_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``wayne_cli.web_server`` consumes: ``spawn``, ``read``,

...
def _set_process_title() -> None:
    """Set the process title to 'wayne' so tools like 'ps', 'top', and
    ...
    Strategy (try in order):
      ...
      4. No-op on Windows (the .exe name is already ``wayne.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("wayne")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"wayne", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"wayne")
        # Windows: the .exe name is already ``wayne.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* wayne_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("WAYNE_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".wayne", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``WAYNE_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("WAYNE_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``WAYNE_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("WAYNE_TUI_NO_EARLY_DISABLE") == "1":
        return
...
from wayne_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from wayne_cli.subcommands.cron import build_cron_parser
from wayne_cli.subcommands.gateway import build_gateway_parser
from wayne_cli.subcommands.profile import build_profile_parser
from wayne_cli.subcommands.model import build_model_parser
from wayne_cli.subcommands.setup import build_setup_parser
from wayne_cli.subcommands.postinstall import build_postinstall_parser
from wayne_cli.subcommands.whatsapp import build_whatsapp_parser
from wayne_cli.subcommands.slack import build_slack_parser
from wayne_cli.subcommands.login import build_login_parser
from wayne_cli.subcommands.logout import build_logout_parser
from wayne_cli.subcommands.auth import build_auth_parser
from wayne_cli.subcommands.status import build_status_parser
from wayne_cli.subcommands.webhook import build_webhook_parser
from wayne_cli.subcommands.hooks import build_hooks_parser
from wayne_cli.subcommands.doctor import build_doctor_parser
from wayne_cli.subcommands.security import build_security_parser
from wayne_cli.subcommands.dump import build_dump_parser
from wayne_cli.subcommands.debug import build_debug_parser
from wayne_cli.subcommands.backup import build_backup_parser
from wayne_cli.subcommands.import_cmd import build_import_cmd_parser
from wayne_cli.subcommands.config import build_config_parser
from wayne_cli.subcommands.console import build_console_parser
from wayne_cli.subcommands.version import build_version_parser
from wayne_cli.subcommands.update import build_update_parser
from wayne_cli.subcommands.uninstall import build_uninstall_parser
from wayne_cli.subcommands.dashboard import build_dashboard_parser
from wayne_cli.subcommands.gui import build_gui_parser
from wayne_cli.subcommands.logs import build_logs_parser
from wayne_cli.subcommands.prompt_size import build_prompt_size_parser
from wayne_cli.subcommands.memory import build_memory_parser
from wayne_cli.subcommands.acp import build_acp_parser
from wayne_cli.subcommands.tools import build_tools_parser
from wayne_cli.subcommands.insights import build_insights_parser
from wayne_cli.subcommands.skills import build_skills_parser
from wayne_cli.subcommands.pairing import build_pairing_parser
from wayne_cli.subcommands.plugins import build_plugins_parser
from wayne_cli.subcommands.mcp import build_mcp_parser
from wayne_cli.subcommands.claw import build_claw_parser
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any wayne module import.
#
# Many modules cache WAYNE_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("WAYNE_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.wayne/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set WAYNE_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to Wayne' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        ...
        candidate = home / ".wayne" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # Mirrors wayne_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If WAYNE_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.wayne/profiles/coder or
    ...
    if profile_name is None and wayne_home_env:
        if Path(wayne_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the wayne root.
    #
    ...
    if profile_name is None and not os.environ.get("WAYNE_S6_SUPERVISED_CHILD"):
        try:
            from wayne_constants import get_default_wayne_root

            active_path = get_default_wayne_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set WAYNE_HOME
    if profile_name is not None:
        try:
            from wayne_cli.profiles import resolve_profile_env

            wayne_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            wayne_home = _resolve_sudo_user_profile_env(profile_name)
            if not wayne_home:
            ...
            # A bug in profiles.py must NEVER prevent wayne from starting
            print(
...
load_wayne_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → WAYNE_REDACT_SECRETS env
# var BEFORE wayne_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_wayne_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "WAYNE_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["WAYNE_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `wayne` subcommands
...
try:
    from wayne_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from wayne_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if wayne_constants not importable yet
...
# Provider model-selection wizard flows extracted to wayne_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (wayne_cli.main._model_flow_*) keep resolving unchanged.
from wayne_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_wayne_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("WAYNE_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("WAYNE_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from wayne_cli.config import get_env_path, get_wayne_home, load_config
    from wayne_cli.auth import get_auth_status

    # Determine whether Wayne itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_wayne_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_wayne_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Wayne has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Wayne to use their tokens.
    if _has_wayne_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, wayne_bin
        cli_args: the original CLI arguments (everything after 'wayne')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    wayne_bin = container_info["wayne_bin"]
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @wayne/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@wayne" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/wayne-ink/package.json",
    "packages/wayne-ink/index.js",
    "packages/wayne-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``WAYNE_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("WAYNE_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``WAYNE_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("WAYNE_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    wayne_home = os.environ.get("WAYNE_HOME") or str(Path.home() / ".wayne")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "WAYNE_HOME": wayne_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(wayne_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(wayne_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if wayne_cli_dir is None:
        wayne_cli_dir = Path(__file__).parent
    bundled = wayne_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (WAYNE_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("WAYNE_NODE")
    ...
    if tui_dev:
        # Keep the local @wayne/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @wayne/ink resolves through
        # packages/wayne-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "wayne-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from wayne_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from wayne_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="wayne-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["WAYNE_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["WAYNE_PYTHON_SRC_ROOT"] = os.environ.get(
        "WAYNE_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("WAYNE_PYTHON", sys.executable)
    env.setdefault("WAYNE_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["WAYNE_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["WAYNE_MODEL"] = model
        env["WAYNE_INFERENCE_MODEL"] = model
    if provider:
        env["WAYNE_TUI_PROVIDER"] = provider
        env["WAYNE_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["WAYNE_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["WAYNE_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["WAYNE_TUI_SKILLS"] = value
    if query:
        env["WAYNE_TUI_QUERY"] = query
    if image:
        env["WAYNE_TUI_IMAGE"] = image
    if checkpoints:
        env["WAYNE_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["WAYNE_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["WAYNE_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["WAYNE_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["WAYNE_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["WAYNE_ACCEPT_HOOKS"] = "1"
    ...
    # still set WAYNE_TUI_RESUME themselves.
    env.pop("WAYNE_TUI_RESUME", None)
    if resume_session_id:
        env["WAYNE_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    ...
    """
    if os.environ.get("WAYNE_KANBAN_BOARD"):
        return
    try:
        from wayne_cli.kanban_db import get_current_board

        os.environ["WAYNE_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.wayne/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Wayne — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Wayne still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``WAYNE_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("WAYNE_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    try:
        from wayne_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from wayne_cli.config import load_config as _load_config_for_xai_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from Wayne itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (WAYNE_SAFE_MODE is checked by
    # wayne_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["WAYNE_SAFE_MODE"] = "1"
        os.environ["WAYNE_IGNORE_USER_CONFIG"] = "1"
        os.environ["WAYNE_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.wayne/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["WAYNE_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from wayne_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from wayne_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from wayne_constants import find_node_executable, with_wayne_node_path
    ...
    session_dir = get_wayne_home() / "whatsapp" / "session"
...
def cmd_whatsapp_cloud(args):
    ...
    from wayne_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from wayne_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from wayne_cli.config import stamp_install_method
    from wayne_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from wayne_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("WAYNE_INFERENCE_PROVIDER") or "auto"
    )
    ...
    # PROVIDER_GROUPS in wayne_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.wayne/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.wayne/.env if the active provider is not 'custom'.
    ...
    """
    from wayne_cli.config import get_env_value, save_env_value, load_config
...
#
# Wayne uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`wayne_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_flow_provider_model(
    ...
    from wayne_cli.auth import _prompt_model_selection
    from wayne_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from wayne_cli.setup import _curses_prompt_choice
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from wayne_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.wayne/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from wayne_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from wayne_cli.env_loader import format_secret_source_suffix
...
def cmd_login(args):
    """Authenticate Wayne CLI with a provider."""
    from wayne_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from wayne_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from wayne_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from wayne_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from wayne_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from wayne_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from wayne_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from wayne_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from wayne_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from wayne_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from wayne_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from wayne_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from wayne_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from wayne_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Wayne home directory to a zip file."""
    if getattr(args, "quick", False):
        from wayne_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from wayne_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Wayne backup from a zip file."""
    from wayne_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from wayne_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from wayne_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from wayne_cli.uninstall import run_gui_uninstall
    ...
    from wayne_cli.uninstall import run_uninstall
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``wayne`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="wayne-syntax-check-") as tmpdir:
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``wayne_cli/web_dist/`` (per vite.config.ts
    outDir: "../wayne_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "wayne_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``wayne`` launcher present but ``wayne_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the wayne venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — wayne venv python3, no nix-shell overhead
    ...
    # a wayne venv (pip / non-managed / bare-git installs).  The resolved
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m wayne_cli.main web` invocation.
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $WAYNE_HOME."""
    from wayne_constants import get_wayne_home
    return get_wayne_home() / "desktop-build-stamp.json"
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Wayne.app/Contents/MacOS/Wayne"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Wayne.exe",
            release_dir / "win-ia32-unpacked" / "Wayne.exe",
            release_dir / "win-arm64-unpacked" / "Wayne.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "wayne",
            release_dir / "linux-unpacked" / "Wayne",
            release_dir / "linux-arm64-unpacked" / "wayne",
            release_dir / "linux-arm64-unpacked" / "Wayne",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Wayne'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_wayne_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Wayne.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Wayne.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Wayne.exe: Access is
    ...
    an unrelated "Wayne" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Wayne is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Wayne.app/Contents/MacOS/Wayne  ->  app bundle = .../Wayne.app
    app = exe.parents[2]
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the WAYNE_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from wayne_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["WAYNE_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["WAYNE_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "wayne_root", None):
        env["WAYNE_DESKTOP_WAYNE_ROOT"] = str(Path(args.wayne_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["WAYNE_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Wayne.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Wayne.exe
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Wayne is
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Wayne Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``wayne serve`` as
    ...
    environment variable ``WAYNE_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "wayne dashboard",
        "wayne_cli.main dashboard",
        "wayne_cli/main.py dashboard",
        # The headless backend (`wayne serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "wayne serve",
        "wayne_cli.main serve",
        "wayne_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "wayne.*dashboard"`) keeps us consistent
            # with `wayne_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    """
    # When the Wayne Desktop Electron app spawns this dashboard as a
    # backend child, it sets WAYNE_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("WAYNE_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.wayne-update-staging"
    backup = f"{dst}.wayne-update-old"
...
def _update_via_zip(args):
    """ZIP-archive update path — disabled in the W4Y fork.

    Upstream Hermes downloaded a GitHub source ZIP of
    NousResearch/hermes-agent and unpacked it over the install (the Windows
    fallback for broken git file I/O). Applying that archive here would
    replace Wayne with upstream Nous code. Prints the disabled notice and
    returns.
    """
    print(SELF_UPDATE_DISABLED_MESSAGE)
    return
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "wayne-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes wayne completely unrunnable (SyntaxError on import).
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_wayne_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_wayne_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    """Fork/upstream sync — disabled in the W4Y fork.

    Upstream Hermes offered to run `git remote add upstream
    https://github.com/NousResearch/hermes-agent.git`, then fetched and
    fast-forward-pulled upstream/main over the checkout — exactly the code
    path this fork must never run. No-op.
    """
    return
...
def _invalidate_update_cache():
    ...
    default_home = get_default_wayne_root()
...
# never landed).  Lives next to the venv (not under $WAYNE_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    ...
    # Windows self-lock guard: if wayne.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _wayne_exe_shims(scripts_dir)
            if shims:
    ...
    try:
        ...
        try:
            from wayne_cli.managed_uv import ensure_uv
...
def _wayne_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_wayne_instances(
    ...
    for shim in _wayne_exe_shims(scripts_dir):
        try:
    ...
    #      wayne.exe sitting *under* a non-Wayne parent (e.g. a Wayne
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "wayne.exe"
    lines = ["✗ Another wayne.exe is running:"]
...
def _quarantine_running_wayne_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``wayne.exe``.
    ...
    the next wayne invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Wayne Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Wayne Desktop / gateway / REPL) and pointing to ``--force``.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_wayne_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``wayne.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``wayne.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``wayne`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_wayne_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``wayne.exe.old.*`` left by prior updates.

    Called early on every wayne invocation. The .old files are unlocked once
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``wayne.exe`` / ``wayne-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_wayne_exe`` for the rationale.
    """
    ...
    *,
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m wayne_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    #
    # Quarantine the running ``wayne.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``wayne`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from wayne_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$WAYNE_HOME/bin/uv``, but on Termux the official
    ...
    """
    from wayne_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    nixos_env = with_wayne_node_path(_nixos_build_env())
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.wayne/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.wayne/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # wayne_cli.config.get_wayne_home to simulate setup failure.
        from wayne_cli.config import get_wayne_home as _get_wayne_home

        logs_dir = _get_wayne_home() / "logs"
...
def _ensure_fhs_path_guard() -> None:
    ...
    ``wayne`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/wayne.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v wayne'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/wayne, code at /usr/local/lib/wayne-agent).
    fhs_link = Path("/usr/local/bin/wayne")
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of WAYNE_HOME before running the update.
    ...
    # ``wayne_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # WAYNE_HOME can add minutes to every update. Users who want the
    ...
    try:
        from wayne_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_wayne_home so the user sees ~/.wayne/...
    try:
        from wayne_constants import get_wayne_home, display_wayne_home

        home = get_wayne_home()
        try:
            display_path = f"{display_wayne_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        # dev may run wayne from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.wayne-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".wayne-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The wayne.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m wayne_cli.main
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    wayne.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from wayne_cli import gateway_windows
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m wayne_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from wayne_cli import gateway_windows
        from wayne_cli.gateway import find_gateway_pids
    except Exception as exc:
...
SELF_UPDATE_DISABLED_MESSAGE = (
    "Self-update is disabled in this W4Y fork — "
    "updates ship via the W4Y platform deploy flow."
)
...
def _cmd_update_pip(args):
    """PyPI update path — disabled in the W4Y fork.

    Upstream Hermes ran ``uv tool upgrade wayne-agent`` / ``pipx upgrade`` /
    ``pip install --upgrade wayne-agent`` here. The PyPI package of that name
    is upstream's code, not this fork's, so upgrading from it would replace
    Wayne with Nous code. Prints the disabled notice and returns.
    """
    print(SELF_UPDATE_DISABLED_MESSAGE)
    return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_wayne_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `wayne serve` backend) will keep .pyd
    ...
    # the wayne.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        # though `git pull` can't touch $WAYNE_HOME, this is cheap
        ...
        try:
            from wayne_cli.backup import create_quick_snapshot
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``wayne
        ...
        # the old bytecode (e.g. get_wayne_home added to wayne_constants).
        ...
        # the marker survives and the next ``wayne`` launch finishes the
        ...
        from wayne_cli.managed_uv import ensure_uv, update_managed_uv
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "wayne_cli.main", "desktop", "--build-only"]
        ...
        # current — so copy it straight over ~/.wayne/cache/model_catalog.json
        ...
        try:
            from wayne_cli.model_catalog import seed_cache_from_checkout
        ...
        # modules in this process.  Reload wayne_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_wayne_home() added since the last release.
        try:
            import importlib
            import wayne_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit WAYNE_HOME so
        # it is not affected by sync_skills()'s module-level WAYNE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's WAYNE_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from wayne_cli.profiles import backfill_profile_envs
        ...
        # it and warn loudly.
        try:
            from wayne_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from wayne_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_wayne_home() / ".update_exit_code"
            try:
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from wayne_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from wayne_constants import get_wayne_home as _hh
        ...
        from wayne_cli import profile_describer as _pd
        ...
        from wayne_cli.profiles import profile_exists, validate_alias_name
        ...
        from wayne_cli.profiles import rename_profile
        ...
        from wayne_cli.profiles import export_profile
        ...
        from wayne_cli.profiles import import_profile
        ...
        import tempfile
        from wayne_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="wayne_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from wayne_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from wayne_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from wayne_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from wayne_cli.web_server import should_require_auth
    ...
    try:
        from wayne_cli.dashboard_auth import list_providers
        if list_providers():
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → WAYNE_HOME points into
    ...
    if (
        ...
        and os.environ.get("WAYNE_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "wayne_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # WAYNE_HOME.  We must resolve the root explicitly instead of just
        # dropping WAYNE_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV WAYNE_HOME=/opt/data`), so an unset
        # WAYNE_HOME falls back to $HOME/.wayne = /opt/data/.wayne — an
        ...
        # update-button guard also misfires).  get_default_wayne_root()
        # returns the root for both layouts: ~/.wayne for a standard install
        ...
        try:
            from wayne_constants import get_default_wayne_root
            env["WAYNE_HOME"] = str(get_default_wayne_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop WAYNE_HOME) rather than block the reroute.
            env.pop("WAYNE_HOME", None)
    ...
    # the same logs directory as every other Wayne surface.
    try:
        from wayne_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "WAYNE_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "wayne_cli" / "web_dist"
        )
    ...
    from wayne_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from wayne_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from wayne_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from wayne_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from wayne_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Wayne log files."""
    from wayne_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Wayne command console."""
    from wayne_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from wayne_cli.models import CANONICAL_PROVIDERS as _cp
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("WAYNE_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from wayne_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from wayne_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["WAYNE_DEFER_AGENT_STARTUP"] = "1"
            os.environ["WAYNE_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["WAYNE_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_wayne_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_wayne_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_wayne_home()}/memories/\n")
    else:
        from wayne_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from wayne_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from wayne_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from wayne_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from wayne_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from wayne_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from wayne_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from wayne_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from wayne_cli.claw import claw_command
...
def main():
    ...
    # =========================================================================
    # model command  (parser built in wayne_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from wayne_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from wayne_cli.fallback_cmd import cmd_fallback
    ...
    from wayne_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from wayne_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in wayne_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in wayne_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in wayne_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in wayne_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in wayne_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from wayne_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in wayne_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in wayne_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in wayne_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in wayne_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in wayne_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in wayne_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from wayne_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from wayne_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from wayne_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in wayne_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in wayne_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in wayne_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in wayne_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in wayne_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in wayne_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from wayne_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in wayne_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in wayne_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in wayne_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in wayne_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in wayne_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from wayne_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in wayne_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    try:
        from wayne_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from wayne_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from wayne_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in wayne_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in wayne_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            # Honor WAYNE_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from wayne_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in wayne_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_wayne_home() / "sessions"
    ...
    # =========================================================================
    # insights command  (parser built in wayne_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in wayne_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in wayne_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in wayne_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in wayne_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in wayne_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in wayne_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in wayne_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in wayne_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in wayne_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in wayne_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    from wayne_cli.config import get_container_exec_info

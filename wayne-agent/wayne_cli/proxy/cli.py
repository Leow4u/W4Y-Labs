"""CLI handlers for the ``wayne proxy`` subcommand."""
...
from wayne_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"wayne auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Wayne proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Wayne proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: wayne proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``wayne proxy <subcommand>``."""
    ...
    print(
        "wayne proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  wayne proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  wayne proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  wayne proxy providers\n"

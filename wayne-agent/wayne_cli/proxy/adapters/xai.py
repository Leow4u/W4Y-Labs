...
# xAI's public API is OpenAI-compatible for the endpoints Wayne commonly
# uses. The Responses endpoint is included because Wayne' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Wayne-managed OAuth credentials."""

    auth_hint = "wayne auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`wayne auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`wayne auth reset xai-oauth` or re-authenticate with "
                    "`wayne auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `wayne auth add xai-oauth --type oauth`."
            )

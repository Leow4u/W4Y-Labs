"""Agent-construction and session-resume display methods for ``WayneCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.wayne/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``WayneCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``WayneCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        # `wayne chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `wayne auth add openai-codex`
        # without `wayne model`), fall back to the provider's first catalog
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Wayne: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Wayne: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")

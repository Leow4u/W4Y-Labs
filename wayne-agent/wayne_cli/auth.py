...
is persisted in ~/.wayne/auth.json with cross-process file locking.
...
from wayne_cli.config import get_wayne_home, get_config_path, read_raw_config
from wayne_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_CLIENT_ID = "wayne-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from wayne_cli import __version__ as _WAYNE_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _WAYNE_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"wayne-cli/{_WAYNE_CLI_VERSION}"
...
def get_anthropic_key() -> str:
    ...
    ``~/.wayne/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from wayne_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from wayne_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.wayne/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``wayne auth``.
    """
    return (
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("WAYNE_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.wayne/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_wayne_home() / "auth.json"
    # Seat belt: if pytest is running and WAYNE_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch WAYNE_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".wayne" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set WAYNE_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom WAYNE_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_wayne_root()
    except Exception:
        return None
    profile_home = get_wayne_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.wayne/auth.json`` even when WAYNE_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_wayne_root()`` for a profile-shaped WAYNE_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".wayne" / "auth.json"
            try:
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``wayne auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``wayne auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``wayne auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Wayne.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `wayne logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from wayne_cli.config import validate_config_structure
...
def resolve_provider(
    ...
    *,
    ...
    # Auto-detect an OpenRouter credential added via `wayne auth add openrouter`
    ...
    # `wayne auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.wayne/.env isn't silently
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``WAYNE_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Wayne at a non-production Nous Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``WAYNE_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("WAYNE_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `wayne model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.wayne/auth.json
# =============================================================================
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("WAYNE_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("WAYNE_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.wayne/.env, and return it.
    ...
    print("       App name:     anything (e.g. wayne-agent)")
...
def login_spotify_command(args) -> None:
    ...
    # with "WAYNE_SPOTIFY_CLIENT_ID is required".
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.wayne/auth.json (not ~/.codex/)
#
# Wayne maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Wayne auth store (~/.wayne/auth.json).
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``wayne setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``wayne auth add openai-codex``
      ...
      ``wayne auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Wayne auth store (~/.wayne/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``wayne auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Wayne auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Wayne auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run wayne auth" prompt (see issue #32790).
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Wayne auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Wayne keeps its OWN
        ...
        # the Codex CLI (or another Wayne process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Wayne's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("WAYNE_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("WAYNE_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Wayne processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.wayne/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.wayne/auth.json even when WAYNE_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``wayne auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.wayne/auth.json``. A single MITM during initial
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``WAYNE_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `wayne model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Wayne (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `wayne model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``wayne model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("WAYNE_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("WAYNE_XAI_BASE_URL", "").strip().rstrip("/")
...
# so a new `wayne --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${WAYNE_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<wayne-root>/shared/nous_auth.json`` where ``<wayne-root>`` is what
# ``get_default_wayne_root()`` returns — ``~/.wayne`` on Linux/macOS,
# ``%LOCALAPPDATA%\wayne`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's WAYNE_HOME so named profiles (which
# typically live under ``<wayne-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``WAYNE_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<wayne-root>/shared/``, where ``<wayne-root>`` is what
    :func:`wayne_constants.get_default_wayne_root` returns — so
    Linux/macOS classic installs land at ``~/.wayne/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\wayne\\shared\\``, and
    Docker / custom ``WAYNE_HOME`` deployments at
    ``<WAYNE_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("WAYNE_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from wayne_constants import get_default_wayne_root
    return get_default_wayne_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Wayne root, refuse rather than silently corrupt cross-profile
    # state. Tests must set WAYNE_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from wayne_constants import get_default_wayne_root
        real_home_shared = (
            get_default_wayne_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set WAYNE_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No WAYNE_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Wayne's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Wayne's next refresh uses it, and the whole
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Wayne models — they're not reliable for agentic tool-calling
            if "wayne" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Wayne is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )

        # WAYNE_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``wayne auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``wayne auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `wayne auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Wayne is not logged into Nous Portal.",
        ...
        portal_base_url = (
            _optional_base_url(state.get("portal_base_url"))
            or os.getenv("WAYNE_PORTAL_BASE_URL")
        ...
        # The trusted operator/deployment env override (WAYNE_PORTAL_BASE_URL /
...
# ~350ms even on the failure path, and read-only UI surfaces (`wayne tools`, status panels,
# subscription-feature checks) call it many times per render — `wayne tools` → "All Platforms"
...
# `wayne auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `wayne auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `wayne auth` and
    # `wayne model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("WAYNE_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("WAYNE_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``wayne doctor`` runs the live
    ...
    try:
        from wayne_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Wayne' "
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from wayne_cli.copilot_auth import (
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from wayne_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `wayne logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from wayne_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from wayne_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.wayne/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Wayne-owned credentials
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("WAYNE_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
    ...
    # Save tokens to Wayne auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    # xAI credential re-enabled. ``wayne auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``wayne auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Wayne auth store (~/.wayne/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Wayne login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``wayne status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Wayne login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``WAYNE_PORTAL_BASE_URL`` set
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Wayne builds still see

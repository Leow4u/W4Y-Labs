...
on Japanese locales, etc.).  Wayne's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``wayne_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Wayne is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``WAYNE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("WAYNE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Wayne-managed Git install directories so
    ...
    # already-running shells don't see the change — which means wayne
    ...
    # install without Wayne-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Wayne's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Wayne's
    ``wayne config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Wayne (User env var in Windows Settings, or
    export in a PowerShell profile) and Wayne picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Wayne-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\wayne\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``wayne`` from right after install) retain their old PATH.

    Any subprocess Wayne spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Wayne-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "wayne", "git", "cmd"),
        os.path.join(local_appdata, "wayne", "git", "bin"),
        os.path.join(local_appdata, "wayne", "git", "usr", "bin"),
        # Wayne venv Scripts directory — host of the wayne.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes wayne, but harmless to include.
        os.path.join(local_appdata, "wayne", "wayne-agent", "venv", "Scripts"),

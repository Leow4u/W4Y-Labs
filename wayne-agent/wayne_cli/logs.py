"""``wayne logs`` — view and filter Wayne log files.
...
Usage examples::
    ...
    wayne logs                    # last 50 lines of agent.log
    wayne logs -f                 # follow agent.log in real time
    wayne logs errors             # last 50 lines of errors.log
    wayne logs gateway -n 100    # last 100 lines of gateway.log
    wayne logs gui -f            # follow gui.log (dashboard/pty/ws)
    wayne logs desktop -f        # follow desktop.log (Electron app boot/backend)
    wayne logs --level WARNING    # only WARNING+ lines
    wayne logs --session abc123   # filter by session ID substring
    wayne logs --component tools  # only tool-related lines
    wayne logs --since 1h         # lines from the last hour
    wayne logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_wayne_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Wayne runs — try 'wayne chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_wayne_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_wayne_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_wayne_home()}/logs/")
        return

    print(f"Log files in {display_wayne_home()}/logs/:\n")
    found = False

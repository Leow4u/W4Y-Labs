...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.wayne/.env``
as ``WAYNE_DASHBOARD_OAUTH_CLIENT_ID``.
...
This command:
  ...
  3. Writes ``WAYNE_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``WAYNE_DASHBOARD_PORTAL_URL`` into ``~/.wayne/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``WAYNE_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from wayne_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _print_post_register_hint(
    *,
    ...
    from wayne_cli.config import get_env_path
...
def cmd_dashboard_register(args) -> None:
    ...
    from wayne_cli.auth import AuthError, resolve_nous_access_token
    from wayne_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets WAYNE_DASHBOARD_OAUTH_CLIENT_ID
    ...
    # WAYNE_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (WAYNE_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    #      WAYNE_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    # the portal, e.g. https://wayne.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking WAYNE_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as WAYNE_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("WAYNE_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("WAYNE_DASHBOARD_PUBLIC_URL", public_url)

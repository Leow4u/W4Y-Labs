...
``cli.py``'s ``WayneCLI`` class. ``WayneCLI`` inherits ``CLICommandsMixin`` so
...
from wayne_constants import display_wayne_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Wayne config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_wayne_home()}/state-snapshots/):\n")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `wayne journey`).
        ...
        from cli import _cprint
        from wayne_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from wayne_cli.clipboard import has_clipboard_image
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in wayne_cli/colors.py
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if target.isdigit():
            ...
            from wayne_cli.main import _resolve_session_by_name_or_id
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from wayne_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to wayne_cli.curator so the CLI and the `wayne curator`
        ...
        try:
            from wayne_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to wayne_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from wayne_cli.skills_hub import handle_skills_slash
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``wayne bundles list`` but renders inside the running
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Wayne-managed isolated debug "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from wayne_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_wayne_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="wayne_prompt_")
        try:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Wayne is working.
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from wayne_cli.models import _is_anthropic_fast_model
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        """Handle /debug — write a debug report bundle to local files.

        Accepts optional destination words after the command:

        - ``/debug``        → write report + full logs to local files (default)
        - ``/debug nous``   → write a single gzipped JSON bundle file
        - ``/debug local``  → render the report to stdout, no files written

        W4Y fork: remote upload is disabled — every destination is local.
        ...
        # ``expire``/``yes`` are legacy attributes kept for compatibility;
        # run_debug_share no longer reads them (nothing is uploaded, so no
        # consent prompt exists that could hang prompt_toolkit's event loop).

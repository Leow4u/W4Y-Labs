"""
wayne fallback — manage the fallback provider chain.
...
Subcommands:
  wayne fallback [list]   Show the current fallback chain (default when no subcommand)
  wayne fallback add      Pick provider + model via the same picker as `wayne model`,
                           then append the selection to the chain
  wayne fallback remove   Pick an entry to delete from the chain
  wayne fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.wayne/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from wayne_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `wayne model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  wayne fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `wayne model`, then append the selection to the chain."""
    from wayne_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `wayne model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `wayne fallback list` to view, or `wayne fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``wayne fallback [subcommand]``."""

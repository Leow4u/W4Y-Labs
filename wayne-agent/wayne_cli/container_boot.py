...
``$WAYNE_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-wayne-setup
(the stage2 hook) has chowned the volume and seeded $WAYNE_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$WAYNE_HOME``,
    not under ``profiles/``). The dispatcher in ``wayne_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``wayne gateway start`` (no ``-p``) targets. Without it,
    bare ``wayne gateway start`` inside the container would land on
    ...
    ``$WAYNE_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        wayne_home: The container's WAYNE_HOME (typically /opt/data).
            Profiles live under ``<wayne_home>/profiles/<name>/``;
            the default profile lives at ``<wayne_home>`` itself.
    ...
    # ``wayne gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(wayne_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(wayne_home)
    ...
    profiles_root = wayne_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            # via ``wayne -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(wayne_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = wayne_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("WAYNE_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the wayne args.
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    ``wayne`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``wayne <subcommand>``, so an
    explicit leading ``wayne`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `wayne <subcommand>`; peel an explicit wayne.
    if args and Path(args[0]).name == "wayne":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``wayne dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted WAYNE_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `wayne -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit wayne
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the wayne user) won't hit EACCES. See
...
def _write_reconcile_log(
    wayne_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $WAYNE_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted WAYNE_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        wayne_home=wayne_home, scandir=scandir,
    )
    for a in actions:
        print(

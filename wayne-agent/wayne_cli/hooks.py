"""wayne hooks — inspect and manage shell-script hooks.

Usage::

    wayne hooks list
    wayne hooks test <event> [--for-tool X] [--payload-file F]
    wayne hooks revoke <command>
    wayne hooks doctor

Consent records live under ``~/.wayne/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.wayne/config.yaml``
...
# stdin a script sees under `wayne hooks test` and `wayne hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    # is already allowlisted.  Otherwise `wayne hooks doctor` would execute

...
3. Running inside a container with no persistent volume mount over the
   WAYNE_HOME data dir (state is ephemeral — lost on container restart).
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Wayne as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("WAYNE_DESKTOP_CHILD_PID"):
...
def run_security_audit(
    *, wayne_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(wayne_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(wayne_home=wayne_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(

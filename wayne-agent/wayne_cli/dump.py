"""
Dump command for wayne CLI.

Outputs a compact, plain-text summary of the user's Wayne setup
...
from wayne_cli.config import get_wayne_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.wayne/.env.
    ...
    exports. ``wayne debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.wayne_build_sha`` by the Dockerfile's
    ``WAYNE_GIT_SHA`` build-arg (see ``wayne_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``wayne dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from wayne_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(wayne_home: Path) -> int:
...
def _cron_summary(wayne_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = wayne_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"wayne_home:      {display_wayne_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.wayne/.env: a managed
        ...
        # A credential added via `wayne auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `wayne auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["wayne-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(wayne_home)}")
    lines.append(f"  skills:             {_count_skills(wayne_home)}")

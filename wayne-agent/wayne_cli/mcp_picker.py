"""MCP picker — interactive `wayne mcp picker` (also the default `wayne mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``wayne mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `wayne plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from wayne_cli.cli_output import prompt_yes_no
from wayne_cli.curses_ui import curses_single_select
from wayne_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from wayne_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `wayne mcp catalog`."""
    if not rows:
        print()
    ...
    # Surface manifest-version warnings so users know when their Wayne is
...
def show_catalog() -> None:
    """`wayne mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`wayne mcp picker` (and default `wayne mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`wayne mcp install <name>` — non-interactive entry-point.

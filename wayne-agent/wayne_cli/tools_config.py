...
Saves per-platform tool configuration to ~/.wayne/config.yaml under
...
from wayne_cli.nous_subscription import (
...
from wayne_cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from wayne_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `wayne tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``wayne tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from wayne_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``wayne tools`` checklist actually offers
    for ``platform``.
    ...
    so ``wayne tools`` never claims to add or remove a toolset the user was
...
from wayne_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Wayne routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set WAYNE_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "WAYNE_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "WAYNE_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "WAYNE_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "WAYNE_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "WAYNE_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `wayne model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("WAYNE_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Wayne telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``wayne update`` and
      by ``wayne computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `wayne update` calls this
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    driver_cmd = _cua_driver_cmd()
    try:
        # When not verbose (e.g. `wayne update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `wayne update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.wayne/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Wayne process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_wayne_home()}/wayne-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in wayne-agent directory)")
            return
        ...
        # Run the full `wayne auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.wayne/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from wayne_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: wayne auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `wayne auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: wayne auth spotify")
        except Exception as exc:
        ...
        try:
            from wayne_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        try:
            from wayne_cli.auth import get_xai_oauth_auth_status
        ...
        try:
            ...
            _print_info("    Run later: wayne auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `wayne auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: wayne auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: wayne auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``wayne tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``wayne tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: wayne tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    # "wayne-cli" (which include all _WAYNE_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[wayne-cli, spotify]`` after enabling Spotify via ``wayne
        ...
        # (e.g. "wayne-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``wayne tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `wayne tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"wayne-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("wayne-")}
    ...
    # shipped as a bundled plugin but user must opt in via `wayne tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `wayne tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by wayne tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `wayne` instead of
    # `wayne-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `wayne update`/`wayne doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `wayne tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (wayne-cli, wayne-telegram, etc.)
    ...
    # Opening `wayne tools` is the user's opt-in to reconfigure tools, so treat
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from wayne_cli.curses_ui import curses_checklist
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `wayne tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `wayne model`).
    if managed_feature:
        from wayne_cli.nous_subscription import (
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``wayne model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `wayne model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `wayne model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `wayne tools` and `wayne setup tools`.
    ...
    print()
    from wayne_constants import display_wayne_home
    print(color(f"  Tool configuration saved to {display_wayne_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'wayne' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, wayne mcp

...
the existing module-level functions in wayne_cli.gateway and
wayne_cli.gateway_windows directly. This protocol is a thin facade
...
``wayne gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``wayne gateway start/stop/restart``).
    """
    ...
    # `wayne gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged wayne
    ...
    detection always fails. Since every Wayne runtime call inside the
    container drops to wayne via ``s6-setuidgid``, that silent failure
...
# in ``wayne_cli.gateway`` (systemd/launchd) and ``wayne_cli.gateway_windows``
...
# active (set via the ``wayne -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in wayne_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from wayne_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from wayne_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from wayne_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from wayne_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in wayne_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from wayne_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from wayne_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from wayne_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from wayne_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``wayne_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `wayne profile create`
# runs inside the container (Phase 4). Static services (main-wayne, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's WAYNE_HOME blindly;
    derive the shared profile root from the current WAYNE_HOME and map the
    ...
    wayne_home = Path(os.environ.get("WAYNE_HOME", "/opt/data"))
    if wayne_home.parent.name == "profiles":
        root = wayne_home.parent.parent
    else:
        root = wayne_home
...
# UID/GID of the in-image ``wayne`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the wayne user.
    ...
    (the ``wayne`` user — UID 10000 — running every Wayne runtime
    ...
    is dispatched as the wayne user.
    ...
    with wayne ownership before triggering ``s6-svscanctl -a``,
    ...
    slot's supervise/ was wayne-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _WAYNE_UID, _WAYNE_GID)
        except PermissionError:
            # Running as the wayne user already — directory is wayne-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _WAYNE_UID, _WAYNE_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is wayne-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _WAYNE_UID, _WAYNE_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``wayne -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`wayne profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-wayne, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources WAYNE_HOME (and any extra env) via with-contenv —
             so e.g. ``-e WAYNE_HOME=/data/wayne`` is honored at run
          ...
          4. Drops to the wayne user and exec's
             ``wayne -p <profile> gateway run`` (or just ``wayne
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``wayne gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root WAYNE_HOME profile" (the implicit profile that exists at
        the top of $WAYNE_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``wayne_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$WAYNE_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``WAYNE_HOME``,
        ...
        allocator in ``wayne_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export WAYNE_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's WAYNE_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``wayne gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-WAYNE_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The WAYNE_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own WAYNE_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${WAYNE_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the WAYNE_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e WAYNE_HOME=/data/wayne``
        gets its logs under /data/wayne/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``wayne logs`` reads and what
        ...
        return (
            ...
            f': "${{WAYNE_HOME:=/opt/data}}"\n'
            f'log_dir="$WAYNE_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as wayne and crash-loops on
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``wayne gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``wayne -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with wayne ownership
            ...
            # the wayne user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is wayne-owned (see ``_seed_supervise_skeleton``), and on

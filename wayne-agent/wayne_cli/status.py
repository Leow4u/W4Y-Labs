...
from wayne_cli.auth import AuthError, resolve_provider
from wayne_cli.colors import Colors, color
from wayne_cli.config import get_env_path, get_env_value, get_wayne_home, load_config
from wayne_cli.models import provider_label
from wayne_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from wayne_cli.nous_subscription import get_nous_subscription_features
from wayne_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``wayne config``'s
...
def show_status(args):
    ...
    print(color("│                 ⚕ Wayne Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: wayne portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: wayne model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: wayne auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: wayne auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: wayne model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from wayne_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from wayne_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   wayne gateway")
    ...
    jobs_file = get_wayne_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_wayne_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'wayne doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'wayne setup' to configure", Colors.DIM))
    print()

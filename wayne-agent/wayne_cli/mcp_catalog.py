...
entries via ``wayne mcp catalog`` or the interactive ``wayne mcp picker``,
and install them with ``wayne mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into wayne-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``wayne mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.wayne/.env`` (the
...
from wayne_constants import get_wayne_home, get_optional_mcps_dir
...
from wayne_cli.cli_output import prompt as _prompt_input
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Wayne install."""
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Wayne is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Wayne
        understands. Update Wayne to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_wayne_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.wayne/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.wayne/.env via save_env_value()."""
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``wayne mcp configure <name>``.
    """
    print()
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `wayne mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `wayne auth <provider>` flow. Surface guidance

"""Centralized logging setup for Wayne Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.wayne/logs/`` (profile-aware via ``get_wayne_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``wayne_cli.web_server``,
    ``wayne_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Wayne (TUI, gateway, ``hy_memory`` server, MCP
...
from wayne_constants import get_config_path, get_wayne_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_wayne_session_injector", False):
    ...
    _session_record_factory._wayne_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``wayne logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``wayne logs --component gateway``.
    "gateway": ("gateway", "wayne_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("wayne_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "wayne_cli.web_server",
        "wayne_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Wayne logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    wayne_home
        Override for the Wayne home directory.  Falls back to
        ``get_wayne_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_wayne_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._wayne_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the wayne group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from wayne_cli.config import is_managed

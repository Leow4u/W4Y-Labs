"""CLI entry point for the wayne-agent ACP adapter.

Loads environment variables from ``~/.wayne/.env``, configures logging
...
def _load_env() -> None:
    ...
    if loaded:
        ...
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", wayne_home / ".env"
        )
...
def _run_check() -> None:
    ...
    from acp_adapter.server import WayneACPAgent  # noqa: F401

    print("Wayne ACP check OK")
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "wayne", "model"]
        wayne_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``wayne postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting wayne-agent ACP adapter")
    ...
    import acp
    from .server import WayneACPAgent

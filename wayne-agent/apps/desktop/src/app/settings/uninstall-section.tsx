...
const OPTIONS: ModeOption[] = [
  ...
  // app installed, so gui-only is the correct (and only) option there.
        ...
        {loading ? (
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {u.confirmDesc(u.modes[pendingOption.mode].consequence)}
            </p>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <Button disabled={running} onClick={() => void handleConfirm()} size="sm" variant="destructive">
                ...
                {running ? u.running : u.confirmCta}
              </Button>
              ...
              {visibleOptions.map(opt => (
                ...
                >
                  ...
                  <span className="min-w-0">
                    <span className="block text-sm font-medium text-foreground">{u.modes[opt.mode].title}</span>
                    <span className="mt-0.5 block text-xs text-muted-foreground">
                      {u.modes[opt.mode].description}
                    </span>
                  </span>
                </button>
              ))}

...
function encryptDesktopSecret(value, safeStorageApi) {
  ...
  if (!encryptionAvailable) {
    throw new Error(
      'Secure token storage is unavailable, so Work4You Desktop cannot save remote gateway tokens. ' +
        'Set WORK4YOU_DESKTOP_REMOTE_URL and WORK4YOU_DESKTOP_REMOTE_TOKEN in your environment, or enable OS keychain access and try again.'
    )
  }

...
// Our backend (wayne_cli/web_server.py) announces WAYNE_DASHBOARD_READY for
// both `wayne serve` and `wayne dashboard` (they boot the same start_server;
// see web_server.py around line 16439). Accept a WAYNE_BACKEND_READY variant
// too so a future headless-specific announcement doesn't break the spawn.
const _READY_RE = /^WAYNE_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m

// The announcement clock starts the instant the backend process is spawned —
// before uvicorn binds its socket. On a cold install the child must first
// compile and import the whole `wayne_cli.main` → `web_server` → FastAPI/
...
// piling up orphaned processes. A roomier default absorbs the cold-start
// cost; a warm start still announces in well under a second.
 ...
 * WAYNE_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.WAYNE_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `WAYNE_(BACKEND|DASHBOARD)_READY
...
// Ready-file variant: the spawner sets WAYNE_DESKTOP_READY_FILE on the child
// (web_server.py `_write_dashboard_ready_file`, ~line 16188) and passes the
// same path here as `readyFile`; the backend writes {"port": N} atomically.

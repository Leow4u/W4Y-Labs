...
// Backend subcommand routing for the desktop-managed Work4You backend process.
//
// The desktop app launches its own headless backend via `wayne serve` — it
...
// app hasn't updated yet, or an older `wayne` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Wayne profile to pin via `--profile`.
 */
 ...
 * `-m wayne_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `wayne_cli/subcommands/dashboard.py` source registers
 ...
 * never produces a false positive. `\s*` spans newlines, so the multi-line
 * `subparsers.add_parser(\n    "serve",` layout in wayne_cli matches too.
 */

#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Work4You icon + version metadata onto the
// built Work4You.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
// apps/work4you/package.json sets build.win.signAndEditExecutable=false. That
...
//   node scripts/set-exe-identity.cjs <path-to-Work4You.exe>
//
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  // Icon lives at apps/work4you/assets/icon.ico
  const icon = path.join(desktopRoot, 'assets', 'icon.ico')
  if (!fs.existsSync(icon)) {
    throw new Error(`icon not found: ${icon}`)
  }

  // rcedit is a direct devDependency of apps/work4you, so it resolves whether

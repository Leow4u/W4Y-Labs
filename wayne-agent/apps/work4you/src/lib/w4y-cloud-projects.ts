...
/** User explicitly chose Local — do not auto-switch back to cloud on login. */
export const $runTargetExplicitLocal = persistentAtom<boolean>('hermes.desktop.runTargetExplicitLocal', false, {
  decode: raw => raw === 'true',
...
export function markRunTargetUserChoice(target: RunTarget): void {
  $runTargetExplicitLocal.set(target === 'local')
}

/**
 * Cloud-first default when signed in (Trilha F1). Local stays opt-in via the chip.
 * No-op when cloud bridge is unavailable or the user is signed out.
 */
export async function applyCloudFirstRunTargetDefault(): Promise<void> {
  if (!cloudRunAvailable() || $runTargetExplicitLocal.get()) {
    return
  }

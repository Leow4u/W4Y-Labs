...
async function rememberConnection(connectionId: string): Promise<void> {
  ...
  try {
    ...
    // The source is already usable.
  }
}

/**
 * Load the registry once for Sessions and restore the last successfully used
 * source. Later registry refreshes stay side-effect free.
 */
...
/** Re-home Sessions to one registered source, restoring that source's last profile. */

...
export function useSessionActions({
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      const cloudSession = isCloudBrainSession(storedForProfile)
      ...
      try {
        ...
        try {
          if (prefetchPromise) {
            ...
            if (isCurrentResume() && storedMessages?.messages) {
        ...
        try {
          const fallback = cloudSession
            ? await getCloudSessionMessages(storedSessionId)
            : await getSessionMessages(storedSessionId, sessionProfile)
  ...
  const removeSession = useCallback(
    async (storedSessionId: string) => {
      ...
      try {
        ...
        if (isCloudBrainSession(removed)) {
          await deleteCloudSession(storedSessionId)
        } else {
  ...
  const archiveSession = useCallback(
    async (storedSessionId: string) => {
      ...
      try {
        if (isCloudBrainSession(archived)) {
          await patchCloudSession(storedSessionId, { archived: true })
        } else {
  ...
  const unarchiveSession = useCallback(
    async (storedSessionId: string) => {
      ...
      try {
        if (isCloudBrainSession(archived)) {
          await patchCloudSession(storedSessionId, { archived: false })
        } else {

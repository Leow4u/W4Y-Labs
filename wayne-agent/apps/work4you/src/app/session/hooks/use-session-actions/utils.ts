...
import { getCloudSession } from '@/lib/cloud-sessions'
...
export async function resolveStoredSession(storedSessionId: string): Promise<SessionInfo | undefined> {
  ...
  const cloudSession = await getCloudSession(storedSessionId)
  if (cloudSession) {
    upsertResolvedSession(cloudSession, storedSessionId)
    return cloudSession
  }

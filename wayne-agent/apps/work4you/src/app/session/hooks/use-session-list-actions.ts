...
import { probeCloudLogin } from '@/lib/w4y-cloud-projects'
import {
  isMessagingSource,
...
export function useSessionListActions({ profileScope }: UseSessionListActionsArgs) {
  ...
  const refreshSessions = useCallback(async () => {
    ...
    try {
      ...
      let merged = result.sessions
      let total = typeof result.total === 'number' ? result.total : result.sessions.length

      const cloudLoggedIn = await probeCloudLogin()
      if (cloudLoggedIn === true) {
        const cloud = await listCloudSessions(limit, 1, 'exclude', 'recent', {
          excludeSources: SIDEBAR_EXCLUDED_SOURCES
        })
        merged = mergeCloudAndLocalSessions(result.sessions, cloud.sessions)
        total = Math.max(total, cloud.total ?? cloud.sessions.length, merged.length)
      }

      if (refreshSessionsRequestRef.current === requestId) {
        setSessions(prev => mergeSessionPage(prev, merged, sessionsToKeep()))

...
export function ConnectionsRegistrySection() {
  ...
  return (
    <div className="mt-8 border-t border-border/60 pt-6">
      ...
      <p className="mb-4 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">{s.stagedNote}</p>

      {!loading && showSearch && (
        <div className="relative mt-3 mb-0 w-full max-w-sm">
      ...
      <ConfirmDialog
        confirmLabel={s.plainTextConfirmAction}
        description={s.plainTextConfirmDesc}
        destructive
        onClose={() => setPlainTextConfirm(false)}
        onConfirm={() => save(true)}
        open={plainTextConfirm}
        title={s.plainTextConfirmTitle}
      />
    </div>
  )
}

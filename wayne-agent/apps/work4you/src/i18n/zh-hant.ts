...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: '正在啟動 Work4You…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 Work4You。',
      ...
      imageGenNeedsSetup: '影像生成需要在設定 → 工具中完成設定。',
      modelCatalogRejected: '模型要求被拒絕。請檢查設定 → 模型或你的方案。',
  ...
  settings: {
    ...
    appearance: {
      ...
      findMoreThemes: '尋找更多主題',
      findMoreThemesClose: '隱藏主題庫',
      gallerySearchPlaceholder: '搜尋主題庫…',
      marketplaceLoading: '正在搜尋主題庫…',
      marketplaceError: '無法連接主題庫。',
      marketplaceEmpty: '主題庫中沒有符合的主題。',
      ...
      pet: {
        ...
        unreachable: '無法載入寵物畫廊。請檢查網路連線後再試。',
        emptyGallery: '尚未安裝寵物。請在上方搜尋或產生一隻開始使用。',
        loadingGallery: '正在載入寵物…',
        retryGallery: '重試',
    ...
    account: {
      ...
      email: '電子郵件',
      signedOutName: '未登入',
      signedOutEmail: '登入後即可在各裝置間同步 Work4You 帳戶。',
      planUsageGroup: '方案與用量',
      currentPlan: '目前方案',
      currentPlanDesc: '包含用量依計費週期重設。包含額度用盡後進入按需用量。',
      planHobby: 'Hobby',
      planStatusPastDue: '逾期未付',
      planStatusCanceled: '已取消',
      upgrade: '升級',
      includedUsage: '包含用量',
      includedUsageDesc: '本週期方案包含額度的已用比例。',
      includedUsagePct: pct => `已用 ${pct}%`,
      includedUsageUnavailable: '用量暫不可用',
      includedDepleted: '包含額度已用盡',
      onDemand: '按需用量',
      onDemandDesc: '包含額度用盡後可繼續使用，直至支出上限。',
      onDemandNeedsSubscription: '需要有效的付費方案並綁定付款方式。',
      onDemandInactive: '關閉',
      onDemandUsage: '本週期按需',
      onDemandUsageDesc: '本計費週期超出包含用量的支出。',
      onDemandUsageValue: (used, limit) => `$${used.toFixed(2)} / $${limit.toFixed(2)}`,
      spendLimit: '支出上限',
      spendLimitDesc: max => `每週期硬頂（最高 $${max.toFixed(0)}）。會提高用量上限。`,
      spendLimitSave: '儲存',
      spendLimitSaving: '儲存中…',
      spendLimitSaveFailed: '無法儲存支出上限',
      manageSubscription: '管理訂閱',
      manageSubscriptionDesc: '在 Stripe 中變更方案、付款方式或取消。',
      manageSubscriptionNoCustomer: '尚無 Stripe 訂閱 — 開啟方案頁以訂閱。',
      planLogicHint:
        '先使用包含額度，再按需直至支出上限。指揮中心 → 用量是代理遙測，不是帳單。'
    },
    about: {
      heading: 'Work4You',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Work4You 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Work4You 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'Work4You 將使用已儲存的設定重新連線。',
    ...
    toolsets: {
      ...
      ready: '就緒',
      nousIncluded: '包含在 Work4You 訂閱中；登入帳戶即可啟用。',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '設定', detail: '設定 Work4You' },
  ...
  profiles: {
    ...
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 Work4You 環境：各自擁有獨立的設定、技能和指令。',
    nameLabel: '名稱',
    cloneFrom: '複製來源',
    cloneFromNone: '無（空白）',
    cloneFromDesc: '從選取的來源設定檔複製設定、技能和指令。',
    cloneFromDefault: '從預設設定檔複製設定',
    cloneFromDefaultDesc: '從您的預設設定檔複製設定、技能和指令。',
    ...
    failedCopy: '複製安裝指令失敗',
    failedLoadSoul: '載入指令失敗',
    failedSaveSoul: '儲存指令失敗',
  ...
  cron: {
    close: '關閉自動化',
    title: '自動化',
    subtitle: '用始終在線的雲端智慧體自動處理重複任務，回應環境觸發條件。',
    count: count => `${count} 個工作`,
    search: '搜尋排程工作…',
    loading: '正在載入排程工作…',
    statTotal: '自動化總數',
    statSuccessful24h: '成功 · 24 小時',
    statFailed24h: '失敗 · 24 小時',
    ...
    failedSave: '儲存排程工作失敗',
    driftTitle: '自建立此自動化以來，預設模型或提供商已變更',
    driftGeneric: '全域推理設定與建立此自動化時不一致。',
    driftModelChange: (from, to) => `模型：${from} → ${to}。`,
    driftProviderChange: (from, to) => `提供商：${from} → ${to}。`,
    driftHint: '為防止意外費用，執行已被封鎖。請選擇如何繼續。',
    driftAcceptCurrent: '使用目前設定',
    driftKeepOriginal: '保留原始模型',
    driftResolved: '推理設定已更新',
    failedDriftResolve: '更新推理設定失敗',
    duplicateAutomation: '複製',
    duplicateName: name => `${name} 副本`,
    duplicatedAutomation: '已複製自動化',
    failedDuplicate: '複製自動化失敗',
    exportAutomation: '匯出 JSON',
    exportedAutomation: '自動化已匯出',
    toolsSummaryDefault: '設定檔預設',
    toolsSummarySkills: count => `${count} 個技能`,
    toolsSummaryToolsets: count => `${count} 個工具集`,
    editTitle: '編輯排程工作',
    createTitle: '新排程工作',
    ...
    createAction: '建立自動化',
    triggersSection: '觸發器',
    triggersHint: '此自動化何時執行。目前支援排程觸發；應用程式事件觸發即將推出。',
  ...
  artifacts: {
    ...
    noArtifactsDesc: '代理建立的檔案、圖片和連結會顯示在這裡 — 不包括網頁搜尋中的引用。',
    noImagesTitle: '尚無產生的圖片',
    noImagesDesc: 'image_generate 或 MEDIA: 輸出的圖片會顯示在這裡。',
    noFilesTitle: '尚無產生的檔案',
    noFilesDesc: '代理寫入或修改的檔案會顯示在這裡。',
    noLinksTitle: '尚無發布的連結',
    noLinksDesc: '輸出工具回傳的下載或發布 URL 會顯示在這裡 — 不包括聊天中提到的連結。',
    noMatchTitle: '沒有符合的成品',
    noMatchDesc: '請嘗試其他搜尋詞或切換分頁。',
    sessionsBadge: (count, latestTitle) => (count === 1 ? latestTitle : `${latestTitle} · ${count} 個工作階段`),
    failedLoad: '成品載入失敗',
    openFailed: '開啟失敗',
    itemsImage: '張圖片',
    itemsLink: '個連結',
  ...
  updates: {
    ...
    guiSkewBody:
      '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 Work4You（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody:
      'Work4You會在自己的視窗中接管，並在完成後自動重新開啟 Work4You。更新期間請勿自行重新開啟 Work4You。',
  ...
  install: {
    ...
    activeDesc: '這是一次性設定。Work4You正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  shell: {
    ...
    modelMenu: {
      ...
      medium: '中',
      autoMode: 'Auto',
      autoModeHint: '平衡品質與速度，建議用於大多數任務',
      switchToSpecific: '切換到指定模型',
      specificModel: '指定模型'
    },
    modelOptions: {
      noOptions: '此模型沒有可用選項',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 Work4You 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 Work4You 以儲存圖片',
  ...
  browserShell: {
    terminalUnavailableTitle: '本機終端機需要桌面版應用',
    terminalUnavailableBody:
      '在瀏覽器中，代理工具仍會在雲端電腦上執行。安裝 Work4You 以在本機使用互動式 shell。'
  },

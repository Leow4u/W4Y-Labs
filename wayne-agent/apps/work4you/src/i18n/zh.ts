...
export const zh: Translations = {
  ...
  settings: {
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及可通过远程、SSH 或云连接访问的每个 Work4You 网关。',
      stagedNote:
        '在会话中切换网关。配置文件、聊天、消息和定时任务留在各自网关；其他网关上的工作会继续运行。',
      launchModeTitle: '启动时返回上次使用的网关会话',
      launchModeDesc: '关闭时，会话在主网关上打开。',
      searchPlaceholder: '搜索网关…',
      noSearchResults: '没有匹配的网关。',
      loadFailed: '无法加载连接',
      currentPill: '当前',
      primaryPill: '主网关',
      managedPill: '应用管理',
      ...
      removeConfirmDesc: (label: string) =>
        `将从本应用移除“${label}”。实例本身不会改动 — 随时可以再次添加。`,
      makePrimary: '设为主网关',
      testConnection: '测试',
      testOk: '可达',
      testFailed: '连接测试失败',
      ...
      removeFailed: '无法移除连接',
      updateAll: '更新全部实例',
      updateAllRunning: '正在更新全部实例…',
      updateAllDone: '已分发更新',
      updateAllFailed: '批量更新失败',
      updateSkippedCloud: '由 Work4You Cloud 管理',
      kindLocal: '本地',
      kindRemote: '远程网关',
      ...
      kindCloudDesc: '通过你的 Work4You 账号发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 Work4You 安装。',
      labelTitle: '名称',
      labelDesc: '必填。在本应用中显示；必须唯一（例如“家庭实验室”、“工作电脑”）。',
      labelPlaceholder: '家庭实验室',
      urlTitle: '网关 URL',
      sshHostTitle: 'SSH 主机',
      headersTitle: '额外网关请求头',
      headersDesc:
        '发往该网关的每个 HTTP 和 WebSocket 请求都会带上 — 例如 Cloudflare Access（CF-Access-Client-Id / CF-Access-Client-Secret）。值会加密存储。',
      headerValuePlaceholder: '值',
      headerValueSaved: '已保存 — 留空以保留',
      headerAdd: '添加请求头',
      headerRemove: '移除',
      duplicateLocal: '本应用已有本地连接 — 只能有一个。',
      ...
      localAddHint: '本地不可用：已存在受管本地连接（永远只有一个）。',
      cloudAddHint:
        '提示：在 Work4You Cloud 登录会自动发现你的代理 — 仅在需要手动注册已知实例 URL 时使用此表单。',
      save: '保存连接',
      saving: '保存中…',
      cancel: '取消',
      empty: '尚未注册连接。',
      plainTextConfirmTitle: '在没有钥匙串加密的情况下存储令牌？',
      plainTextConfirmDesc: '此设备无法使用系统钥匙串。继续保存将以明文存储令牌，除非你取消。',
      plainTextConfirmAction: '仍然保存'
    },
    gateway: {
      loading: '正在加载网关设置...',
    ...
    mcp: {
      ...
      reload: '重新加载 MCP',
      reloading: '重新加载中...',
      emptyTitle: '用 MCP 连接外部工具',
      emptyDesc: 'Model Context Protocol 服务器将 Work4You 连接到 Linear、Figma、Notion 等外部工具与数据源。',
      disabled: '已禁用',
      editServer: '编辑服务器',
      name: '名称',
      serverJson: '服务器 JSON',
    ...
    toolsets: {
      ...
      ready: '就绪',
      nousIncluded: '包含在 Work4You 订阅中；登录账户即可激活。',
  ...
  skills: {
    ...
    manageConnected: '管理',
    documentation: '文档',
    searchFor: (kind, scope) => `搜索 ${kind}（${scope}）…`,
    all: '全部',
    searchSkills: '搜索技能…',
    ...
    rowMenu: '技能操作',
    addSkill: '+ 添加',
    userSection: count => `用户技能${count > 0 ? `（${count}）` : ''}`,
    newSkillTitle: '新建技能',
    newSkillDesc: '在已学技能下创建 SKILL.md，创建后可立即编辑。',
    newSkillPlaceholder: 'skill-name',
    newSkillHint: '小写字母、数字、点、连字符和下划线。',
    skillCreated: '技能已创建',
    createFailed: '无法创建技能',
    editLearnedOnlyTitle: '仅可打开已学技能',
    editLearnedOnlyDesc: '中心与项目技能请在源文件中编辑——已学技能会在下方编辑器中打开。',
    archive: '归档',
  ...
  cron: {
    ...
    failedSave: '保存自动化失败',
    driftTitle: '自创建此自动化以来，默认模型或提供商已更改',
    driftGeneric: '全局推理设置与创建此自动化时不一致。',
    driftModelChange: (from, to) => `模型：${from} → ${to}。`,
    driftProviderChange: (from, to) => `提供商：${from} → ${to}。`,
    driftHint: '为防止意外费用，运行已被阻止。请选择如何继续。',
    driftAcceptCurrent: '使用当前设置',
    driftKeepOriginal: '保留原始模型',
    driftResolved: '推理设置已更新',
    failedDriftResolve: '更新推理设置失败',
    duplicateAutomation: '复制',
    duplicateName: name => `${name} 副本`,
    duplicatedAutomation: '已复制自动化',
    failedDuplicate: '复制自动化失败',
    exportAutomation: '导出 JSON',
    exportedAutomation: '自动化已导出',
    toolsSummaryDefault: '配置文件默认',
    toolsSummarySkills: count => `${count} 个技能`,
    toolsSummaryToolsets: count => `${count} 个工具集`,
    editTitle: '编辑自动化',
    createTitle: '新建自动化',
    ...
    createAction: '创建自动化',
    triggersSection: '触发器',
    triggersHint: '此自动化何时运行 — 定时、应用事件或 webhook。',
    instructionsSection: '代理说明',
    instructionsHint: '每次自动化触发时代理应执行的操作。',
    toolsSection: '工具',
    toolsHint:
      '选择此自动化可使用的应用和工具。连接器和投递按自动化配置 — 不会更改全局配置。',
    openConnectors: '连接器',
    openChannels: '频道',
    addJobConnector: '连接器',
    searchJobConnectors: '搜索连接器…',
    noJobConnectors: '暂无已连接的连接器',
    manageConnectorsLink: '管理连接器…',
    jobConnectorsHint: '仅在此添加的连接器可用于此自动化。',
    jobConnectorsLegacyHint: '旧版自动化：在添加明确列表之前，所有已连接的应用均可用。',
    jobConnectorAdded: '已添加',
    browseConnectorsMarketplace: '浏览市场…',
    tabSettings: '设置',
    tabHistory: '运行历史',
    addTrigger: '添加触发器',
    ...
    webhookHint: '启用 webhook 频道以获取 URL，或添加应用事件触发器以注册连接器 webhook。',
    ...
    jobSkillsHint: '每次运行前加载这些 skills（可选）。',
    addJobSkill: '添加 skill',
    searchJobSkills: '搜索 skills…',
    noJobSkills: '没有匹配的 skills',
    noJobSkillsSelected: '未附加 skills — 使用配置默认值。',
    jobToolsetsLabel: 'Toolsets',
    jobToolsetsHint: '限制此自动化可使用的工具集（可选）。',
    addJobToolset: '添加 toolset',
    searchJobToolsets: '搜索 toolsets…',
    noJobToolsets: '没有匹配的 toolsets',
  ...
  connectors: {
    ...
    disconnect: '断开',
    disconnectAll: '断开全部',
    disconnectAllConfirm: '撤销此代理的全部已连接应用账号？连接时聊天会再次要求授权。',
    ...
    backToFeatured: '返回精选',
    showMore: count => `显示另外 ${count} 个`,
    showLess: '收起',
    emptyConnectedTitle: '连接外部工具',
  ...
  artifacts: {
    ...
    noArtifactsDesc: '代理创建的文件、图片和链接会显示在这里 — 不包括网页搜索中的引用。',
    noImagesTitle: '暂无生成的图片',
    noImagesDesc: 'image_generate 或 MEDIA: 输出的图片会显示在这里。',
    noFilesTitle: '暂无生成的文件',
    noFilesDesc: '代理写入或修改的文件会显示在这里。',
    noLinksTitle: '暂无发布的链接',
    noLinksDesc: '输出工具返回的下载或发布 URL 会显示在这里 — 不包括聊天中提到的链接。',
    noMatchTitle: '没有匹配的产物',
    noMatchDesc: '尝试其他搜索词或切换标签页。',
    sessionsBadge: (count, latestTitle) => (count === 1 ? latestTitle : `${latestTitle} · ${count} 个会话`),
    failedLoad: '产物加载失败',
    openFailed: '打开失败',
    itemsImage: '张图片',
    itemsLink: '个链接',

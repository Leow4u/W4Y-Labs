...
export const ja = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: 'Work4You を起動中…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Work4You を再起動してください。',
      ...
      imageGenNeedsSetup: '画像生成は設定 → ツールでセットアップが必要です。',
      modelCatalogRejected: 'モデル要求が拒否されました。設定 → モデルまたはプランを確認してください。',
  ...
  settings: {
    ...
    appearance: {
      ...
      findMoreThemes: 'ほかのテーマを探す',
      findMoreThemesClose: 'テーマギャラリーを閉じる',
      gallerySearchPlaceholder: 'テーマギャラリーを検索…',
      marketplaceLoading: 'テーマギャラリーを検索中…',
      marketplaceError: 'テーマギャラリーに接続できませんでした。',
      marketplaceEmpty: '一致するテーマがありません。',
      ...
      pet: {
        ...
        unreachable: 'ペットギャラリーを読み込めませんでした。接続を確認して再試行してください。',
        emptyGallery: 'まだペットがありません。上で検索するか生成して始めましょう。',
        loadingGallery: 'ペットを読み込み中…',
        retryGallery: '再試行',
    ...
    account: {
      ...
      displayName: '名前',
      email: 'メール',
      signedOutName: '未サインイン',
      signedOutEmail: 'サインインすると Work4You アカウントをデバイス間で同期できます。',
      planUsageGroup: 'プランと使用量',
      currentPlan: '現在のプラン',
      currentPlanDesc: '含まれる使用量は請求サイクルごとにリセットされます。枠を使い切るとオンデマンドが始まります。',
      planHobby: 'Hobby',
      planStatusPastDue: '支払遅延',
      planStatusCanceled: 'キャンセル済み',
      upgrade: 'アップグレード',
      includedUsage: '含まれる使用量',
      includedUsageDesc: '今サイクルのプラン枠の使用割合。',
      includedUsagePct: pct => `${pct}% 使用`,
      includedUsageUnavailable: 'メーター利用不可',
      includedDepleted: '含まれる枠は使い切りました',
      onDemand: 'オンデマンド使用',
      onDemandDesc: '含まれる枠の後も、支出上限まで作業を続けられます。',
      onDemandNeedsSubscription: '有効な有料プランと登録済みの支払い方法が必要です。',
      onDemandInactive: 'オフ',
      onDemandUsage: '今サイクルのオンデマンド',
      onDemandUsageDesc: '今請求サイクルで含まれる使用量を超えた支出。',
      onDemandUsageValue: (used, limit) => `$${used.toFixed(2)} / $${limit.toFixed(2)}`,
      spendLimit: '支出上限',
      spendLimitDesc: max => `サイクルごとの上限（最大 $${max.toFixed(0)}）。使用上限を引き上げます。`,
      spendLimitSave: '保存',
      spendLimitSaving: '保存中…',
      spendLimitSaveFailed: '支出上限を保存できませんでした',
      manageSubscription: 'サブスクリプション管理',
      manageSubscriptionDesc: 'Stripe でプラン・支払い方法の変更や解約。',
      manageSubscriptionNoCustomer: 'Stripe サブスクリプションはまだありません — プランページで登録してください。',
      planLogicHint:
        'まず含まれる枠、その後オンデマンドで支出上限まで。Command Center → Usage はエージェントのテレメトリであり、請求ではありません。'
    },
    about: {
      heading: 'Work4You',
    ...
    gateway: {
      ...
      intro:
        'Work4You はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Work4You バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      restartingMessage: 'Work4You は保存された設定を使用して再接続します。',
    ...
    mcp: {
      ...
      reload: 'MCP を再読み込み',
      reloading: '再読み込み中...',
      emptyTitle: 'MCP で外部ツールを接続',
      emptyDesc:
        'Model Context Protocol サーバーは、Linear、Figma、Notion などの外部ツールやデータソースに Work4You を接続します。',
      disabled: '無効',
      editServer: 'サーバーを編集',
      name: '名前',
    ...
    toolsets: {
      ...
      ready: '準備完了',
      nousIncluded: 'Work4You サブスクリプションに含まれています。有効にするにはアカウントにサインインしてください。',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Work4Youを設定' },
  ...
  profiles: {
    ...
    unsavedChanges: '未保存の変更',
    loadingSoul: '指示を読み込み中...',
    emptySoul: '空 — ペルソナの指示を書き始めてください...',
    saving: '保存中...',
    saveSoul: '指示を保存',
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Work4You 環境です：設定、スキル、指示が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
    cloneFromDesc: '選択したプロファイルから設定、スキル、指示をコピーします。',
    cloneFromDefault: 'デフォルトプロファイルから設定を複製',
    cloneFromDefaultDesc: 'デフォルトプロファイルから設定、スキル、指示をコピーします。',
    ...
    failedLoadSoul: '指示の読み込みに失敗しました',
    failedSaveSoul: '指示の保存に失敗しました',
  ...
  cron: {
    ...
    driftTitle: 'この自動化作成以降、デフォルトのモデルまたはプロバイダーが変更されました',
    driftGeneric: 'グローバルな推論設定が作成時と一致しません。',
    driftModelChange: (from, to) => `モデル: ${from} → ${to}.`,
    driftProviderChange: (from, to) => `プロバイダー: ${from} → ${to}.`,
    driftHint: '意図しない課金を防ぐため実行がブロックされました。続行方法を選択してください。',
    driftAcceptCurrent: '現在の設定を使う',
    driftKeepOriginal: '元のモデルを維持',
    driftResolved: '推論設定を更新しました',
    failedDriftResolve: '推論設定の更新に失敗しました',
    duplicateAutomation: '複製',
    duplicateName: name => `${name} のコピー`,
    duplicatedAutomation: '自動化を複製しました',
    failedDuplicate: '自動化の複製に失敗しました',
    exportAutomation: 'JSON をエクスポート',
    exportedAutomation: '自動化をエクスポートしました',
    toolsSummaryDefault: 'プロファイル既定',
    toolsSummarySkills: count => `スキル ${count} 件`,
    toolsSummaryToolsets: count => `ツールセット ${count} 件`,
    ...
    saveChanges: '変更を保存',
    createAction: '自動化を作成',
    triggersSection: 'トリガー',
    triggersHint: 'この自動化を実行するタイミング。スケジュールは利用可能です。アプリイベントのトリガーは今後追加予定です。',
  ...
  connectors: {
    ...
    disconnectAllConfirm:
      'このエージェントの Composio アカウントをすべて取り消しますか？接続時にチャットで再承認が必要になります。',
    disconnectAllDone: '{count} 件のアカウントを切断しました',
  ...
  artifacts: {
    ...
    noArtifactsDesc: 'エージェントが作成したファイル、画像、リンクがここに表示されます — Web 検索の引用は含みません。',
    noImagesTitle: '生成された画像はありません',
    noImagesDesc: 'image_generate や MEDIA: 出力の画像がここに表示されます。',
    noFilesTitle: '生成されたファイルはありません',
    noFilesDesc: 'エージェントが書き込みまたは変更したファイルがここに表示されます。',
    noLinksTitle: '公開リンクはありません',
    noLinksDesc: '出力ツールが返すダウンロードまたは公開 URL がここに表示されます — チャットで言及されたリンクは含みません。',
    noMatchTitle: '一致するアーティファクトがありません',
    noMatchDesc: '別の検索語を試すか、タブを切り替えてください。',
    sessionsBadge: (count, latestTitle) => (count === 1 ? latestTitle : `${latestTitle} · ${count} セッション`),
  ...
  updates: {
    ...
    applyingBody:
      'Work4Youが独自のウィンドウで引き継ぎ、完了後に自動的に Work4You を再度開きます。更新中はご自分で Work4You を開き直さないでください。',
  ...
  install: {
    ...
    activeDesc:
      'これは一回限りのセットアップです。Work4Youが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  shell: {
    ...
    modelMenu: {
      ...
      medium: '中',
      autoMode: 'Auto',
      autoModeHint: '品質と速度のバランス。ほとんどの用途におすすめ',
      switchToSpecific: '特定モデルに切替',
      specificModel: '特定モデル'
    },
    modelOptions: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'Work4You 推論ゲートウェイのステータス',
      agents: 'エージェント',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Work4You を再起動してください。',
    restartToSaveImages: '画像を保存するには Work4You を再起動してください',
  ...
  browserShell: {
    terminalUnavailableTitle: 'ローカルターミナルはデスクトップ版が必要です',
    terminalUnavailableBody:
      'ブラウザではエージェントツールはクラウド上で動作します。対話型シェルには Work4You デスクトップ版をインストールしてください。'
  },

...
const STORAGE_KEY = "wayne-dashboard-theme";
...
const FONT_STORAGE_KEY = "wayne-dashboard-font";
...
const THEME_NAME_ALIASES: Record<string, string> = {
  // W4Y 2026-07-04: theme set reduced to white/mono/cyberpunk/rose.
  // Old dark themes land on "mono" (Black); old light/default land on "white".
  "lens-5i": "white",
  "nous-blue": "white",
  default: "white",
  "default-large": "white",
 ...
 *  `_THEME_NAMED_ASSET_KEYS` in `wayne_cli/web_server.py`. */
...
const CUSTOM_CSS_STYLE_ID = "wayne-theme-custom-css";
...
function applyCustomCSS(css: string | undefined) {
  ...
  if (!el) {
    ...
    el.setAttribute("data-wayne-theme-css", "true");
...
function injectFontStylesheet(url: string | undefined) {
  ...
  link.rel = "stylesheet";
  link.href = url;
  link.setAttribute("data-wayne-theme-font", "true");
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [themeName, setThemeName] = useState<string>(() => {
    ...
    const stored = window.localStorage.getItem(STORAGE_KEY) ?? "white";
  ...
  const setTheme = useCallback(
    (name: string) => {
      ...
      const next = knownNames.has(name) ? name : "white";
      setThemeName(next);
      if (typeof window !== "undefined") {

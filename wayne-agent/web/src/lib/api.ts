import { buildWayneWebSocketUrl } from "@wayne/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/wayne/). The Python backend
// injects ``window.__WAYNE_BASE_PATH__`` into index.html based on the
...
export const WAYNE_BASE_PATH = readBasePath();
const BASE = WAYNE_BASE_PATH;
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``wayne update``, ``wayne gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__WAYNE_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__WAYNE_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("wayne.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("wayne.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__WAYNE_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("wayne.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__WAYNE_SESSION_TOKEN__;
  if (injected) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Wayne-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``wayne_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__WAYNE_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  checkWayneUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/wayne/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** wayne-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``wayne update``. */
  can_update_wayne?: boolean;
  config_path: string;
...
export interface EnvVarInfo {
  ...
  /** True when Work4You injects this credential from the cloud (writes are refused). */

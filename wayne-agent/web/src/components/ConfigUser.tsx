...
import { UserCircle, SlidersHorizontal, Sparkles, Shield, Database, LogOut, Package, Plug, Puzzle, Search } from "lucide-react";
...
export default function ConfigUser() {
  ...
  const searchText = (key: SectionKey): string => {
    ...
    else if (key === "general") p.push(cu.language, cu.appearance, cu.font, cu.timezone, cu.communication, cu.memoryNotif);
    else if (key === "personalization") p.push(cu.instructions);
    else if (key === "privacy") p.push(cu.memoryBetween, cu.userProfile, cu.redactPii);
    else if (key === "dataControl") p.push(cu.clearMemory, cu.clearSessions);
        ...
        <aside className="sm:w-52 sm:shrink-0 sm:flex sm:min-h-0 sm:flex-col">
          <div className="relative px-1 pb-2 pt-1">
            ...
            <input
              ...
              className="h-8 w-full rounded border border-input bg-transparent pl-8 pr-2 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
            />
          </div>

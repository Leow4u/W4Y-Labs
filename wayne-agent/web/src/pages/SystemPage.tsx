...
import { WayneConsoleModal } from "@/components/WayneConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `wayne import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_wayne === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkWayneUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_wayne],
  );
  ...
  const applyUpdate = async () => {
    ...
    try {
      const resp = await api.updateWayne();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "wayne-update");
  ...
  const canUpdateWayne = status?.can_update_wayne !== false;

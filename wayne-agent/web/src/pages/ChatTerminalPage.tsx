/**
 * ChatTerminalPage — embeds `wayne --tui` inside the dashboard.
 *
 * Curadoria: this is now the ?full=1 internal/support view — ChatPage.tsx
 * routes end users to NativeChatPage instead (same gateway protocol, HTML
 * bubbles instead of a terminal). This file is otherwise unchanged; the
 * embedded terminal keeps working exactly as before for support/dev use.
 *
...
export default function ChatTerminalPage({ isActive = true }: { isActive?: boolean }) {
  ...
  useEffect(() => {
    ...
    const gated = !!window.__WAYNE_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Wayne TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[wayne-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__WAYNE_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Wayne. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="wayne-chat-xterm-host min-h-0 min-w-0 flex-1"
          />

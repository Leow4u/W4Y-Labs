...
Uses a throwaway WAYNE_HOME so it never touches ~/.wayne. No real names.
"""
...
def _fresh_home():
    """Point WAYNE_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_wayne_home() at import time)."""
...
# Import AFTER WAYNE_HOME is set.
...
# a different get_wayne_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"WAYNE_HOME (throwaway): {HOME}")

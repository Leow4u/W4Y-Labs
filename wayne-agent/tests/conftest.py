"""Shared fixtures for the wayne-agent test suite.
...
2. **Isolated WAYNE_HOME.** WAYNE_HOME points to a per-test tempdir so
   code reading ``~/.wayne/*`` via ``get_wayne_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".wayne"`` instead of the canonical
   ``get_wayne_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No WAYNE_SESSION_* inheritance** — the agent's current gateway
...
# WAYNE_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_WAYNE_BEHAVIORAL_VARS = frozenset({
    "WAYNE_YOLO_MODE",
    "WAYNE_INTERACTIVE",
    "WAYNE_QUIET",
    "WAYNE_TOOL_PROGRESS",
    "WAYNE_TOOL_PROGRESS_MODE",
    "WAYNE_MAX_ITERATIONS",
    "WAYNE_SESSION_PLATFORM",
    "WAYNE_SESSION_CHAT_ID",
    "WAYNE_SESSION_CHAT_NAME",
    "WAYNE_SESSION_THREAD_ID",
    "WAYNE_SESSION_SOURCE",
    "WAYNE_SESSION_KEY",
    "WAYNE_GATEWAY_SESSION",
    "WAYNE_CRON_SESSION",
    "_WAYNE_GATEWAY",
    "WAYNE_PLATFORM",
    "WAYNE_MODEL",
    "WAYNE_INFERENCE_MODEL",
    "WAYNE_INFERENCE_PROVIDER",
    "WAYNE_TUI_PROVIDER",
    "WAYNE_MANAGED",
    "WAYNE_MANAGED_DIR",
    "WAYNE_DEV",
    ...
    "WAYNE_TIMEZONE",
    "WAYNE_REDACT_SECRETS",
    "WAYNE_BACKGROUND_NOTIFICATIONS",
    "WAYNE_EXEC_ASK",
    "WAYNE_HOME_MODE",
    "WAYNE_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.wayne/kanban.db instead of the per-test WAYNE_HOME.
    "WAYNE_KANBAN_DB",
    "WAYNE_KANBAN_BOARD",
    "WAYNE_KANBAN_HOME",
    "WAYNE_KANBAN_WORKSPACES_ROOT",
    "WAYNE_KANBAN_LOGS_ROOT",
    "WAYNE_KANBAN_TASK",
    "WAYNE_KANBAN_WORKSPACE",
    "WAYNE_KANBAN_RUN_ID",
    "WAYNE_KANBAN_CLAIM_LOCK",
    "WAYNE_KANBAN_DISPATCH_IN_GATEWAY",
    "WAYNE_TENANT",
    ...
    "WAYNE_DASHBOARD_OAUTH_CLIENT_ID",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and WAYNE_HOME to per-test tempdirs so code that
    reads ``~/.wayne/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral WAYNE_* vars that could change test semantics.
    for name in _WAYNE_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect WAYNE_HOME to a per-test tempdir. Code that reads
    #    ``~/.wayne/*`` via ``get_wayne_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.wayne/*`` via
    #    ``Path.home() / ".wayne"`` instead of ``get_wayne_home()``
    #    is a bug to fix at the callsite.
    fake_wayne_home = tmp_path / "wayne_test"
    fake_wayne_home.mkdir()
    (fake_wayne_home / "sessions").mkdir()
    (fake_wayne_home / "cron").mkdir()
    (fake_wayne_home / "memories").mkdir()
    (fake_wayne_home / "skills").mkdir()
    monkeypatch.setenv("WAYNE_HOME", str(fake_wayne_home))
...
@pytest.fixture(autouse=True)
def _isolate_wayne_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal wayne config dict suitable for unit tests."""
    return {
...
#    ``check_output`` reject any ``systemctl ... <verb> wayne-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart wayne-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting wayne/python patterns are also blocked.
    """
    ...
    def _matches_wayne_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _WAYNE_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_wayne_gateway(cmd_str):
            return False
        try:
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live wayne-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting wayne/python could hit the live gateway. "
        ...
        if "update" in low and (
            ...
            raise RuntimeError(
                ...
                "`wayne update` against the real checkout, fetching "

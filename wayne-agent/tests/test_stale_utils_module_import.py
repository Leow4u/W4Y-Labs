...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.wayne/wayne-agent/utils.py)
...
2. ``wayne update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``wayne_cli/main.py`` (the ``wayne update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``wayne_constants`` after the

...
Regression: the old implementation wrote ``wayne_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``wayne sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.wayne/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_wayne_home(wayne_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.wayne/sessions/saved/, not CWD."""
    ...
    cli.WayneCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("wayne_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.wayne/sessions/saved/
    saved_dir = wayne_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("wayne_conversation_*.json"))

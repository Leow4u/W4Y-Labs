...
def _isolate(tmp_path, monkeypatch):
    """Isolate WAYNE_HOME so tests don't touch real config."""
    home = tmp_path / ".wayne"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal WayneCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = WayneCLI.__new__(WayneCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["wayne-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Wayne' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Wayne' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Wayne' minimum context length."""

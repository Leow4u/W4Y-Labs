...
minimal ``WayneCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def wayne_home(tmp_path, monkeypatch):
    """Isolated WAYNE_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".wayne"
    home.mkdir()
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, wayne_home):
    ...
    def test_interrupted_turn_is_resumable(self, wayne_home):
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, wayne_home):
    ...
    def test_no_assistant_message_skipped(self, wayne_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, wayne_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, wayne_home):
        ...
        src = inspect.getsource(WayneCLI.chat)

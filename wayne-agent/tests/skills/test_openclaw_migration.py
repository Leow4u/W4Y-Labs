...
"""
    ...
    assert "Tyler Williams > Active Projects: Wayne Agent" in entries
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # wayne_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.wayne/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "wayne_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Wayne``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Wayne prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Wayne to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Wayne config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Wayne uses tools well"
    # All-lowercase matches → lowercase ``wayne``; this preserves the
    # real filesystem path ``~/.wayne`` (Wayne home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "wayne should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Wayne remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "wayne prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Wayne was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "wayne uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Wayne said to ask Wayne about Wayne settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.wayne`` (the real Wayne home), not the broken
    ``.Wayne``.
    ...
    rewritten to ``~/.Wayne/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.wayne/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .wayne directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.wayne'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "wayne config path: ~/.wayne/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Wayne" in result

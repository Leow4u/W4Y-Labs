...
The ``wayne update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m wayne_cli.main gateway run`` — image name
    ``pythonw``, not ``wayne.exe`` — so the ``taskkill /IM wayne.exe`` guard
    ...
    # The wayne.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM wayne.exe' in text

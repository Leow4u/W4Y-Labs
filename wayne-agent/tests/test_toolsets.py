...
class TestToolsetConsistency:
    ...
    def test_wayne_platforms_share_core_tools(self):
        """All wayne-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on wayne-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["wayne-cli", "wayne-telegram", "wayne-discord", "wayne-whatsapp", "wayne-slack", "wayne-signal", "wayne-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_wayne_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("wayne-whatsapp")

    def test_wayne_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("wayne-api-server")

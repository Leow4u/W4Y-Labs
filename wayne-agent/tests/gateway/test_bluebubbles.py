...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_wayne_variants(self, monkeypatch):
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Wayne what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Wayne, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Wayne agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Wayne about this") == "please ask Wayne about this"

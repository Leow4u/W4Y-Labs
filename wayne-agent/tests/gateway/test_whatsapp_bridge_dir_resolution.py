...
(/opt/wayne/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable WAYNE_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    # Nothing mirrored into WAYNE_HOME.
    assert not (wayne_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_wayne_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable WAYNE_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = wayne_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the WAYNE_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    wayne_home = tmp_path / "wayne_home"
    mirror = wayne_home / "scripts" / "whatsapp-bridge"

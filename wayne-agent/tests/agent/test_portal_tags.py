...
def test_wayne_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import wayne_client_tag

    tag = wayne_client_tag()
    assert tag.startswith("client=wayne-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import wayne_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=wayne-agent" in tags
    assert wayne_client_tag() in tags
    assert len(tags) == 2

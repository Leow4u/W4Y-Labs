...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_wayne_env(self):
        """Wayne' own .env inside WAYNE_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: wayne internal .env is in ~/.wayne/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.wayne/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.wayne/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        wayne_home = tmp_path / ".wayne"
        hub = wayne_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_wayne_home(self, tmp_path):
        """The env basename guard does not depend on WAYNE_HOME resolution."""
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        wayne_home = tmp_path / ".wayne"
        cache = wayne_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")

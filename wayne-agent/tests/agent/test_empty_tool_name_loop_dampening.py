...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated WAYNE_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="wayne_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".wayne"))
    prev_home = os.environ.get("WAYNE_HOME")
    os.environ["WAYNE_HOME"] = os.path.join(test_home, ".wayne")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("wayne_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["WAYNE_HOME"] = prev_home

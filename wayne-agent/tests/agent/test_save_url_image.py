...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate WAYNE_HOME under tmp_path."""
    monkeypatch.setenv("WAYNE_HOME", str(tmp_path / ".wayne"))
    (tmp_path / ".wayne").mkdir()

    # Force the constants/image cache helpers to re-read WAYNE_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("wayne_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_wayne_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under WAYNE_HOME — gateway cleanup

...
path via ``wayne chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp WAYNE_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point WAYNE_HOME at a temp dir and turn moa.save_traces on."""
    wayne_home = tmp_path / ".wayne"
    wayne_home.mkdir()
    monkeypatch.setenv("WAYNE_HOME", str(wayne_home))

    # save_moa_turn reads config via wayne_cli.config.load_config; stub it to

...
def test_metadata_as_dict_with_wayne():
    """Normal case: metadata is a dict containing wayne keys."""
    frontmatter = {
        "metadata": {
            "wayne": {
...
def test_metadata_hermes_namespace_fallback():
    """agentskills.io ecosystem skills publish metadata.hermes; it must be
    honoured whenever metadata.wayne is absent."""
    frontmatter = {
        "metadata": {
            "hermes": {
                "fallback_for_toolsets": ["toolset_a"],
...
def test_metadata_wayne_wins_over_hermes():
    """When both namespaces are present, metadata.wayne takes precedence."""
    frontmatter = {
        "metadata": {
            "wayne": {"requires_tools": ["wayne_tool"]},
            "hermes": {"requires_tools": ["hermes_tool"]},
        }
    }
    result = extract_skill_conditions(frontmatter)
    assert result["requires_tools"] == ["wayne_tool"]


def test_extract_skill_config_vars_hermes_fallback():
    """Config var declarations under metadata.hermes are read too."""
    frontmatter = {
        "metadata": {
            "hermes": {
                "config": [

...
def isolated_home(monkeypatch):
    """Temp WAYNE_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="wayne_test_31179_")
    wayne_home = os.path.join(test_home, ".wayne")
    os.makedirs(wayne_home)
...
def _fresh_modules():
    """Drop cached wayne modules so each test reloads against current env."""

...
def curator_env(tmp_path, monkeypatch):
    """Isolated WAYNE_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".wayne"
    ...
    # save_state() resolves the state path from WAYNE_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.wayne once monkeypatch
    ...
    # thread here, while WAYNE_HOME is still pinned to this test's tmp home
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `wayne curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_run_review_consolidate_override_runs_llm(curator_env, monkeypatch):
    ...
    the LLM consolidation pass — mirrors `wayne curator run --consolidate`."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
# `wayne model` → auxiliary picker, the dashboard Models tab, and the full

...
def _make_enabled_plugin(wayne_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <wayne_home>/plugins/<name> and opt it in."""
    plugin_dir = wayne_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WAYNE_HOME", str(tmp_path / "wayne_empty"))

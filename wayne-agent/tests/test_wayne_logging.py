...
def wayne_home(tmp_path, monkeypatch):
    """Provide an isolated WAYNE_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_wayne_home from conftest,
...
class TestSetupLogging:
    ...
    def test_writes_to_agent_log(self, wayne_home):
        ...
        agent_log = wayne_home / "logs" / "agent.log"
    ...
    def test_warnings_appear_in_both_logs(self, wayne_home):
        ...
        agent_log = wayne_home / "logs" / "agent.log"
        errors_log = wayne_home / "logs" / "errors.log"
    ...
    def test_reads_config_yaml(self, wayne_home):
    ...
    def test_explicit_params_override_config(self, wayne_home):
...
class TestGatewayMode:
    ...
    def test_gateway_log_created_after_cli_init(self, wayne_home):
        ...
        gw_log = wayne_home / "logs" / "gateway.log"
...
class TestGuiMode:
    ...
    def test_gui_log_receives_only_gui_components(self, wayne_home):
        ...
        gui_log = wayne_home / "logs" / "gui.log"
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, wayne_home):
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_wayne_verbose", False)
        ]
...
class TestReadLoggingConfig:
    ...
    def test_reads_logging_section(self, wayne_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, wayne_home):
        import yaml
        config = {"model": "test"}
        (wayne_home / "config.yaml").write_text(yaml.dump(config))

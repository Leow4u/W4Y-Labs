"""Regression tests for WAYNE_HOME override propagation onto the MCP loop.
...
get_wayne_home() inside the coroutine, most visibly OAuth token-store
...
def test_override_propagates_to_mcp_loop(tmp_path, monkeypatch, mcp_loop):
    ...
    process_home.mkdir()
    profile_home.mkdir()
    monkeypatch.setenv("WAYNE_HOME", str(process_home))
    ...
    token = set_wayne_home_override(str(profile_home))
    try:
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: WayneTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(WayneTokenStorage("probe-srv")._tokens_path())

...
def test_wayne_provider_subclass_exists():
    """WayneMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _WAYNE_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _WAYNE_PROVIDER_CLS is not None
    assert issubclass(_WAYNE_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_wayne_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns WayneMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _WAYNE_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("WAYNE_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _WAYNE_PROVIDER_CLS is not None
    assert isinstance(provider, _WAYNE_PROVIDER_CLS)
    assert provider._wayne_server_name == "srv"

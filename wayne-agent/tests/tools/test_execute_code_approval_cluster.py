...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad WAYNE_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: WAYNE_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("WAYNE_GATEWAY_SESSION", "1")
...
def test_env_scrub_wayne_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "WAYNE_HOME": "/h", "WAYNE_PROFILE": "p",
        "WAYNE_CONFIG": "/c.yaml", "WAYNE_ENV": "/e",
        # other WAYNE_* → dropped (broad prefix removed)
        "WAYNE_BASE_URL": "https://x", "WAYNE_INTERACTIVE": "1",
        "WAYNE_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("WAYNE_HOME", "WAYNE_PROFILE", "WAYNE_CONFIG", "WAYNE_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "WAYNE_BASE_URL", "WAYNE_INTERACTIVE", "WAYNE_KANBAN_DB",
...
def test_env_scrub_logs_dropped_wayne_vars(caplog):
    """Dropping a non-allowlisted, non-secret WAYNE_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset WAYNE_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "WAYNE_HOME": "/h",          # allowlisted → kept, not logged
        "WAYNE_BASE_URL": "https://x",   # dropped → logged
        "WAYNE_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "WAYNE_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "WAYNE_HOME" in out and "PATH" in out
    assert "WAYNE_BASE_URL" not in out and "WAYNE_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "WAYNE_BASE_URL" in msgs and "WAYNE_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "WAYNE_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped WAYNE_* vars."""
    import logging

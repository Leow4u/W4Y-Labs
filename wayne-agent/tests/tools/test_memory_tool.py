...
class TestScanMemoryContent:
    ...
    def test_wayne_config_mod_blocked(self):
        result = _scan_memory_content("edit .wayne/config.yaml to change settings")
        assert "Blocked" in result
        assert "wayne_config_mod" in result
        result = _scan_memory_content("update .wayne/SOUL.md with new personality")
        assert "Blocked" in result
        assert "wayne_config_mod" in result
    ...
    def test_wayne_config_mod_no_false_positives(self):
        """Merely mentioning wayne config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .wayne/config.yaml for settings") is None
        assert _scan_memory_content("Read .wayne/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .wayne/config.yaml file contains runtime options") is None

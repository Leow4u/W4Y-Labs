...
def _restore_tool_modules():
    original_wayne_home = os.environ.get("WAYNE_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_wayne_home is None:
...
def _install_modal_test_modules(
    ...
    *,
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="wayne-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": wayne_home / "modal_snapshots.json",

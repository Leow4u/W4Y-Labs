...
Older images made /opt/wayne/ui-tui and /opt/wayne/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV WAYNE_TUI_DIR=/opt/wayne/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R wayne:wayne /opt/wayne/ui-tui" not in text
    assert "chown -R wayne:wayne /opt/wayne/node_modules" not in text

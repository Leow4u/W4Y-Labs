...
class TestManagedGatewayErrorTranslation:
    ...
    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError with Work4You settings remediation."""
        ...
        assert "403" in msg
        assert "https://fal.ai" not in msg.lower()
        assert "FAL_KEY" not in msg
        assert "Settings" in msg or "Work4You" in msg or "plan" in msg.lower()
...
class TestManagedKreaRouting:
    ...
    def test_routes_native_krea_model_to_krea_plugin_in_managed_mode(
        ...
        monkeypatch.setattr(
            "wayne_cli.plugins._ensure_plugins_discovered", lambda *a, **k: None
        )

...
A blueprint is a skill with a metadata.wayne.blueprint block. These verify parsing,
...
class TestParseBlueprint:
    ...
    def test_parses_hermes_namespace_blueprint(self):
        # Ecosystem skills authored for upstream hermes-agent declare the
        # automation under metadata.hermes.blueprint; that spelling must
        # parse identically (read-side fallback only — export still writes
        # metadata.wayne).
        skill = (
            "---\nname: eco-brief\ndescription: d\nmetadata:\n  hermes:\n"
            '    blueprint:\n      schedule: "0 8 * * *"\n      deliver: telegram\n---\n\nbody'
        ...
        assert spec is not None
        assert spec.skill_name == "eco-brief"
        assert spec.schedule == "0 8 * * *"
        assert spec.deliver == "telegram"

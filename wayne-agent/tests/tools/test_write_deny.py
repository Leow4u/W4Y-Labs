...
class TestWriteDenyExactPaths:
    ...
    def test_wayne_env(self):
        # ``.env`` under the active WAYNE_HOME (profile-aware, not just
    ...
    def test_wayne_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.wayne/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "wayne_root"

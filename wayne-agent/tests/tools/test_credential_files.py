...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        wayne_home.mkdir()
        (wayne_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"WAYNE_HOME": str(wayne_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(wayne_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.wayne/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        (wayne_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        (wayne_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        (wayne_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.wayne/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"WAYNE_HOME": str(wayne_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.wayne")

        assert mounts[0]["container_path"] == "/home/user/.wayne/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.wayne/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.wayne/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape WAYNE_HOME."""
        monkeypatch.setenv("WAYNE_HOME", str(tmp_path / ".wayne"))
        (tmp_path / ".wayne").mkdir()

        # Create a sensitive file one level above wayne_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside wayne_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside WAYNE_HOME must still be registered."""
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        monkeypatch.setenv("WAYNE_HOME", str(wayne_home))
        (wayne_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_wayne_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of WAYNE_HOME must be allowed."""
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        subdir = wayne_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside WAYNE_HOME pointing outside must be rejected."""
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        monkeypatch.setenv("WAYNE_HOME", str(wayne_home))

        # Create a sensitive file outside wayne_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside wayne_home pointing outside
        symlink = wayne_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes WAYNE_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, wayne_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape WAYNE_HOME."""
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        monkeypatch.setenv("WAYNE_HOME", str(wayne_home))
        ...
        sensitive.write_text("{}")
        self._write_config(wayne_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(wayne_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside WAYNE_HOME via config must still mount."""
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        monkeypatch.setenv("WAYNE_HOME", str(wayne_home))

        (wayne_home / "oauth.json").write_text("{}")
        self._write_config(wayne_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        (wayne_home / "cache" / "documents").mkdir(parents=True)
        (wayne_home / "cache" / "audio").mkdir(parents=True)
        (wayne_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("WAYNE_HOME", str(wayne_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.wayne/cache/documents" in paths
        assert "/root/.wayne/cache/audio" in paths
        assert "/root/.wayne/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.wayne/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_wayne_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir()
        # Use legacy dir name with content — get_wayne_dir prefers
        # populated old over new.
        legacy_doc = wayne_home / "document_cache"
        legacy_img = wayne_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(wayne_home / "document_cache") in host_paths
        assert str(wayne_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.wayne/cache/documents" in container_paths
        assert "/root/.wayne/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        wayne_home = tmp_path / ".wayne"
        img_dir = wayne_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.wayne/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.wayne")
            == "/home/agent/.wayne/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        wayne_home = tmp_path / ".wayne"
        (wayne_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("WAYNE_HOME", str(wayne_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(wayne_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        wayne_home = tmp_path / ".wayne"
        doc_dir = wayne_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        wayne_home = tmp_path / ".wayne"
        ss_dir = wayne_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.wayne/cache/screenshots/session_abc/screen1.png"

...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._wayne_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(wayne_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(wayne_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_wayne_bin(self):
        """After _INSTALL_FAILED, manual install in WAYNE_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        wayne_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(wayne_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(wayne_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._wayne_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == wayne_bin
            assert _tirith_mod._resolved_path == wayne_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# WAYNE_HOME isolation
# ---------------------------------------------------------------------------

class TestWayneHomeIsolation:
    def test_wayne_bin_dir_respects_wayne_home(self):
        """_wayne_bin_dir must use WAYNE_HOME, not hardcoded ~/.wayne."""
        from tools.tirith_security import _wayne_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"WAYNE_HOME": tmpdir}):
            result = _wayne_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_wayne_home(self):
        """_failure_marker_path must use WAYNE_HOME, not hardcoded ~/.wayne."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"WAYNE_HOME": "/custom/wayne"}):
            result = _failure_marker_path()
        assert result == "/custom/wayne/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets WAYNE_HOME; verify it's active."""
        wayne_home = os.getenv("WAYNE_HOME")
        assert wayne_home is not None, "WAYNE_HOME should be set by conftest"
        assert "wayne_test" in wayne_home, "Should point to test temp dir"

    def test_get_wayne_home_fallback(self):
        """Without WAYNE_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_wayne_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove WAYNE_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".wayne")
            result = _get_wayne_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._wayne_bin_dir",

...
@pytest.fixture
def wayne_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="wayne_wa_test_")
    home = os.path.join(d, ".wayne")
    os.makedirs(home)
    monkeypatch.setenv("WAYNE_HOME", home)
    yield home
...
def test_default_gate_is_off(wayne_home):
...
def test_invalid_subsystem_is_off(wayne_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(wayne_home):
...
def test_memory_gate_on_no_interactive_stages(wayne_home):
...
def test_memory_gate_on_then_apply(wayne_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(wayne_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(wayne_home, monkeypatch):
...
def test_skill_gate_off_allows_create(wayne_home):
...
def test_skill_gate_on_always_stages(wayne_home):
...
def test_skill_gate_on_then_apply_writes_file(wayne_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's WAYNE_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(wayne_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(wayne_home):
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(wayne_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(wayne_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(wayne_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(wayne_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(wayne_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(wayne_home):

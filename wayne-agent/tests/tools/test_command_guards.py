...
def _clean_state():
    ...
    saved = {}
    for k in ("WAYNE_INTERACTIVE", "WAYNE_GATEWAY_SESSION", "WAYNE_EXEC_ASK", "WAYNE_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["WAYNE_INTERACTIVE"] = "1"
        session_key = os.getenv("WAYNE_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No WAYNE_INTERACTIVE or WAYNE_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

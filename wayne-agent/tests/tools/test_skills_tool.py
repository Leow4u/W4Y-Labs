...
class TestParseFrontmatter:
    ...
    def test_nested_yaml(self):
        ...
        fm, body = _parse_frontmatter(content)
        assert fm["metadata"]["wayne"]["tags"] == ["a", "b"]
    ...
    def test_skill_view_applies_template_vars(self, tmp_path):
        with (
            ...
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run ${WAYNE_SKILL_DIR}/scripts/do.sh in ${WAYNE_SESSION_ID}",
            )
        ...
        assert "${WAYNE_SKILL_DIR}" not in result["content"]

    def test_skill_view_applies_hermes_template_vars(self, tmp_path):
        # Skills published for the upstream hermes-agent / agentskills.io
        # ecosystem use ${HERMES_SKILL_DIR}/${HERMES_SESSION_ID}; both
        # spellings must resolve to the same values.
        with (
            patch("tools.skills_tool.SKILLS_DIR", tmp_path),
            patch(
                "agent.skill_preprocessing.load_skills_config",
            ...
            raw = skill_view("eco-templated", task_id="session-123")
...
""",
    ...
    def test_metadata_hermes_namespace_tags_fallback(self, tmp_path):
        # Ecosystem skills (agentskills.io hubs) publish metadata.hermes
        # instead of metadata.wayne; their tags must still be surfaced.
        skill_dir = tmp_path / "eco-skill"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            "---\n"
            "name: eco-skill\n"
            "description: Ecosystem skill using the upstream metadata namespace.\n"
            "metadata:\n"
            "  hermes:\n"
            "    tags: [eco, hub]\n"
            "---\n\n"
        ...
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            raw = skill_view("eco-skill")

        result = json.loads(raw)
        assert result["success"] is True
        assert result["tags"] == ["eco", "hub"]

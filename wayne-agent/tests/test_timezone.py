...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When WAYNE_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) WAYNE_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("WAYNE_TIMEZONE=" + os.environ.get("WAYNE_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "WAYNE_TIMEZONE=NOT_SET" in result["output"], (
            "WAYNE_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When WAYNE_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("WAYNE_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=wayne_tz) which shifted
        absolute time when system-local tz != Wayne tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_wayne_tz(self):
        """Already-aware datetimes should be normalized to Wayne tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Wayne tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Wayne tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Wayne is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Wayne tz
        ...
        # Use a Wayne timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _wayne_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["WAYNE_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_wayne_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Wayne timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["WAYNE_TIMEZONE"] = "US/Eastern"
        _reset_wayne_time_cache()

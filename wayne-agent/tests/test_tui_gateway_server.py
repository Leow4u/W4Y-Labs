...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["WAYNE_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("WAYNE_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("WAYNE_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("WAYNE_VOICE", "1")
    monkeypatch.delenv("WAYNE_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("WAYNE_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("WAYNE_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("WAYNE_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # _get_platform_tools (a non-configurable platform toolset in wayne-cli's
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("WAYNE_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("WAYNE_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown WAYNE_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("WAYNE_MODEL", raising=False)
    monkeypatch.setenv("WAYNE_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("WAYNE_MODEL", raising=False)
    monkeypatch.delenv("WAYNE_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/wayne-test "}}
    )

    assert server._resolve_model() == "nous/wayne-test"
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set WAYNE_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("WAYNE_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("WAYNE_MODEL", raising=False)
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("WAYNE_MODEL", "nous/wayne-test")
    monkeypatch.setenv("WAYNE_TUI_PROVIDER", "nous")
    monkeypatch.delenv("WAYNE_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/wayne-test", "nous")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    assert server._resolve_startup_runtime() == ("nous/wayne-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("WAYNE_MODEL", "sonnet")
    monkeypatch.delenv("WAYNE_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("WAYNE_INFERENCE_PROVIDER", raising=False)
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with WayneCLI / gateway: fallback_providers stays first and keeps
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_wayne_home", tmp_path)
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("WAYNE_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("WAYNE_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("WAYNE_EXEC_ASK", raising=False)
    monkeypatch.delenv("WAYNE_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("WAYNE_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["WAYNE_GATEWAY_SESSION"] == "1"
    assert server.os.environ["WAYNE_EXEC_ASK"] == "1"
    assert server.os.environ["WAYNE_INTERACTIVE"] == "1"
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing WAYNE_INFERENCE_PROVIDER on a switch leaked the new
    ...
    try:
        ...
        assert os.environ["WAYNE_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "WAYNE_TUI_PROVIDER" not in os.environ
        assert "WAYNE_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write WAYNE_MODEL / WAYNE_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("WAYNE_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("WAYNE_MODEL", raising=False)
    monkeypatch.delenv("WAYNE_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["WAYNE_TUI_PROVIDER"] == "openai-codex"
        assert "WAYNE_MODEL" not in os.environ
        assert "WAYNE_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors WayneCLI._manual_compress's
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".wayne" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.wayne/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".wayne" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.wayne/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".wayne" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.wayne/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_create_stores_desktop_cwd_without_host_isdir(monkeypatch):
    """F3: a Windows folder is not a directory on Fly — still bind it."""
    ...
    resp = server.handle_request(
        {
            ...
            "params": {
                "cols": 80,
                "source": "desktop",
                "desktop_cwd": r"C:\Users\demo\repo",
            },
        }
    )
    ...
    sid = resp["result"]["session_id"]
    session = server._sessions[sid]
    assert session["desktop_cwd"] == r"C:\Users\demo\repo"
    assert resp["result"]["info"]["desktop_cwd"] == r"C:\Users\demo\repo"
    # Fly cwd stays on the volume — the PC path is desktop_cwd only.
    assert session["cwd"] == "/opt/data"

    from tools.desktop_body import clear_desktop_cwd, get_desktop_cwd

    assert get_desktop_cwd(session["session_key"]) == r"C:\Users\demo\repo"
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_wayne_home
    # fixture pins WAYNE_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `wayne model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `wayne model` and the classic CLI /model picker.
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``wayne --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_reload_env_rpc_calls_wayne_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.wayne/.env
...
def test_session_save_writes_under_wayne_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Wayne profile
    ...
    Regression: the gateway handler wrote ``wayne_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Wayne.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("wayne_conversation_*.json"))
    ...
    assert payload["model"] == "wayne-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Wayne."
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._wayne_home, which
    # is captured at import time — patch it directly (set_wayne_home_override
    # does NOT affect this snapshot).
    monkeypatch.setattr(cli, "_wayne_home", tmp_path)

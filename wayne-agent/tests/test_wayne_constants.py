...
from wayne_constants import (
    ...
    get_default_wayne_root,
    get_wayne_dir,
    get_wayne_home,
    heal_wayne_managed_node,
    wayne_managed_node_tree_present,
    iter_wayne_node_dirs,
    is_container,
    node_tool_runnable,
    parse_reasoning_effort,
    secure_parent_dir,
    with_wayne_node_path,
)


class TestGetDefaultWayneRoot:
    ...
    def test_wayne_home_is_native(self, tmp_path, monkeypatch):
        """When WAYNE_HOME = ~/.wayne, returns ~/.wayne."""
        native = tmp_path / ".wayne"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WAYNE_HOME", str(native))
        assert get_default_wayne_root() == native

    def test_wayne_home_is_profile(self, tmp_path, monkeypatch):
        """When WAYNE_HOME is a profile under ~/.wayne, returns ~/.wayne."""
        native = tmp_path / ".wayne"
    ...
    def test_wayne_home_is_docker(self, tmp_path, monkeypatch):
        """When WAYNE_HOME points outside ~/.wayne (Docker), returns WAYNE_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WAYNE_HOME", str(docker_home))
        assert get_default_wayne_root() == docker_home

    def test_wayne_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any WAYNE_HOME outside ~/.wayne is treated as the root."""
        custom = tmp_path / "my-wayne-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WAYNE_HOME", str(custom))
        assert get_default_wayne_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (WAYNE_HOME=<root>/profiles/<name>),
...
class TestWayneManagedNode:
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert wayne_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_wayne_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "wayne"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Wayne-managed npm/node wrappers."""
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".wayne"
...
class TestGetWayneDir:
    """Tests for ``get_wayne_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_wayne_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_wayne_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_wayne_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy

...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "wayne"
        assert config.workspace_id == "wayne"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.wayne/honcho.json
        monkeypatch.setenv("WAYNE_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"wayne": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"wayne": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"wayne": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="wayne-agent"
        ):
            result = config.resolve_session_name("/home/user/wayne-agent/subdir")
        assert result == "wayne-agent"
...
class TestResolveConfigPath:
    def test_prefers_wayne_home_when_exists(self, tmp_path):
        wayne_home = tmp_path / "wayne"
        wayne_home.mkdir()
        local_cfg = wayne_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: WAYNE_HOME points at ~/.wayne/profiles/<name>, so
        # _get_default_wayne_home() must resolve back to ~/.wayne — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".wayne"
        ...
        assert _get_default_wayne_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_wayne_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".wayne".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".wayne"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "wayne_coder"
        assert profile_host_key("default") == "wayne"

    def test_default_returns_wayne(self):
        with patch.dict(os.environ, {}, clear=True):
    ...
    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"WAYNE_HONCHO_HOST": "wayne.coder"}):
            assert resolve_active_host() == "wayne.coder"
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="wayne_coder")
        assert config.host == "wayne_coder"
        assert config.workspace_id == "wayne"  # shared workspace
        assert config.ai_peer == "wayne_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="wayne")
        assert config.host == "wayne"
        assert config.workspace_id == "wayne"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "wayne": {"aiPeer": "wayne", "peerName": "alice"},
                "wayne_coder": {
                    "aiPeer": "wayne_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="wayne_coder", config_path=config_file,
        )
        assert config.host == "wayne_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "wayne_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="wayne_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "wayne_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "wayne.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="wayne_dreamer",
        ...
        assert config.host == "wayne_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "wayne_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"wayne": {"enabled": True, "aiPeer": "wayne"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"wayne": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_wayne_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="wayne",
    ...
    def test_wayne_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="wayne",

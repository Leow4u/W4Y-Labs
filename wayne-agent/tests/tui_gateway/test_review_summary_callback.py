    ...
    with patch.dict(
        "sys.modules",
        {
            "work4you_constants": MagicMock(
                get_wayne_home=MagicMock(return_value="/tmp/wayne_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'wayne-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'wayne-release' patched"
    }

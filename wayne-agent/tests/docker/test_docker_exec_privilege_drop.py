...
def test_shim_drops_root_to_work4you_uid(sleep_container: str) -> None:
    ...
    Use `work4you config set` which writes config.yaml under WAYNE_HOME.
...
def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user work4you already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "work4you", sleep_container,
         "work4you", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user work4you failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up work4you:work4you — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "work4you", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_opt_out_strict_truthiness(
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "work4you", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "work4you:work4you", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected work4you:work4you"
    )

...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    false-positive `work4you doctor` and any other health check).
...
def test_dashboard_opt_in_starts(
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'work4you dashboard'", deadline_s=30.0,
    )
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "work4you dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as work4you, so the
    # work4you user can kill it (same UID).

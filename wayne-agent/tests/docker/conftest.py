...
Override the image with ``WAYNE_TEST_IMAGE`` env var to point at a pre-built
...
def built_image() -> str:
    ...
    Override with ``WAYNE_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("WAYNE_TEST_IMAGE"):
        return IMAGE_TAG

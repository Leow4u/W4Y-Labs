...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Wayne CLI
...
class TestEntryPointsImportBootstrap:
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        """
        # Resolve relative to the wayne-agent repo root.  Tests live
        # at tests/test_wayne_bootstrap.py, so go up one dir.
        import pathlib
...
class TestHardenImportPath:
    ...
    directory from shadowing Wayne's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("WAYNE_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["WAYNE_PYTHON_SRC_ROOT"] = env
            elif "WAYNE_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["WAYNE_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/wayne")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("WAYNE_PYTHON_SRC_ROOT", None)
            else:
                os.environ["WAYNE_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/wayne", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Wayne root.  The guard must relocate Wayne to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/wayne"])
        assert result[0] == "/opt/wayne"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Wayne root.
        assert result.index("/opt/wayne") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/wayne", "/opt/wayne", ""])
        assert result.count("/opt/wayne") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["WAYNE_PYTHON_SRC_ROOT"] = "/env/wayne"
            hb.harden_import_path()
            assert sys.path[0] == "/env/wayne"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("WAYNE_PYTHON_SRC_ROOT", None)

...
def test_root_session_no_compression(db):
    ...
    assert prov["currentWayneSessionId"] == "root1"
    assert prov["rootWayneSessionId"] == "root1"
    assert prov["parentWayneSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_wayne_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentWayneSessionId"] == "old"
    assert prov["rootWayneSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousWayneSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootWayneSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootWayneSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_wayne_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousWayneSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"wayne"}
    assert "sessionProvenance" in meta["wayne"]
    assert meta["wayne"]["sessionProvenance"]["currentWayneSessionId"] == "root1"

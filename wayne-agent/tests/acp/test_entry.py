...
def test_main_version_prints_without_starting_server(monkeypatch, capsys):
    ...
    assert output
    assert "Starting wayne-agent ACP adapter" not in output
...
def test_main_check_prints_ok_without_starting_server(monkeypatch, capsys):
    ...
    assert capsys.readouterr().out.strip() == "Wayne ACP check OK"
...
def test_main_setup_runs_model_configuration(monkeypatch):
    ...
    monkeypatch.setattr("wayne_cli.main.main", fake_wayne_main)
...
def test_main_setup_offers_browser_install_when_tty(monkeypatch):
    ...
    monkeypatch.setattr("wayne_cli.main.main", lambda: None)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    """`wayne-acp --setup-browser` routes through dep_ensure.ensure_dependency."""
    calls = []
    ...
    monkeypatch.setattr("wayne_cli.dep_ensure.ensure_dependency", fake_ensure)

...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    wayne_state = ModuleType("wayne_state")
    wayne_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "wayne_state", wayne_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Wayne"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Wayne"

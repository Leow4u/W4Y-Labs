"""Tests for the per-profile Projects store (wayne_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Wayne Agent", folders=["/tmp/wayne"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "wayne-agent"
    assert proj.name == "Wayne Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/wayne"
    assert [f.path for f in proj.folders] == ["/tmp/wayne"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "wayne-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Wayne Agent")
    pdb.create_project(conn, name="Wayne Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["wayne-agent", "wayne-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' WAYNE_HOME.
...
def test_db_path_under_wayne_home():
    # Resolves under WAYNE_HOME (set by the autouse isolation fixture).

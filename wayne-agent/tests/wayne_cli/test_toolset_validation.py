"""Unit tests for wayne_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Wayne.
"""
...
# A representative set of real toolset names. `wayne` is deliberately absent —
# that is the corruption #38798 reported (`wayne-cli` rewritten to `wayne`).
_KNOWN = {
    "wayne-cli",
    "wayne-telegram",
    "wayne-discord",
...
def test_valid_config_produces_no_warnings():
    cfg = {"cli": ["wayne-cli"], "telegram": ["wayne-telegram"]}
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'wayne' instead of 'wayne-cli'.
    warnings = validate_platform_toolsets({"cli": ["wayne"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'wayne'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'wayne-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["wayne-cli"], "discord": ["bogus"]}
...
def test_unknown_without_valid_platform_default_omits_suggestion():
    # wayne-mystery is not a known toolset, so no "did you mean" hint.
...
@pytest.mark.parametrize("value", [None, {}, [], "wayne-cli", 42])
...
def test_scalar_toolset_value_is_accepted():
    ...
    assert validate_platform_toolsets({"cli": "wayne-cli"}, _is_valid) == []


def test_non_string_entries_are_skipped_not_counted_invalid():
    cfg = {"cli": [None, 123, "wayne-cli"]}
    # The junk entries are ignored; the valid one keeps it from being "zero".
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["wayne"], "discord": ["wayne"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_wayne_cli_valid_and_wayne_invalid():
    # Ties the helper to reality: the canonical registry check agrees that
    # `wayne-cli` is the real toolset and `wayne` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("wayne-cli") is True
    assert validate_toolset("wayne") is False
    warnings = validate_platform_toolsets({"cli": ["wayne"]}, validate_toolset)
    assert any("did you mean 'wayne-cli'?" in w for w in warnings)

...
Before the fix, ``wayne whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``wayne gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    wayne = home / ".wayne"
    wayne.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("WAYNE_HOME", str(wayne))
...
def _env_value(wayne_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from wayne_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("wayne_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``wayne whatsapp`` with an existing paired session and

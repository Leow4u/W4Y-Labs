...
def _patch_managed_uv(request):
    ...
    with patch("wayne_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("wayne_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("wayne_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M wayne_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="wayne_cli/main.py\n", stderr="", returncode=0)
    ...
    assert "wayne_cli/main.py" in out
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    # Exact flags used by wayne update (wayne_cli/main.py).

"""Tests for `wayne chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("WAYNE_SAFE_MODE", "WAYNE_IGNORE_USER_CONFIG", "WAYNE_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in wayne_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("WAYNE_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under WAYNE_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("WAYNE_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("WAYNE_SAFE_MODE", raising=False)
        from wayne_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under WAYNE_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `wayne chat`."""

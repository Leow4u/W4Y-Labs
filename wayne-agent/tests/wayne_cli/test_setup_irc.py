"""Tests for IRC gateway configuration via `wayne setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Wayne install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Wayne install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#wayne")
            monkeypatch.setenv("IRC_NICKNAME", "wayne-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `wayne setup gateway` experience with IRC present."""

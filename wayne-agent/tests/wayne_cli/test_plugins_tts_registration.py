...
``$WAYNE_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/wayne_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        wayne_home = Path(os.environ["WAYNE_HOME"])
        _write_plugin(
            wayne_home / "plugins",
            "my-tts-plugin",
            register_body=(
        ...
        _enable(wayne_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(wayne_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(wayne_home, "shadow-tts-plugin")

"""Tests for wayne_cli.security_advisories.
...
import wayne_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect WAYNE_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".wayne"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full wayne_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "wayne_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "wayne_cli.config.save_config",
...
class TestRendering:
    def test_short_banner_lines_includes_id_and_version(self, fake_advisory):
        ...
        assert "work4you doctor" in joined

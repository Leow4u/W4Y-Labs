"""Guard: every `wayne update` path that reports user-modified skills must
also tell the user how to find them.

`wayne update` keeps (does not overwrite) bundled skills the user edited and
prints a ``~ N user-modified (kept)`` count. There are two independent update
code paths in ``wayne_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``wayne skills list-modified`` so the count is actionable — otherwise,

"""Regression tests for #34107 — Docker UID/GID handling in ensure_wayne_home.

When Wayne runs in Docker with ``WAYNE_UID=1000`` / ``WAYNE_GID=911``,
the entrypoint chowns the top-level ``WAYNE_HOME`` once at startup. But
...
The fix is a ``_chown_to_wayne_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_wayne_uid_gid
# ---------------------------------------------------------------------------


class TestResolveWayneUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("WAYNE_UID", "1000")
        monkeypatch.setenv("WAYNE_GID", "911")
        from wayne_cli.config import _resolve_wayne_uid_gid
        uid, gid = _resolve_wayne_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("WAYNE_UID", raising=False)
        monkeypatch.delenv("WAYNE_GID", raising=False)
        from wayne_cli.config import _resolve_wayne_uid_gid
        uid, gid = _resolve_wayne_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("WAYNE_UID", "not-a-number")
        monkeypatch.setenv("WAYNE_GID", "911")
        from wayne_cli.config import _resolve_wayne_uid_gid
        uid, gid = _resolve_wayne_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("WAYNE_UID", "")
        monkeypatch.setenv("WAYNE_GID", "")
        from wayne_cli.config import _resolve_wayne_uid_gid
        uid, gid = _resolve_wayne_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToWayneUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_wayne_uid(d)  # must not raise

...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        ...
        with patch("wayne_cli.main._is_windows", return_value=True), \
             patch("wayne_cli.main._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("wayne_cli.main._run_quarantined_install") as mock_install:
            from wayne_cli.main import _verify_console_scripts_installed

            _verify_console_scripts_installed(["uv", "pip"], env={})

        mock_install.assert_not_called()

    def test_triggers_reinstall_when_wayne_exe_missing(
        self, temp_pyproject, fake_scripts_dir
    ):
        (fake_scripts_dir / "wayne-agent.exe").write_bytes(b"fake")
        (fake_scripts_dir / "wayne-acp.exe").write_bytes(b"fake")
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("wayne_cli.main._is_windows", return_value=False), \
             patch("wayne_cli.main._run_quarantined_install") as mock_install:
            from wayne_cli.main import _verify_console_scripts_installed
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("wayne_cli.main._is_windows", return_value=False), \
             patch("wayne_cli.main._run_quarantined_install") as mock_install, \
             patch("wayne_cli.main._verify_console_scripts_installed") as mock_verify:

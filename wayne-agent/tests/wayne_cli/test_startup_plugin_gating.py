...
``wayne_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``wayne --help``, ``wayne version``,
``wayne logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``wayne --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from wayne_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["wayne", "-w", "--tui"], None),
        (["wayne", "version"], "version"),
        (["wayne", "--tui", "chat"], "chat"),
        (["wayne", "-w", "logs"], "logs"),
        (["wayne", "chat", "hello world"], "chat"),
        (["wayne", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["wayne", "-m", "gpt5", "chat"], "chat"),
        (["wayne", "--model", "gpt5", "chat", "hi"], "chat"),
        (["wayne", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["wayne", "-z", "hello world"], None),
        (["wayne", "-z", "hello", "chat"], "chat"),
        (["wayne", "--model=gpt5", "chat"], "chat"),     # inline form
        (["wayne", "--", "chat"], "chat"),               # -- terminator
        (["wayne", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["wayne", "some-plugin-cmd"], "some-plugin-cmd"),
        (["wayne", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["wayne"],                          # bare → chat
        ["wayne", "--help"],                # top-level help
        ["wayne", "-h"],
        ["wayne", "version"],               # known built-in
        ["wayne", "logs"],
        ["wayne", "gateway", "run"],
        ["wayne", "--tui"],
        ["wayne", "-w", "--tui"],
        ["wayne", "chat", "hi"],
        ["wayne", "help"],                  # accepted built-in-ish
        ["wayne", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["wayne", "meet", "join"],          # potential google_meet plugin
        ["wayne", "honcho", "status"],      # potential memory plugin
        ["wayne", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``wayne help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"wayne_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "

...
hand-rolled parsers — ``wayne_cli.config.load_env``,
``wayne_cli.main._has_any_provider_configured``, and
...
def test_config_load_env_strips_export_prefix(tmp_path):
    from wayne_cli.config import invalidate_env_cache, load_env
...
def test_skills_tool_load_env_strips_export_prefix(tmp_path, monkeypatch):
    ...
    # skills_tool.load_env reads get_wayne_home()/.env directly.
    import importlib

    import tools.skills_tool as skills_tool

    importlib.reload(skills_tool)
    with patch.object(skills_tool, "get_wayne_home", return_value=tmp_path):
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import wayne_cli.main as hmain

    importlib.reload(hmain)
    # get_env_path() derives from WAYNE_HOME (set above) → tmp_path/.env, so
    # no patching is needed. Re-clear os.environ provider keys that
    # load_wayne_dotenv may have populated at import/reload time, forcing the

...
def wayne_home(tmp_path, monkeypatch):
    ...
    # /etc/wayne on the dev/CI box can't influence the test.
    monkeypatch.setenv("WAYNE_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(wayne_home, monkeypatch):
    from wayne_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(wayne_home, monkeypatch):
    from wayne_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(wayne_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_wayne_dotenv(wayne_home=str(home))

...
from wayne_cli import home_migration as hm
...
class TestPlatformDefaultDynamicResolution:
    def test_prefers_new_root_when_present(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        assert wayne_constants._get_platform_default_wayne_home() == legacy

        new.mkdir()
        assert wayne_constants._get_platform_default_wayne_home() == new

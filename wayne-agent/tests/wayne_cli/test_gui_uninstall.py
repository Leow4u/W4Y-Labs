"""Tests for wayne_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import wayne_cli.gui_uninstall as gu


def _make_agent(wayne_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = wayne_home / "wayne-agent"
    (agent_root / "wayne_cli").mkdir(parents=True)
    (agent_root / "wayne_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(wayne_home: Path) -> None:
    """Create the source-built GUI artifacts a `wayne desktop` run produces."""
    desktop = wayne_home / "wayne-agent" / "apps" / "desktop"
    ...
    (wayne_home / "wayne-agent" / "node_modules").mkdir(parents=True)
    (wayne_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(wayne_home: Path) -> None:
    (wayne_home / "config.yaml").write_text("x: 1\n")
    (wayne_home / ".env").write_text("KEY=secret\n")
    (wayne_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    wayne_home = tmp_path / ".wayne"
    wayne_home.mkdir()
    assert gu.agent_is_installed(wayne_home) is False
    _make_agent(wayne_home)
    assert gu.agent_is_installed(wayne_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    wayne_home = tmp_path / ".wayne"
    (wayne_home / "wayne-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(wayne_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    wayne_home = tmp_path / ".wayne"
    _make_gui_build(wayne_home)
    artifacts = gu.source_built_gui_artifacts(wayne_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(wayne_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(wayne_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    wayne_home = tmp_path / ".wayne"
    agent_root = _make_agent(wayne_home)
    _make_gui_build(wayne_home)
    _make_user_data(wayne_home)
    ...
    removed = gu.uninstall_gui(wayne_home)
    ...
    assert not (wayne_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "wayne_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (wayne_home / "config.yaml").exists()
    assert (wayne_home / ".env").exists()
    assert (wayne_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    wayne_home = tmp_path / ".wayne"
    _make_agent(wayne_home)
    userdata = tmp_path / "Wayne-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(wayne_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(wayne_home)
    ...
    assert summary["wayne_home"] == str(wayne_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Wayne"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Wayne"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Wayne"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Wayne"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    WAYNE_HOME with all the destructive externals stubbed out.
    """
    import wayne_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_wayne_home", lambda: wayne_home)
    ...
    from wayne_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (wayne_home / "config.yaml").exists()
    assert wayne_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole WAYNE_HOME non-interactively."""
    import wayne_cli.uninstall as uninstall

    wayne_home = tmp_path / ".wayne"
    (wayne_home / "wayne-agent" / "wayne_cli").mkdir(parents=True)
    (wayne_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not wayne_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m wayne_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_wayne_home", lambda: wayne_home)
    ...
    assert not (desktop / "release").exists()
    assert not (wayne_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "wayne_cli").exists()
    assert (wayne_home / "config.yaml").exists()

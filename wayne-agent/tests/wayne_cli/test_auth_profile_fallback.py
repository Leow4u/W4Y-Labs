...
When ``WAYNE_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.wayne/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.wayne            (has its own auth.json fixture)
    * Profile     -> tmp_path/.wayne/profiles/coder   (active, WAYNE_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Wayne is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from wayne_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    wayne_home = tmp_path / "classic"
    wayne_home.mkdir()
    monkeypatch.setenv("WAYNE_HOME", str(wayne_home))

    _write(wayne_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (WAYNE_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.wayne as the "real home" — which is NOT equal
    # to the WAYNE_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    wayne_home = tmp_path / "classic"
    ...
    _write(wayne_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from wayne_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: WAYNE_HOME is set to a custom path that is NOT under
    # ~/.wayne/profiles/ — get_default_wayne_root() returns WAYNE_HOME

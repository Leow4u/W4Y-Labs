"""Tests for the top-level `./wayne` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./wayne` should use `wayne_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "wayne"
    called = []

    fake_main_module = types.ModuleType("wayne_cli.main")

    def fake_main():
        called.append("wayne_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "wayne_cli.main", fake_main_module)

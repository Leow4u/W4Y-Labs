...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (WAYNE_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via WAYNE_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in WAYNE_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from wayne_cli.providers import WAYNE_OVERLAYS
        assert "bedrock" in WAYNE_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from wayne_cli.providers import WAYNE_OVERLAYS
        assert WAYNE_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from wayne_cli.providers import WAYNE_OVERLAYS
        assert WAYNE_OVERLAYS["bedrock"].auth_type == "aws_sdk"

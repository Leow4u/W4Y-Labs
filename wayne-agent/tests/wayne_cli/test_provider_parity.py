...
the SAME provider universe as ``wayne model`` (the CLI/TUI picker).
...
def test_every_wayne_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `wayne model` universe."""
    ...
    assert not missing, (
        "providers shown in `wayne model` but not configurable in the desktop "

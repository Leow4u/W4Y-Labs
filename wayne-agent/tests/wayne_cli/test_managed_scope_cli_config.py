...
wayne_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `wayne config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_wayne_home = get_wayne_home()`` at import time (module
    singleton), so monkeypatching WAYNE_HOME after import doesn't move it.
    ...
    this only matters for tests that swap WAYNE_HOME.)
    """
    import cli

    cli._wayne_home = home

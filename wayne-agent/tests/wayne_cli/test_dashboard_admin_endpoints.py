...
def _client():
    ...
    # Keep the state DB under the isolated WAYNE_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_wayne_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_wayne_home):
        ...
        (get_wayne_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "wayne_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("wayne-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # wayne-index source advertises availability + featured search.
            idx = _Src("wayne-index")
        ...
        assert {"official", "github", "wayne-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "wayne_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/wayne/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/wayne/update`` runs ``wayne update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/wayne/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/wayne/update/check").json()
...
class TestDebugShareEndpoint:
    """POST /api/ops/debug-share writes the bundle to local files and returns
    the file paths synchronously (W4Y fork: remote upload is disabled)."""
    ...
    def test_returns_structured_local_paths(self, monkeypatch):
        import wayne_cli.debug as dbg

        monkeypatch.setattr(dbg, "_best_effort_sweep_expired_pastes", lambda: None)
        monkeypatch.setattr("wayne_cli.dump.run_dump", lambda a: None)
        ...
        # ``urls`` carries local file paths, not paste URLs.
        from pathlib import Path

        for path in body["urls"].values():
            assert Path(path).is_file()
            assert "paste.rs" not in path and "dpaste.com" not in path
    ...
    def test_write_failure_returns_502(self, monkeypatch):
        import wayne_cli.debug as dbg

        def _boom():
            raise OSError("disk full")

        monkeypatch.setattr(dbg, "_new_share_dir", _boom)
        monkeypatch.setattr(dbg, "_best_effort_sweep_expired_pastes", lambda: None)
        monkeypatch.setattr("wayne_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `wayne tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_wayne_action", _fake_spawn)
        r = self.client.post(

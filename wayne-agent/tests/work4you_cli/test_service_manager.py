...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "work4you_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "work4you_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "work4you_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "work4you_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "work4you_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "work4you_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "work4you_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "work4you_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "work4you_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "work4you_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    # in work4you_cli/gateway.py::_gateway_command_inner.
...
def test_s6_register_staging_dir_is_dotfile_hidden_from_svscan(
    ...
    """
    import work4you_cli.service_manager as sm
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from work4you_cli.service_manager import S6CommandError

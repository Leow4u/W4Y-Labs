"""Tests for the `wayne memory reset` CLI command.
...
- Profile-scoped reset (uses WAYNE_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake WAYNE_HOME with memory files."""
    wayne_home = tmp_path / ".wayne"
    memories = wayne_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("WAYNE_HOME", str(wayne_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nWayne repo is at ~/.wayne/wayne-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `wayne memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        wayne_home = tmp_path / ".wayne"
        (wayne_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("WAYNE_HOME", str(wayne_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's WAYNE_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_wayne_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"

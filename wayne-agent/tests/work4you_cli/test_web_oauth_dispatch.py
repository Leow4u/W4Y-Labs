...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "work4you_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "work4you_cli.web_server._start_anthropic_pkce",
...
def test_status_hardcoded_branch_wins_over_generic_fallback():
    ...
    with patch(
        "work4you_cli.auth.get_nous_auth_status",
...
def test_status_unknown_provider_degrades_to_logged_out():
    ...
    with patch("work4you_cli.auth.get_auth_status", return_value={"logged_in": False}):

...
def _make_wayne_tree(root: Path) -> None:
    ...
    (root / "work4you_state.db").write_bytes(b"fake-state")
...
class TestValidateBackupZip:
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only work4you_state.db (old wrong name) is rejected."""
        from work4you_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["work4you_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestQuickSnapshot:
    ...
    def test_restore_nonexistent(self, wayne_home):
        from work4you_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", wayne_home=wayne_home) is False

    def test_auto_prune(self, wayne_home):
        from work4you_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, wayne_home):
        from work4you_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", wayne_home=wayne_home)
...
class TestRunPreUpdateBackup:
    ...
    def wayne_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for work4you_cli.config + work4you_constants so they pick up WAYNE_HOME
    ...
    def test_config_enabled_creates_backup(self, wayne_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("work4you_cli.config"):
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, wayne_home):
        from work4you_cli.backup import create_pre_migration_backup

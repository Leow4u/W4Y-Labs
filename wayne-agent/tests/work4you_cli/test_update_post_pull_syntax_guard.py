...
markers in ``work4you_cli/config.py``; users who ran ``wayne update`` in
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="work4you_cli/config.py")

    ok, failing_path, error = wayne_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("work4you_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="work4you_cli/main.py")

    ok, failing_path, _ = wayne_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("work4you_cli/main.py")

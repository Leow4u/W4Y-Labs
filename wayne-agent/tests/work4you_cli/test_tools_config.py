...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "work4you_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(
        "work4you_cli.nous_subscription.get_nous_portal_account_info",
...
def test_visible_providers_force_fresh_shows_nous_subscription_after_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        "work4you_cli.tools_config.get_nous_subscription_features",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from work4you_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "work4you_cli.tools_config._configure_tool_category_for_reconfig",
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("work4you_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
    }
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    with mock_patch("work4you_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from work4you_cli.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from work4you_cli.tools_config import _save_platform_tools
    config = {}
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "work4you_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("work4you_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("work4you_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("work4you_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("work4you_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "work4you_cli.nous_subscription.ensure_nous_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from work4you_cli.tools_config import apply_provider_selection
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    seq = iter([1, 0, 1])
    with patch("work4you_cli.model_switch.list_authenticated_providers",

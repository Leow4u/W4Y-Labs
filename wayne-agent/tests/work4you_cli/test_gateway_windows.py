...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("work4you_cli.config.get_wayne_home", lambda: str(wayne_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "work4you_cli.main"]
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_wayne_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".wayne"
        home.mkdir()
        monkeypatch.setattr("work4you_cli.config.get_wayne_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.config.get_wayne_home", lambda: missing)
...
def test_remove_legacy_artifacts_deletes_task_and_derived_launchers(monkeypatch, tmp_path, capsys):
    ...
    import work4you_cli.config as wayne_config
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("work4you_cli.gateway._get_restart_drain_timeout", lambda: 5.0)

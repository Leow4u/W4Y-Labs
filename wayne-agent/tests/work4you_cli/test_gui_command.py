...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("work4you_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skip_build_launches_existing_packaged_app_without_npm(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.shutil.which", return_value=None), \
         patch("work4you_cli.main._run_npm_install_deterministic") as mock_install, \
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("work4you_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_falls_back_to_no_sandbox_when_userns_is_restricted(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
         patch("work4you_cli.main._desktop_linux_needs_no_sandbox", return_value=True), \
         patch("work4you_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_exits_when_sandbox_fixup_fails_without_safe_fallback(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
         patch("work4you_cli.main._desktop_linux_needs_no_sandbox", return_value=False), \
         patch("work4you_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("work4you_cli.main._desktop_build_needed", return_value=True), \
         patch("work4you_cli.main._write_desktop_build_stamp"), \
         patch("work4you_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("work4you_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("work4you_cli.main._desktop_build_needed", return_value=False), \
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("work4you_cli.main._electron_dist_ok", return_value=False), \
         patch("work4you_cli.main._redownload_electron_dist", return_value=True), \
         patch("work4you_cli.main.subprocess.run", side_effect=run_side_effect) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_redownloads_electron_via_mirror_then_repacks(tmp_path, monkeypatch, capsys):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("work4you_cli.main._electron_dist_ok", return_value=False), \
         patch("work4you_cli.main._redownload_electron_dist", side_effect=[False, True]) as mock_dl, \
         patch("work4you_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_retries_pack_under_mirror_even_when_prefetch_blocked(tmp_path, monkeypatch, capsys):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("work4you_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("work4you_cli.main._electron_dist_ok", return_value=False), \
         patch("work4you_cli.main._redownload_electron_dist", return_value=False), \
         patch("work4you_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_self_heals_electron_and_continues(tmp_path, monkeypatch, capsys):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("work4you_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("work4you_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("work4you_cli.main._write_desktop_build_stamp"), \
         patch("work4you_cli.main._electron_dist_ok", return_value=False), \
         patch("work4you_cli.main._try_redownload_electron_dist", return_value=True) as mock_dl, \
         patch("work4you_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_hard_fails_when_electron_dist_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("work4you_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("work4you_cli.main._electron_dist_ok", return_value=True), \
         patch("work4you_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_override_user_electron_mirror(tmp_path, monkeypatch, capsys):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("work4you_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("work4you_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("work4you_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
         patch("work4you_cli.main.subprocess.run", side_effect=[pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_redownload_electron_dist_noop_when_present(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("work4you_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_runs_installer_with_mirror(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("work4you_cli.main.subprocess.run", side_effect=fake_run):
...
def test_redownload_electron_dist_returns_false_when_download_fails(tmp_path, monkeypatch):
    ...
    with patch("work4you_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("work4you_cli.main.subprocess.run",

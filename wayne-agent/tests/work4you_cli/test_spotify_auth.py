...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes WORK4YOU_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("WAYNE_HOME", str(tmp_path))
    ...
    assert "WORK4YOU_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "WORK4YOU_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "WORK4YOU_SPOTIFY_CLIENT_ID" not in env_path.read_text()

...
def test_resolve_wayne_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_wayne_argv() == [sys.executable, "-m", "work4you_cli.main"]
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from work4you_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("work4you_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from work4you_cli.kanban_db import connect, write_txn
    import work4you_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from work4you_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("work4you_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("work4you_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.kanban_db.os.name", "nt")
    with patch("work4you_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("work4you_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("work4you_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "work4you_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("work4you_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "work4you_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("work4you_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("work4you_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("work4you_cli.kanban_db._record_worker_exit"):
            with patch("work4you_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()

...
Run from the PR worktree:
    ...
    cd ~/.wayne/wayne-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.wayne/wayne-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.wayne/wayne-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a wayne-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a wayne-agent checkout"
)
...
# Isolated WAYNE_HOME for the config write.

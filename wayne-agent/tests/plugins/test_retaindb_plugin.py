...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure WAYNE_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", wayne_home=str(tmp_path / ".wayne"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", wayne_home=profile_home)
        assert p._client.project == "wayne-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".wayne" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        wayne_home = tmp_path / ".wayne"
        wayne_home.mkdir(exist_ok=True)
        monkeypatch.setenv("WAYNE_HOME", str(wayne_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", wayne_home=str(wayne_home))
        return p

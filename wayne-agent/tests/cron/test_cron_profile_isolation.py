...
profile's WAYNE_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.wayne/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active WAYNE_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever WAYNE_HOME the ticker happens to have —
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile WAYNE_HOME (<root>/profiles/<name>), the cron store
    ...
    try:
        assert jobs.WAYNE_DIR.resolve() == profile_home.resolve()
        assert (
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_wayne_home", None, raising=False)
    assert scheduler._get_wayne_home().resolve() == profile_home.resolve()
    assert scheduler._get_wayne_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (WAYNE_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.WAYNE_DIR.resolve() == root.resolve()

...
- Path resolution (absolute, relative to WAYNE_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp WAYNE_HOME."""
    wayne_home = tmp_path / ".wayne"
    wayne_home.mkdir()
    (wayne_home / "cron").mkdir()
    (wayne_home / "cron" / "output").mkdir()
    (wayne_home / "scripts").mkdir()
    monkeypatch.setenv("WAYNE_HOME", str(wayne_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "WAYNE_DIR", wayne_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", wayne_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", wayne_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", wayne_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Wayne provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _WAYNE_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_WAYNE_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.wayne/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("WAYNE_SESSION_PLATFORM") is None
        assert os.environ.get("WAYNE_SESSION_CHAT_ID") is None
        assert os.environ.get("WAYNE_SESSION_CHAT_NAME") is None

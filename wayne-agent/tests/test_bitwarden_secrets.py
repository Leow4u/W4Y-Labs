...
binary download are exercised manually by `wayne secrets bitwarden
...
def wayne_home(tmp_path, monkeypatch):
    """Point Wayne at an isolated home directory."""
    home = tmp_path / ".wayne"
    home.mkdir()
...
def test_install_bws_rejects_malicious_member(wayne_home, monkeypatch):
...
def test_install_bws_happy_path(wayne_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(wayne_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(wayne_home, monkeypatch):
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under wayne_home/cache/."""
    home = tmp_path / ".wayne"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")

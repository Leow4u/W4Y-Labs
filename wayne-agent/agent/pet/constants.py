...
row taxonomy is inferred from the atlas shape so Wayne can render both legacy
...
# rows (1536x1872), while older Wayne/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Wayne' activity state names. They are not always identical to the
...
# Legacy Wayne/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Wayne activity names -> accepted row-name aliases in descending

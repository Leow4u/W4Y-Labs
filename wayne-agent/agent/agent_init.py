...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .wayne.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / WAYNE_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.wayne/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from wayne_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind WAYNE_DEV_CREDITS.
    ...
    # both live under ~/.wayne/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from wayne_logging import setup_logging, setup_verbose_logging
    setup_logging(wayne_home=_ra()._wayne_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that wayne_logging.setup_logging() does not
        ...
        # handler level inside wayne_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from wayne_cli.config import load_config as _load_br_cfg
    ...
    # worker (kanban_show tool is present iff WAYNE_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["WAYNE_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.wayne/sessions/ alongside gateway sessions
    wayne_home = get_wayne_home()
    agent.logs_dir = wayne_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.wayne/sessions/session_{sid}.json)
    ...
    try:
        from wayne_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from wayne_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "wayne"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Wayne' built-in
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from wayne_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from wayne_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Wayne Agent.  Choose a model with at least "
    ...
    # Nous Wayne 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from wayne_cli.model_switch import _check_wayne_model_warning

            _wayne_warn = _check_wayne_model_warning(agent.model or "")
            if _wayne_warn:
                _user_msg = (
                    "⚠ Nous Research Wayne 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_wayne_warn)
        except Exception:
            pass

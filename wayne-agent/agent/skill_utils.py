...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``WAYNE_KANBAN_TASK`` / ``WAYNE_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("WAYNE_KANBAN_TASK") or os.getenv("WAYNE_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Wayne Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``WAYNE_PLATFORM`` or
            ``WAYNE_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``wayne`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.wayne/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``wayne`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against WAYNE_HOME, not cwd
        if not p.is_absolute():
            p = (wayne_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Wayne can discover and view
...
def get_skill_metadata_namespace(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """Return the agent-specific metadata block from parsed frontmatter.

    Reads ``metadata.wayne`` first and falls back to ``metadata.hermes`` so
    skills published for the upstream hermes-agent / agentskills.io ecosystem
    keep working unmodified. Skill creation always writes ``metadata.wayne``;
    the fallback is read-side only. Returns ``{}`` when neither namespace is
    a mapping (e.g. malformed YAML parsed the value as a string).
    """
    metadata = frontmatter.get("metadata")
    if not isinstance(metadata, dict):
        return {}
    for key in ("wayne", "hermes"):
        ns = metadata.get(key)
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    wayne = get_skill_metadata_namespace(frontmatter)
    return {
        "fallback_for_toolsets": wayne.get("fallback_for_toolsets", []),
        "requires_toolsets": wayne.get("requires_toolsets", []),
        "fallback_for_tools": wayne.get("fallback_for_tools", []),
        "requires_tools": wayne.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Wayne metadata, VCS, virtualenv/dependency, cache, and skill

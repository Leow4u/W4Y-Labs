...
Storage
-------
Bundles live in ``~/.wayne/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``WAYNE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``wayne bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under WAYNE_HOME.

    Honors ``WAYNE_BUNDLES_DIR`` for tests; falls back to
    ``<WAYNE_HOME>/skill-bundles``.
    """
    override = os.environ.get("WAYNE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_wayne_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `wayne bundles` CLI subcommand.
# ---------------------------------------------------------------------------

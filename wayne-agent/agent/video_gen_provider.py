...
as ``kind: backend``) or ``~/.wayne/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``wayne tools``
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$WAYNE_HOME/cache/videos/``.

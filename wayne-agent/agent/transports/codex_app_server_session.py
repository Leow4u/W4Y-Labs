...
Owns one Codex thread per Wayne session. Drives `turn/start`, consumes
...
# Wayne' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_WAYNE_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Wayne/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Wayne session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _WAYNE_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("WAYNE_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="wayne",
            client_title="Wayne Agent",
            client_version=_get_wayne_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Wayne' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Wayne' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own wayne-tools callback we
            # auto-accept — the user already approved Wayne' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Wayne approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Wayne returns 'once', 'session', 'always', or 'deny'.
...
def _get_wayne_version() -> str:
    """Best-effort Wayne version string for codex's userAgent line."""
    try:

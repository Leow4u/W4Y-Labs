...
projection into Wayne' display, approval bridging, transcript projection into
...
"codex_app_server"`. Wayne' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Wayne secret — gateway
        ...
        spawn_env = wayne_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("WAYNE_KANBAN_TASK"):
            kanban_db = spawn_env.get("WAYNE_KANBAN_DB")
            kanban_root = (
    ...
    def initialize(
        self,
        client_name: str = "wayne",
        client_title: str = "Wayne Agent",

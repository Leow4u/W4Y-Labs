...
def _find_wayne_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.wayne.md`` or ``WAYNE.md``.
    ...
    # could pick up a .wayne.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _WAYNE_MD_NAMES:
...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $WAYNE_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $WAYNE_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${WAYNE_KANBAN_BRANCH:-wt/$WAYNE_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$WAYNE_KANBAN_BRANCH` a deterministic "
...
#
# The wayne-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to wayne-agent's Python
...
# container / remote host rather than on the machine where Wayne itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Wayne runs on.
    """
...
def build_environment_hints() -> str:
    ...
    # Wayne desktop GUI — any agent running under the desktop app should know
    # it. WAYNE_DESKTOP marks the backend powering the chat; WAYNE_DESKTOP_TERMINAL
    # marks a wayne launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    ...
    # Embedder-supplied environment description. Lets a host that wraps Wayne
...
def _skills_prompt_snapshot_path() -> Path:
    return get_wayne_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.wayne/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("WAYNE_PLATFORM")
        or get_session_env("WAYNE_SESSION_PLATFORM")
        or ""
    )
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    ...
    soul_path = get_wayne_home() / "SOUL.md"
...
def _load_wayne_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.wayne.md / WAYNE.md — walk to git root."""
    wayne_md_path = _find_wayne_md(cwd_path)
    if not wayne_md_path:
        return ""
    try:
        content = wayne_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = wayne_md_path.name
        try:
            rel = str(wayne_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".wayne.md", context_length=context_length,
            read_path=str(wayne_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", wayne_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .wayne.md / WAYNE.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from WAYNE_HOME is independent and always included when present.
    ...
    project_context = (
        _load_wayne_md(cwd_path, context_length)
    ...
    # SOUL.md from WAYNE_HOME only — skip when already loaded as identity
    if not skip_soul:

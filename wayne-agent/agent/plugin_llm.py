...
Plugins built on Wayne Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Wayne
...
:class:`~wayne_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Wayne' main config
...
handles every provider, fallback chain, and per-task override Wayne
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`wayne_cli.plugins.PluginContext`

...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Wayne starts and finds ``~/.openclaw/``.

    Points users at ``wayne claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``wayne claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Wayne, run "
        "`wayne claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `wayne claw cleanup` (renames it to ~/.openclaw.pre-migration — "

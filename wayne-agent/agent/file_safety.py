...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    wayne_home = _wayne_home_path()
    wayne_root = _wayne_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(wayne_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved WAYNE_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("WAYNE_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    for base in (_wayne_home_path(), _wayne_root_path()):
        try:
            ...
            if real not in wayne_dirs:
                wayne_dirs.append(real)
        except Exception:
            continue

    for base_real in wayne_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Wayne path.

    Three categories are blocked:

      * Internal Wayne cache files under ``WAYNE_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under WAYNE_HOME and the global Wayne
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.wayne/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active WAYNE_HOME (profile-aware) AND the global
    # Wayne root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (WAYNE_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    wayne_dirs: list[Path] = []
    for base in (_wayne_home_path(), _wayne_root_path()):
        try:
            real = base.resolve()
            if real not in wayne_dirs:
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Wayne read (see
...
#
# Wayne profiles are separate WAYNE_HOME dirs under
...
#
# Reference: May 2026 incident where a wayne-security profile session
# edited skills under both ``~/.wayne/profiles/wayne-security/skills/``
# AND ``~/.wayne/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under WAYNE_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from WAYNE_HOME.
    ...
    """
    try:
        home_real = _wayne_home_path().resolve()
        root_real = _wayne_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Wayne scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Wayne entirely. Caller is expected to surface the warning to the
...
#
#   <WAYNE_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.wayne/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.wayne/…`` segment and warns
# regardless of which Wayne profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.wayne/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.wayne`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.wayne/…`` and returns the
    index where the inner Wayne-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .wayne / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".wayne":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Wayne state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.wayne``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.wayne`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Wayne resolver to
    succeed, so it works correctly even when called from contexts where
    WAYNE_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
# full ``…/sandboxes/<backend>/<task>/home/.wayne/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.wayne/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host WAYNE_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Wayne state.

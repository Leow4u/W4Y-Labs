...
def _state_file() -> Path:
    return get_wayne_home() / "skills" / ".curator_state"
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `wayne update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``wayne curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``wayne update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``wayne curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `wayne curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    ...
    "to find: wayne-config-*, wayne-dashboard-*, gateway-*, codex-*, "
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.wayne/logs/curator/``)
    ...
    authored skill data in ``~/.wayne/skills/``.

    ``ensure_wayne_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_wayne_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every wayne install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: wayne curator status
        keep an umbrella stable: wayne curator pin document-tools
    ...
    lines.append("full report: wayne curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: wayne curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.wayne/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "safety and can be restored via `wayne curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Restore any via `wayne curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `wayne curator restore <name>`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``wayne curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `wayne curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `wayne update` only if "
        ...
        # recorded in state so `wayne curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``wayne model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `wayne model` → auxiliary

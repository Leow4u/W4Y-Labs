"""Unified removal contract for every credential source Wayne reads from.

Wayne seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.wayne/.env
    claude_code   — ~/.claude/.credentials.json
    wayne_pkce   — ~/.wayne/.anthropic_oauth.json
    ...
    manual        — user ran `wayne auth add`
...
unify here is **removal**:
    ...
    ``wayne auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), wayne_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.wayne/.env  → clear it
    ...
    """
    from wayne_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"  The pool entry is now suppressed — Wayne will ignore "
            f"{env_var} until you run `wayne auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Wayne stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `wayne auth add anthropic` to re-enable if needed.",
    ])


def _remove_wayne_pkce(provider: str, removed) -> RemovalResult:
    """~/.wayne/.anthropic_oauth.json is ours — delete it outright."""
    from wayne_constants import get_wayne_home

    result = RemovalResult()
    oauth_file = get_wayne_home() / ".anthropic_oauth.json"
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `wayne auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `wayne auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``wayne auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `wayne model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Wayne auth store is not enough — _seed_from_singletons() would
    ...
    via ``wayne auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `wayne auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `wayne auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Wayne stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `wayne auth add copilot` to re-enable if needed.",
    ])

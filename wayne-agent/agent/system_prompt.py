...
upstream prefix cache warm.  See ``wayne-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Wayne never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want WAYNE_HOME persona while keeping
    ...
    # Pointer to the wayne-agent skill + docs for user questions about Wayne itself.
    stable_parts.append(WAYNE_AGENT_HELP_GUIDANCE)
    ...
    # WAYNE_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Wayne, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Wayne profile the agent is running
    # under so it doesn't conflate ~/.wayne/skills/ (default profile) with
    # ~/.wayne/profiles/<active>/skills/ (this profile's). Deterministic
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Wayne never rebuilds or reinjects parts of it

...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `wayne backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - wayne_home (str): The active WAYNE_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.wayne``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "wayne").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'wayne memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], wayne_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'wayne memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``wayne_home`` is the active WAYNE_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE WAYNE_HOME.

        ``wayne backup`` only walks WAYNE_HOME, so any provider state kept
        ...
        of the archive, and ``wayne import`` restores them to their original

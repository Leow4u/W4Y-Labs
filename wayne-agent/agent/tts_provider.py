...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Wayne with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.wayne/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Wayne
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``wayne tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``wayne setup`` for

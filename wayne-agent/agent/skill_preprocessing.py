...
# Matches ${WAYNE_SKILL_DIR} / ${WAYNE_SESSION_ID} tokens in SKILL.md.
# The HERMES_* spellings are accepted too so skills published for the upstream
# hermes-agent / agentskills.io ecosystem keep working unmodified.
# Tokens that don't resolve (e.g. ${WAYNE_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(
    r"\$\{(?:WAYNE|HERMES)_(SKILL_DIR|SESSION_ID)\}"
)
...
def substitute_template_vars(
    ...
    """Replace ${WAYNE_SKILL_DIR} / ${WAYNE_SESSION_ID} in skill content.

    ${HERMES_SKILL_DIR} / ${HERMES_SESSION_ID} are accepted as aliases for
    ecosystem skills authored against upstream hermes-agent.

...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the WAYNE_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Wayne runtime / deployment
    "WAYNE_HOME", "WAYNE_PROFILE", "WAYNE_GATEWAY_LOCK_DIR",
    "WAYNE_MAX_ITERATIONS", "WAYNE_MAX_TOKENS", "WAYNE_API_TIMEOUT",
    "WAYNE_REDACT_SECRETS", "WAYNE_NOUS_TIMEOUT_SECONDS",
    "_WAYNE_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "WAYNE_KANBAN_DB", "WAYNE_KANBAN_WORKSPACES_ROOT", "WAYNE_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "WAYNE_KANBAN_",
    "WAYNE_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(wayne_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(wayne_home) / ".env")

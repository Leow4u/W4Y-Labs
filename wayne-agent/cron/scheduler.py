...
Uses a file-based lock (~/.wayne/cron/.tick.lock) so only one tick
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``wayne tools`` config for the ``cron`` platform.
...
_wayne_home: Path | None = None


def _get_wayne_home() -> Path:
    """Resolve Wayne home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active WAYNE_HOME at call time
    ...
    """
    return _wayne_home or get_wayne_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    wayne_home = _get_wayne_home()
    lock_dir = wayne_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("WAYNE_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid WAYNE_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within WAYNE_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Wayne-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against WAYNE_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_wayne_home() / "scripts"
    ...
    # escape — scripts MUST reside within WAYNE_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _wayne_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_wayne_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    # gateway message. Do not seed WAYNE_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads WAYNE_SESSION_PLATFORM
    #     and WAYNE_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via WAYNE_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     WAYNE_SESSION_PLATFORM == "telegram".
    ...
    #     WAYNE_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read WAYNE_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the WAYNE_CRON_AUTO_DELIVER_* vars set
    # below, so clearing WAYNE_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "WAYNE_CRON_AUTO_DELIVER_PLATFORM",
        "WAYNE_CRON_AUTO_DELIVER_CHAT_ID",
        "WAYNE_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        # load_wayne_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this WAYNE_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        reset_secret_source_cache()
        load_wayne_dotenv(wayne_home=_get_wayne_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["WAYNE_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["WAYNE_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["WAYNE_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > WAYNE_MODEL env >
        ...
        model = job.get("model") or os.getenv("WAYNE_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_wayne_home() / "config.yaml")
        ...
        prefill_file = (
            os.getenv("WAYNE_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject WAYNE_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # WAYNE_HOME. When a workdir is configured, also inject project
        ...
        # override via WAYNE_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("WAYNE_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid WAYNE_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_wayne_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_wayne_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_wayne_home())
        )
        try:
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _wayne_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _wayne_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set WAYNE_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("WAYNE_CRON_MAX_PARALLEL", "").strip()
            if _env_par:

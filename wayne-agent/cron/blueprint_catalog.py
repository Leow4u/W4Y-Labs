...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``wayne://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``wayne://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"wayne://blueprint/{quote(blueprint.key)}{qs}"

...
``wayne cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
blocks these commands at *execution* time when ``_WAYNE_GATEWAY=1``, and
``wayne gateway stop|restart`` refuse to self-target from inside the
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<WAYNE_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.wayne/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_wayne_home() / "scripts" / raw

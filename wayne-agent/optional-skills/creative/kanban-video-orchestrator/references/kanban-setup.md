...
the actual `setup.sh` that creates the project workspace, configures Wayne
...
2. **Create profiles** — `wayne profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.wayne/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `wayne kanban create` assigned to the director
...
```bash
wayne profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.wayne/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.wayne/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Wayne
...
Each profile gets a `SOUL.md` at `~/.wayne/profiles/<name>/SOUL.md` that
...
```bash
wayne kanban create "Direct production of <video title>" \
    --assignee director \
...
the Wayne `.env` (`${WAYNE_HOME:-$HOME/.wayne}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _wayne_env="${WAYNE_HOME:-$HOME/.wayne}/.env"
    if grep -q "^${var}=" "$_wayne_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_wayne_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_wayne_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY wayne ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY wayne OPENROUTER_API_KEY || exit 1
# ...
```

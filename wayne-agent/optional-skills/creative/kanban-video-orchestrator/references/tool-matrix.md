...
Maps each role archetype to the Wayne skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public wayne-agent
...
## Wayne skills relevant to video production

### Visual / rendering skills (`wayne-agent/skills/creative/`)
...
### Design / pre-production skills (`wayne-agent/skills/creative/`)
...
### Audio / media skills (`wayne-agent/skills/creative/` + `skills/media/`)
...
These are **not** Wayne skills but external CLIs / APIs that profiles invoke.
...
## Built-in Wayne tools for media review

These are native Wayne tools — not invoked via terminal but through their own
...
key is present in `${WAYNE_HOME:-~/.wayne}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Wayne profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${WAYNE_HOME:-~/.wayne}/.env` → environment variable.
...
When a new Wayne-public video skill ships:

...
work4you kanban watch --tenant <project-slug>

# Snapshot of the board
work4you kanban list --tenant <project-slug>
work4you kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
work4you kanban stats --tenant <project-slug>

# Visual dashboard (browser)
work4you dashboard

# Inspect a specific task (includes comments + events)
work4you kanban show <task-id>

# Follow a single task's event stream
work4you kanban tail <task-id>
```

Verify available subcommands with `work4you kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `work4you kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `work4you kanban block <id>` then `work4you kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `work4you kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `work4you kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `work4you profile list` (profile exists?), `work4you status` (gateway/dispatcher up?), and verify tenant. |
...
work4you kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
work4you kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(work4you kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
work4you kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
work4you kanban block <task-id>
work4you kanban archive <task-id>

# Diagnose what happened
work4you kanban show <task-id>      # task body, comments, recent events
work4you kanban tail <task-id>      # follow the live event stream
work4you kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `work4you kanban create` for the director
...
while true; do
    clear
    work4you kanban list --tenant <slug>
    echo "---"
    work4you kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `work4you kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `work4you profile list` and start

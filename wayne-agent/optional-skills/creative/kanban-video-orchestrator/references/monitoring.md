...
wayne kanban watch --tenant <project-slug>

# Snapshot of the board
wayne kanban list --tenant <project-slug>
wayne kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
wayne kanban stats --tenant <project-slug>

# Visual dashboard (browser)
wayne dashboard

# Inspect a specific task (includes comments + events)
wayne kanban show <task-id>

# Follow a single task's event stream
wayne kanban tail <task-id>
```

Verify available subcommands with `wayne kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `wayne kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `wayne kanban block <id>` then `wayne kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `wayne kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `wayne kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `wayne profile list` (profile exists?), `wayne status` (gateway/dispatcher up?), and verify tenant. |
...
wayne kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
wayne kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(wayne kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
wayne kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
wayne kanban block <task-id>
wayne kanban archive <task-id>

# Diagnose what happened
wayne kanban show <task-id>      # task body, comments, recent events
wayne kanban tail <task-id>      # follow the live event stream
wayne kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `wayne kanban create` for the director
...
while true; do
    clear
    wayne kanban list --tenant <slug>
    echo "---"
    wayne kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `wayne kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `wayne profile list` and start

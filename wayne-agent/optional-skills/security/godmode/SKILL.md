...
exec(open(os.path.expanduser(
    os.path.join(os.environ.get("WAYNE_HOME", os.path.expanduser("~/.wayne")), "skills/red-teaming/godmode/scripts/load_godmode.py")
)).read())
...
1. **Reads `~/.wayne/config.yaml`** to detect the current model
...
7. **If a strategy works**, locks it in:
   ...
   - Writes prefill messages to `~/.wayne/prefill.json`
...
exec(open(os.path.join(os.environ.get("WAYNE_HOME", os.path.expanduser("~/.wayne")), "skills/red-teaming/godmode/scripts/parseltongue.py")).read())
...
```python
# Via execute_code
exec(open(os.path.join(os.environ.get("WAYNE_HOME", os.path.expanduser("~/.wayne")), "skills/red-teaming/godmode/scripts/godmode_race.py")).read())
...
12. **execute_code sandbox has no env vars** — When Wayne runs auto_jailbreak via execute_code, the sandbox doesn't inherit the Wayne `.env`. Load dotenv explicitly: `import os; from dotenv import load_dotenv; load_dotenv(os.path.join(os.environ.get("WAYNE_HOME", os.path.expanduser("~/.wayne")), ".env"))`

...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("WAYNE_HOME", os.path.expanduser("~/.wayne")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("WAYNE_HOME", Path.home() / ".wayne")) / "skills" / "red-teaming" / "godmode"
...
WAYNE_HOME = Path(os.getenv("WAYNE_HOME", Path.home() / ".wayne"))
CONFIG_PATH = WAYNE_HOME / "config.yaml"
PREFILL_PATH = WAYNE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Wayne models — already uncensored, just needs clean prompt
    "wayne": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "wayne" in model_lower or "nous" in model_lower:
        return "wayne"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Wayne config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.wayne/prefill.json."""

...
This skill is separate from the Wayne Email gateway adapter. The gateway
adapter lets people email the agent and uses Wayne' built-in IMAP/SMTP
...
To reply non-interactively from Wayne, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Wayne)** — pipe the message via stdin:

---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. wayne-agent@agentmail.to).
version: 1.0.0
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `wayne-agent`)
   - The agent gets address: `wayne-agent@agentmail.to`
2. Send an email:

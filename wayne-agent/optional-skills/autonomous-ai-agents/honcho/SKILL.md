---
name: honcho
description: Configure and use Honcho memory with Wayne -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Wayne Agent
license: MIT
...
# Honcho Memory for Wayne

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Wayne profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/wayne#running-honcho-locally-with-wayne

### Verify

```bash
wayne honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Wayne profile's AI peer
...
Honcho models conversations as interactions between **peers**. Wayne creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Wayne instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Wayne run |
| `global` | Single session across all directories |

Manual override: `wayne honcho map my-project-name`
...
Each Wayne profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
wayne profile create coder --clone
# creates host block wayne.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `wayne.coder` host block in `honcho.json`
...
```bash
wayne honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Wayne when Honcho memory is active.
...
Config file: `$WAYNE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `wayne honcho setup`. Ensure `memory.provider: honcho` is in `~/.wayne/config.yaml`.

### Memory not persisting across sessions
Check `wayne honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `wayne profile create <name> --clone`. For existing profiles: `wayne honcho sync`.
...
| Command | Description |
|---------|-------------|
| `wayne honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `wayne honcho status` | Show resolved config, connection test, peer info for active profile |
| `wayne honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `wayne honcho disable` | Disable Honcho for the active profile |
| `wayne honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `wayne honcho peers` | Show peer identities across all profiles |
| `wayne honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `wayne honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `wayne honcho sessions` | List known directory-to-session-name mappings |
| `wayne honcho map <name>` | Map current working directory to a Honcho session name |
| `wayne honcho identity` | Seed AI peer identity or show both peer representations |
| `wayne honcho sync` | Create host blocks for all Wayne profiles that don't have one yet |
| `wayne honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Wayne + Honcho |
| `wayne memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `wayne memory status` | Show active memory provider and config |
| `wayne memory off` | Disable external memory provider |

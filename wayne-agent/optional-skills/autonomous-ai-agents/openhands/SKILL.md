...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Wayne Agent
license: MIT
platforms: [linux, macos]
metadata:
  wayne:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, wayne-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Wayne.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Wayne-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `wayne-agent` (Wayne subagents via `delegate_task`).

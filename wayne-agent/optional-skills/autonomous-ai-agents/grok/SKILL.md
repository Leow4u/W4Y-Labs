...
metadata:
  wayne:
    ...
    related_skills: [codex, claude-code, wayne-agent]
---

# Grok Build CLI — Wayne Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Wayne terminal. Grok
...
1. Prepare stable input files first with Wayne tools (`read_file`,
...
2. **Don't conflate Wayne' xAI auth with the `grok` CLI's auth.** Wayne'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Wayne' xAI auth carries
   over.

...
2. **Don't conflate Work4You's xAI auth with the `grok` CLI's auth.** Work4You's
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Work4You's xAI auth carries
   over.

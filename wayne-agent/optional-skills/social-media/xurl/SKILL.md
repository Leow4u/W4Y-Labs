...
> **Docker HOME pitfall:** In the official Work4You Docker layout, `/opt/data` is `WAYNE_HOME`, but Work4You tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Work4You-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash

...
version: 0.1.0
author: Teknium (teknium1), Wayne Agent
license: MIT
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so Wayne can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — Wayne cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Wayne attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "Wayne" --interval 5 --timeout 300
```

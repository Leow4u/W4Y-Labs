...
All three live in `$WAYNE_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$WAYNE_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $WAYNE_HOME/skills/devops/watchers/scripts/watch_rss.py \
...
```bash
python $WAYNE_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$WAYNE_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $WAYNE_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $WAYNE_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$WAYNE_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.

...
Usage (via cron with --no-agent):
    ...
    wayne cron create wayne-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$WAYNE_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name wayne-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Wayne .env file
(``${WAYNE_HOME:-~/.wayne}/.env``) to avoid the 60 req/hr

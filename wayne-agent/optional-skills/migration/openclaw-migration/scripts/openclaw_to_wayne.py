...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Wayne.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Wayne memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Wayne memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Wayne-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Wayne-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Wayne command_allowlist.",
    },
    "skills": {
        "label": "User skills",
    ...
    "discord-settings": {
        ...
        "description": "Import Discord bot token and allowlist into Wayne .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Wayne .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Wayne .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Wayne .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Wayne .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Wayne config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Wayne config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Wayne.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Wayne MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Wayne config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'wayne cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Wayne config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Wayne session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Wayne custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Wayne config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Wayne config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Wayne config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Wayne config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Wayne in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Wayne`` (prose), but lowercase matches
# like ``openclaw`` → ``wayne`` (so filesystem paths like ``~/.openclaw``
# become ``~/.wayne`` — the real Wayne home — not the broken ``~/.Wayne``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Wayne'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Wayne'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Wayne'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Wayne`` but maps ``openclaw`` → ``wayne`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.wayne/config.yaml`` (the real Wayne home) instead of the broken
    ``~/.Wayne/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Wayne.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Wayne Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-wayne/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Wayne config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-wayne/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Wayne session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Wayne config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Wayne
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Wayne-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Wayne-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: wayne config set",
                        )
                    continue
        ...
        for oc_key, wayne_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and wayne_key not in secret_additions:
                secret_additions[wayne_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, wayne_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and wayne_key not in secret_additions:
                        secret_additions[wayne_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        wayne_config = load_yaml_file(destination)
        current_model = wayne_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = wayne_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                wayne_config["model"] = {"default": model_str}
            dump_yaml_file(destination, wayne_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Wayne still uses "edge"
        ...
        wayne_config = load_yaml_file(destination)
        existing_tts = wayne_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Wayne destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Wayne-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Wayne-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        wayne_cfg_path = self.target_root / "config.yaml"
        wayne_cfg = load_yaml_file(wayne_cfg_path)
        existing_mcp = wayne_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Wayne config")
                continue

            wayne_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                wayne_srv["command"] = srv["command"]
                if srv.get("args"):
                    wayne_srv["args"] = srv["args"]
                if srv.get("env"):
                    wayne_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    wayne_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                wayne_srv["url"] = srv["url"]
                if srv.get("headers"):
                    wayne_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    wayne_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                wayne_srv["enabled"] = False
            if srv.get("timeout"):
                wayne_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                wayne_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                wayne_srv["tools"] = {}
                if tools_cfg.get("include"):
                    wayne_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    wayne_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(wayne_cfg_path)
            wayne_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(wayne_cfg_path, wayne_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'wayne cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = wayne_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Wayne reasoning_effort
        ...
        if compaction:
            compression = wayne_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'wayne gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("WAYNE_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        wayne_cfg_path = self.target_root / "config.yaml"
        wayne_cfg = load_yaml_file(wayne_cfg_path)
        sr = wayne_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        wayne_cfg_path = self.target_root / "config.yaml"
        wayne_cfg = load_yaml_file(wayne_cfg_path)
        custom_providers = wayne_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(wayne_cfg_path)
            wayne_cfg["custom_providers"] = custom_providers
            dump_yaml_file(wayne_cfg_path, wayne_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Wayne config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            wayne_cfg_path = self.target_root / "config.yaml"
            wayne_cfg = load_yaml_file(wayne_cfg_path)
            discord_wayne = wayne_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_wayne["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_wayne["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                wayne_cfg["discord"] = discord_wayne
                dump_yaml_file(wayne_cfg_path, wayne_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        wayne_cfg_path = self.target_root / "config.yaml"
        wayne_cfg = load_yaml_file(wayne_cfg_path)
        browser_wayne = wayne_cfg.get("browser") or {}
        changed = False

        # Map fields that have Wayne equivalents
        if browser.get("cdpUrl"):
            browser_wayne["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_wayne["headless"] = browser["headless"]
            changed = True

        if changed:
            wayne_cfg["browser"] = browser_wayne
            if self.execute:
                self.maybe_backup(wayne_cfg_path)
                dump_yaml_file(wayne_cfg_path, wayne_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            wayne_mode = mode_map.get(mode, "manual")
            wayne_cfg.setdefault("approvals", {})["mode"] = wayne_mode
            if self.execute:
                self.maybe_backup(wayne_cfg_path)
                dump_yaml_file(wayne_cfg_path, wayne_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{wayne_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Wayne Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Wayne. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Wayne Config Not Overwritten)",
                "",
                "These items already existed in your Wayne config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Wayne agent discovers these",
            "directories, it may read/write to them instead of the Wayne state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `wayne claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `wayne claw cleanup` to fix it.",
            "",
            "## Wayne-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `wayne claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `wayne setup` to configure any remaining settings",
            "- Run `wayne mcp list` to verify MCP servers were imported correctly",
        ])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Wayne targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Wayne-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Wayne Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")

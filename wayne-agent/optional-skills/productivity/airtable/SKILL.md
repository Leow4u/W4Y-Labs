...
4. Store the token in `${WAYNE_HOME:-~/.work4you}/.env` (or via `work4you setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${WAYNE_HOME:-~/.work4you}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.

...
- Skill helper paths:
  - `${WAYNE_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${WAYNE_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${WAYNE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client wayne
```
...
```bash
PUBLISH="${WAYNE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client wayne
```
...
```bash
DRIVE="${WAYNE_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `wayne`)    |

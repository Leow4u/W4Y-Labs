...
3. Store in `${WAYNE_HOME:-~/.work4you}/.env`:
   ```
...
Add those exports to your shell profile (or to `${WAYNE_HOME:-~/.work4you}/.env`) so every session inherits them.
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) — wire it via Work4You's MCP support if you want streaming Notion access from inside a session, but the paths above are enough for most one-shot tasks.

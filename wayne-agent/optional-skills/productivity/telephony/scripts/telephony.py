#!/usr/bin/env python3
"""Telephony helper for the Wayne optional telephony skill.

Capabilities:
- Persist telephony provider credentials to the Wayne .env file ($WAYNE_HOME/.env)
...
def _env_path() -> Path:
    return _wayne_home() / ".env"


def _config_path() -> Path:
    return _wayne_home() / "config.yaml"


def _state_path() -> Path:
    return _wayne_home() / "telephony_state.json"
...
def _load_root_config() -> dict[str, Any]:
    ...
    if not path.exists():
        return {}
    try:
        import yaml  # optional dependency; Wayne already ships PyYAML
    except Exception:
        return {}
    try:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Wayne telephony helper")

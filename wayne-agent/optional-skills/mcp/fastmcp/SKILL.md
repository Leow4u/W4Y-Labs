...
- smoke-test a server with the FastMCP CLI before wiring it into Wayne or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Wayne. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.wayne/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.wayne/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Wayne integration, either:

- configure the server in `~/.wayne/config.yaml` using the `native-mcp` skill, or
...
### Wayne cannot see the deployed server

The server-building part may be correct while the Wayne config is not. Load the `native-mcp` skill and configure the server in `~/.wayne/config.yaml`, then restart Wayne.

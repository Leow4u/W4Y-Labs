...
import upstream classes into Wayne itself.
...
```bash
mkdir -p ~/.wayne/cache/darwinian-evolver && cd ~/.wayne/cache/darwinian-evolver
...
```bash
cd ~/.wayne/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.wayne/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.wayne/skills/research/darwinian-evolver
DE_DIR=~/.wayne/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Wayne core.
   Custom driver scripts under `~/.wayne/skills/...` are user-side and fine.

...
Wayne is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Wayne ships a **lot** — most merges are bug fixes to
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `WAYNE_HOME`. Mocks hide integration bugs.
...
- **New `WAYNE_*` env vars for non-secret config.** `.env` is for secrets
...
`$HOME/.wayne/wayne-agent/venv` (for worktrees that share a venv with the
main checkout).
...
│   ├── wayne-achievements/  # Gamified achievement tracking
...
Applies to TypeScript across Wayne: desktop, TUI, website, and future TS packages.
...
```
2. Add handler in `WayneCLI.process_command()` in `cli.py`:
```python
...
core Wayne tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_WAYNE_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_WAYNE_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_wayne_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `WAYNE_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_wayne_home()` for the base directory — never `Path.home() / ".wayne"`. This ensures each profile gets its own state.
...
- `default` — Classic Wayne gold/kawaii (the current look)
...
`post_setup(wayne_home, config)` for setup-wizard integration.
...
2. User: `$WAYNE_HOME/plugins/model-providers/<name>/`
...
[`wayne-example-plugins`](https://github.com/NousResearch/wayne-example-plugins)
...
`metadata.wayne.tags`, `metadata.wayne.category`,
`metadata.wayne.related_skills`, `metadata.wayne.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.wayne.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Wayne tools or
...
lives in the `wayne-agent-dev` skill at
...
`"messaging"`); `_WAYNE_CORE_TOOLS` is the default bundle most
...
Wayne returns a delegation id immediately and the result re-enters the
...
- **Board** is the hard boundary — workers are spawned with
  `WAYNE_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Wayne-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `WAYNE_BACKGROUND_NOTIFICATIONS` env var):
...
3. **Module-level constants are fine** — they cache `get_wayne_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_wayne_home()`,
   not `Path.home() / ".wayne"`.

4. **Tests that mock `Path.home()` must also set `WAYNE_HOME`** — since code now uses
   `get_wayne_home()` (reads env var), not `Path.home() / ".wayne"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"WAYNE_HOME": str(tmp_path / ".wayne")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not WAYNE_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".wayne" / "profiles"`, NOT `get_wayne_home() / "profiles"`.
   This is intentional — it lets `wayne -p coder profile list` see all profiles regardless

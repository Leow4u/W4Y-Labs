...
const ENV_KEYS = ['WAYNE_AUTO_HEAPDUMP', 'WAYNE_HEAPDUMP_DIR', 'WAYNE_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'wayne-heapdump-test-'))
    process.env.WAYNE_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without WAYNE_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without WAYNE_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when WAYNE_AUTO_HEAPDUMP=1', async () => {
    process.env.WAYNE_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.WAYNE_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of WAYNE_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.WAYNE_HEAPDUMP_DIR
    savedMax = process.env.WAYNE_HEAPDUMP_MAX_BYTES
    delete process.env.WAYNE_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'wayne-heapdump-prune-'))
    process.env.WAYNE_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.WAYNE_HEAPDUMP_DIR
    } else {
      process.env.WAYNE_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.WAYNE_HEAPDUMP_MAX_BYTES
    } else {
      process.env.WAYNE_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.WAYNE_HEAPDUMP_MAX_BYTES = String(2 * 1024)

...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.WAYNE_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Wayne on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via WAYNE_TUI_TRUECOLOR=1.

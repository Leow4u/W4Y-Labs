...
const logDir = join(process.env.WAYNE_HOME?.trim() || join(homedir(), '.wayne'), 'logs')
...
// a real ~/.wayne (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('wayne-tui: parent lifecycle log unavailable\n')
    }
  }
}

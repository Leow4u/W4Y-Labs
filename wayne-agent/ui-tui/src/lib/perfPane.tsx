...
//
// Both gate on WAYNE_DEV_PERF=1 and dump JSON-lines (default ~/.wayne/perf.log,
// override WAYNE_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// WAYNE_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
const THRESHOLD_MS = Number(process.env.WAYNE_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.WAYNE_DEV_PERF_LOG?.trim() || join(homedir(), '.wayne', 'perf.log')

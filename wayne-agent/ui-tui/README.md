...
React + Ink terminal UI for Wayne. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `WAYNE_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `wayne setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.wayne/.wayne_history` or under `WAYNE_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `wayne setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/wayne-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external wayne setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.wayne/tui-parent.log
    ...
    types/
      wayne-ink.d.ts            type declarations for @wayne/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent WayneCLI subprocess for slash commands
```
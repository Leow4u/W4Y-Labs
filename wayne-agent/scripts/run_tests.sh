#!/usr/bin/env bash
# Canonical test runner for wayne-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.wayne/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.wayne/wayne-agent/venv"; do
...
if [ -f "$HOME/.wayne/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.wayne"
...
exec env -i \
  ...
  ${WAYNE_RUN_SLOW_PET_TESTS:+WAYNE_RUN_SLOW_PET_TESTS="$WAYNE_RUN_SLOW_PET_TESTS"} \

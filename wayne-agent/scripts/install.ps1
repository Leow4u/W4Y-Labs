...
param(
    ...
    #     `wayne desktop` already builds on demand.
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $WayneHome)) {
        New-Item -ItemType Directory -Force -Path $WayneHome | Out-Null
    }
    $envFile = Join-Path $WayneHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $WayneHome "node"
...
function Install-Uv {
    ...
    Write-Info "Installing managed uv into $WayneHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $WayneHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $WayneHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $WayneHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and WAYNE_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Wayne-Setup.exe
...
function Install-Git {
    ...
    which Wayne uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\wayne\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Wayne needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Wayne copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\wayne\git``
    ...
    # Download PortableGit into $WayneHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $WayneHome\git\ ..."
...
function Set-GitBashEnvVar {
    ...
    #
    # Layouts:
    #   PortableGit (our default): $WayneHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $WayneHome\git\usr\bin\bash.exe
    $candidates += "$WayneHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$WayneHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
...
function Test-Node {
    ...
    # The portable zip path drops node.exe + npm into $WayneHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $WayneHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\wayne-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$WayneHome\node") { Remove-Item -Recurse -Force "$WayneHome\node" }
                Move-Item $extractedDir.FullName "$WayneHome\node"
                ...
                $version = & "$WayneHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $WayneHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\wayne-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
        ...
        if ($SourceZipUrl -and (Test-Path "$InstallDir\pyproject.toml")) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `wayne
                    # update` path (#4735).
                    ...
                    $stashName = "wayne-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``wayne update`` and
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Wayne-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running wayne process.
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Wayne_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `wayne update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `wayne.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install wayne-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and wayne.exe broken with
    ...
    # here, not 30 seconds later when the user runs `wayne`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register wayne.exe in dist-info/RECORD but fail to
        ...
        # `wayne` command while wayne-agent.exe / wayne-acp.exe exist
...
"@ 2>$null
    ...
    # users hit and lazy-import errors from `wayne dashboard` are confusing.
...
function Write-BootstrapMarker {
    ...
    $markerPath = Join-Path $InstallDir ".wayne-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the WAYNE_HOME directory structure ($WayneHome, default %LOCALAPPDATA%\wayne)
    New-Item -ItemType Directory -Force -Path "$WayneHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WayneHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WayneHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WayneHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WayneHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WayneHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WayneHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WayneHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WayneHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$WayneHome\.env"
    ...
    # Create config.yaml
    $configPath = "$WayneHome\config.yaml"
    ...
    Write-Info "Syncing bundled skills to $WayneHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $WayneHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$WayneHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $WayneHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Wayne-Setup.exe runs each -Stage NAME
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\wayne-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\wayne-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\wayne-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Wayne` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Wayne-managed
    ...
    # stamped onto Wayne.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\wayne-desktop-build-$(Get-Random).log"
    ...
    #    Wayne.exe. We deliberately do NOT point them at `wayne desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        # with the Wayne icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.wayne/.env are importable.  Two problems this solves:
    #
...
function Start-GatewayIfConfigured {
    ...
    $whatsappSession = "$WayneHome\whatsapp\session\creds.json"
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$WayneHome\logs\gateway.log"
            Start-Process -FilePath $wayneCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$WayneHome\logs\gateway-error.log" `
                -WindowStyle Hidden
...
function Write-Completion {
    ...
    Write-Host "$WayneHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$WayneHome\.env"
...
if ($IncludeDesktop) {
    ...
    # (Wayne-Setup.exe), never via the irm|iex CLI one-liner.

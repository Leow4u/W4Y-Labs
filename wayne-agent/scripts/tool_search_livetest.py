#!/usr/bin/env python3
"""Live test harness for Wayne Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any wayne imports.
ORIGINAL_HOME = os.environ.get("WAYNE_HOME")
ORIGINAL_AUTH = Path.home() / ".wayne" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.wayne/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.wayne/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="wayne_ts_live_"))
    wayne_home = home_dir / ".wayne"
    wayne_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, wayne_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".wayne" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, wayne_home / ".env")
    ...
    (wayne_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return wayne_home
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["WAYNE_HOME"] = str(home)
...
def main():
    ...
    # Restore original WAYNE_HOME
    if ORIGINAL_HOME is not None:
        os.environ["WAYNE_HOME"] = ORIGINAL_HOME
    else:

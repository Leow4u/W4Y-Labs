...
# Python-driven tool session (e.g. Wayne terminal tool). A pre-set PYTHONPATH
...
#   code at /usr/local/lib/wayne-agent, command at /usr/local/bin/wayne,
#   data still at /root/.wayne (WAYNE_HOME).  Matches Claude Code / Codex CLI
...
# intentional, but they leave the checkout dirty — which forces `wayne update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Wayne-Setup) passes it so
...
# Point a Wayne-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $WAYNE_HOME/node/bin — which is NOT on
...
# Wayne's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Wayne-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$WAYNE_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$WAYNE_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$WAYNE_HOME/node/etc/npmrc"
}
...
install_uv() {
    ...
    log_info "Installing managed uv into $WAYNE_HOME/bin ..."
    mkdir -p "$WAYNE_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/wayne-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/wayne-uv-installer.$$.sh")"
    ...
    # directly into $WAYNE_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$WAYNE_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Wayne-
...
check_node() {
    ...
    # Repair pre-existing Wayne-managed installs where `npm install -g` lands
...
install_node() {
    ...
    # Place into ~/.wayne/node/ and symlink binaries into the same bin dir
    # the wayne command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$WAYNE_HOME/node"
    mkdir -p "$WAYNE_HOME"
    mv "$extracted_dir" "$WAYNE_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$WAYNE_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$WAYNE_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$WAYNE_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    if [ -d "$INSTALL_DIR" ]; then
        ...
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `wayne update` path
                # (#4735).
                ...
                local stash_name
                stash_name="wayne-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``wayne update`` and reset to the
...
EOF
    ...
    rm -f "$command_link_dir/wayne"
    cat > "$command_link_dir/wayne" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$WAYNE_BIN" "\$@"
...
install_node_deps() {
    ...
    # Keep the checkout clean so `wayne update` doesn't autostash every run.
...
maybe_start_gateway() {
    ...
    WHATSAPP_SESSION="$WAYNE_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $WAYNE_CMD gateway install 2>/dev/null; then
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $WAYNE_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $WAYNE_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $WAYNE_HOME/cron/, sessions/, logs/"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$WAYNE_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$WAYNE_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$WAYNE_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$WAYNE_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$WAYNE_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
# extract a tree MISSING the electron binary, so the `electron`->`Wayne` rename
...
# _purge_electron_build_cache() used by `wayne desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Wayne-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `wayne desktop`),
    ...
    # trips Gatekeeper's tamper check ("Wayne is damaged and can't be opened").
    ...
    # checkout stays clean for the next `wayne update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Wayne-managed Node
            # provisioned during prerequisites/node-deps (at $WAYNE_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $WAYNE_HOME.
    # $WAYNE_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'wayne update' on this host install.

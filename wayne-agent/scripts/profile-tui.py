#!/usr/bin/env python3
"""Drive the Wayne TUI under WAYNE_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.wayne/perf.log on exit.
...
Environment overrides:
  WAYNE_PERF_LOG     (default ~/.wayne/perf.log)
  WAYNE_PERF_NODE    (default node from $PATH)
  WAYNE_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests WAYNE_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(
    os.environ.get("WAYNE_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("WAYNE_PERF_LOG", str(get_wayne_home() / "perf.log")))
DEFAULT_STATE_DB = get_wayne_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — WAYNE_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["WAYNE_DEV_PERF"] = "1"
    env["WAYNE_DEV_PERF_MS"] = str(args.threshold_ms)
    env["WAYNE_DEV_PERF_LOG"] = str(log)
    env["WAYNE_TUI_RESUME"] = sid
    ...
    node = os.environ.get("WAYNE_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="WAYNE_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "wayne-ink" / "src"

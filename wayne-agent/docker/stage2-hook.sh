...
#
# Wired into the image as /etc/cont-init.d/01-wayne-setup by the
...
# starts non-root. The baked install tree under /opt/wayne is intentionally
...
# `id -u wayne` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# wayne UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u wayne)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-wayne UID).
...
and the baked /opt/wayne install tree is intentionally root-owned and
...
# `s6-setuidgid wayne mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom WAYNE_HOME paths whose parents only
# root can create (e.g. `WAYNE_HOME=/home/wayne/.wayne` in a Compose
...
if [ -n "${WAYNE_UID:-}" ] && validate_uid_gid "$WAYNE_UID" && [ "$WAYNE_UID" != "$(id -u wayne)" ]; then
    echo "[stage2] Changing wayne UID to $WAYNE_UID"
    usermod -u "$WAYNE_UID" wayne
fi
if [ -n "${WAYNE_GID:-}" ] && validate_uid_gid "$WAYNE_GID" && [ "$WAYNE_GID" != "$(id -g wayne)" ]; then
    echo "[stage2] Changing wayne GID to $WAYNE_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$WAYNE_GID" wayne 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised wayne user (UID 10000)
...
# wayne process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes wayne. Idempotent across container
...
#   - wayne is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G wayne 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] wayne already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" wayne 2>/dev/null; then
        echo "[stage2] Added wayne to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group wayne failed; docker backend may fail with EACCES"
    fi
    break
done
...
# the runtime wayne UID, restore ownership to wayne — but ONLY for the
# directories wayne actually writes to. The full $WAYNE_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of wayne-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_wayne_uid=$(id -u wayne)
...
chown_wayne_tree() {
    ...
    chown -R wayne:wayne "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$WAYNE_HOME" 2>/dev/null)" != "$actual_wayne_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $WAYNE_HOME (targeted) to wayne ($actual_wayne_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $WAYNE_HOME: chown the directory itself (not its contents)
    # so wayne can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$WAYNE_HOME"; then
        :
    else
        chown wayne:wayne "$WAYNE_HOME" 2>/dev/null || \
    ...
    # created and managed exclusively by wayne (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$WAYNE_HOME/$sub" ]; then
            chown_wayne_tree "$WAYNE_HOME/$sub"
        fi
    done
fi
...
# wayne user. Hosted/container instances keep mutable state under
# $WAYNE_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# WAYNE_DISABLE_LAZY_INSTALLS=1. Keeping /opt/wayne root-owned and
...
# even though installs are re-enabled. The dir is seeded + chowned to wayne
...
# Always reset ownership of $WAYNE_HOME/profiles to wayne on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> wayne …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as wayne and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$WAYNE_HOME/profiles" ]; then
    chown_wayne_tree "$WAYNE_HOME/profiles"
fi
...
# (jobs.json) must stay readable by the unprivileged wayne runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$WAYNE_HOME/cron" ]; then
    chown_wayne_tree "$WAYNE_HOME/cron"
fi

# Reset ownership of wayne-owned top-level state files on every boot.
# The targeted data-volume chown above only covers wayne-owned
# *subdirectories*; loose state files living directly under $WAYNE_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> wayne …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged wayne runtime then hits
...
for f in \
    ...
    if [ -e "$WAYNE_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$WAYNE_HOME/$f"; then
            ...
            chown wayne:wayne "$WAYNE_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the wayne runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$WAYNE_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$WAYNE_HOME/config.yaml"; then
        :
    else
        chown wayne:wayne "$WAYNE_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$WAYNE_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as wayne user ---
# Run as wayne via s6-setuidgid so dirs end up owned correctly (matters
...
# /opt/wayne/.install_method (see Dockerfile), NOT written here into
...
# bind-mounted from the host (~/.wayne:/opt/data) and sometimes shared with a
...
# 'wayne update'. To heal homes already poisoned by older images, remove a
...
# the baked /opt/wayne stamp, so deleting the data-dir copy is safe).
...
seed_one() {
    ...
    if [ ! -f "$WAYNE_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$WAYNE_HOME/$dest"; then
            ...
            as_wayne cp "$INSTALL_DIR/$src" "$WAYNE_HOME/$dest"
        fi
    fi
}
...
if [ -f "$WAYNE_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$WAYNE_HOME/.env"; then
        ...
        chown wayne:wayne "$WAYNE_HOME/.env" 2>/dev/null || true
...
# config-schema migrations that `wayne update` runs for non-Docker installs,
...
if [ -f "$WAYNE_HOME/config.yaml" ]; then
    s6-setuidgid wayne "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$WAYNE_HOME/auth.json" ] && [ -n "${WAYNE_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$WAYNE_HOME/auth.json"; then
        ...
        chown wayne:wayne "$WAYNE_HOME/auth.json" 2>/dev/null || true
...
if [ ! -f "$WAYNE_HOME/gateway_state.json" ] && \
    ...
    if refuse_symlinked_path "seed" "$WAYNE_HOME/gateway_state.json"; then
        ...
        chown wayne:wayne "$WAYNE_HOME/gateway_state.json" 2>/dev/null || true
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_wayne "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/wayne/.playwright) with
...
# main-wrapper.sh propagates it into the supervised ``wayne`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-wayne, dashboard, etc.).

...
# by /etc/cont-init.d/01-wayne-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs wayne"
...
echo "[wayne] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/wayne/docker/stage2-hook.sh "$@"

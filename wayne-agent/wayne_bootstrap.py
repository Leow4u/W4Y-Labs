"""Windows UTF-8 bootstrap for Wayne entry points.
...
should be imported at the very top of every Wayne entry point
(``wayne``, ``wayne-agent``, ``wayne-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Wayne modules.

    Wayne ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Wayne source root
    ...
    """
    root = src_root or os.environ.get("WAYNE_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``WAYNE_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("WAYNE_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import wayne_bootstrap``
# (or ``from wayne_bootstrap import apply_windows_utf8_bootstrap``) at

"""
Wayne MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Wayne-specific extra)

Usage:
    wayne mcp serve
    wayne mcp serve --verbose
...
logger = logging.getLogger("wayne.mcp_serve")
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("WAYNE_HOME", Path.home() / ".wayne")
...
class EventBridge:
    ...
    This is the Wayne equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Wayne MCP server with all tools registered."""
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Wayne MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

...
_UNSET = object()
_WAYNE_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
...
def set_wayne_home_override(path: str | Path | None) -> Token:
    ...
    return _WAYNE_HOME_OVERRIDE.set(value)


def reset_wayne_home_override(token: Token) -> None:
    """Restore the previous context-local Wayne home override."""
    _WAYNE_HOME_OVERRIDE.reset(token)


def get_wayne_home_override() -> str | None:
    """Return the active context-local Wayne home override, if any."""
    override = _WAYNE_HOME_OVERRIDE.get()
...
def get_wayne_home() -> Path:
    """Return the Wayne home directory (default: platform-native path).

    Reads WAYNE_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``WAYNE_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``WAYNE_HOME`` explicitly (see the systemd
    template in ``wayne_cli/gateway.py`` and the kanban dispatcher in
    ``wayne_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_wayne_home()
    ...
    return _get_platform_default_wayne_home()


def get_default_wayne_root() -> Path:
    """Return the root Wayne directory for profile-level operations.

    In standard deployments this is the platform-native Wayne home
    (``~/.wayne`` on POSIX, ``%LOCALAPPDATA%\\wayne`` on native Windows).

    In Docker or custom deployments where ``WAYNE_HOME`` points outside
    ``~/.wayne`` (e.g. ``/opt/data``), returns ``WAYNE_HOME`` directly
    — that IS the root.

    In profile mode where ``WAYNE_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.wayne/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_wayne_home()
    env_home = os.environ.get("WAYNE_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # WAYNE_HOME is under ~/.wayne (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — WAYNE_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Wayne is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``WAYNE_OPTIONAL_SKILLS``.
    """
    override = os.getenv("WAYNE_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_wayne_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``WAYNE_OPTIONAL_MCPS``.
    """
    override = os.getenv("WAYNE_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_wayne_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``WAYNE_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<WAYNE_HOME>/skills`` last-resort
    """
    override = os.getenv("WAYNE_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_wayne_home() / "skills"


def get_wayne_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Wayne subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to WAYNE_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to WAYNE_HOME (e.g. ``"image_cache"``).
...
def iter_wayne_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Wayne-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\wayne\\node``. POSIX installs use
    ``$WAYNE_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_wayne_home()
    dirs = [root / "node"]
...
_WAYNE_NODE_TARGET_MAJOR = int(os.environ.get("WAYNE_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Wayne-managed Node trees live under ``$WAYNE_HOME/node`` (or a profile's
    ``WAYNE_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_wayne_node_executable``
    used to trust file presence alone, so ``wayne update`` would pick that
...
def wayne_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Wayne-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_wayne_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%WAYNE_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_wayne_home()
    index_url = f"https://nodejs.org/dist/latest-v{_WAYNE_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_WAYNE_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_wayne_managed_node() -> bool:
    """Redownload Wayne-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not wayne_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "WAYNE_HOME": str(get_wayne_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_wayne_node_executable(command: str) -> str | None:
    """Return a Wayne-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_wayne_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_wayne_managed_node():
        for directory in iter_wayne_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Wayne-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Wayne-managed installs.

    This is for Wayne-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_wayne_node_executable(command)
    if managed:
        return managed
    if wayne_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_wayne_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Wayne-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_wayne_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``wayne update`` — leaving a **dangling symlink** that ``which`` still
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``WAYNE_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{WAYNE_HOME}/home`` when the profile-home directory exists."""
    wayne_home = get_wayne_home_override() or (env or {}).get("WAYNE_HOME") or os.getenv("WAYNE_HOME")
    if not wayne_home:
        return None
    profile_home = os.path.join(wayne_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("WAYNE_REAL_HOME") or os.getenv("WAYNE_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Wayne profile HOME.

    ``WAYNE_HOME`` scopes Wayne state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={WAYNE_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{WAYNE_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{WAYNE_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Wayne' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["WAYNE_REAL_HOME"] = real_home
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under WAYNE_HOME.

    Replaces the ``get_wayne_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, wayne_logging.py, wayne_time.py, etc.).
    """
    return get_wayne_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under WAYNE_HOME."""
    return get_wayne_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under WAYNE_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_wayne_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._wayne_ipv4_patched = True  # type: ignore[attr-defined]

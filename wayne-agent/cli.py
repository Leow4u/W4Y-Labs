...
Usage:
    ...
    python cli.py --skills wayne-agent-dev,github-auth
...
os.environ["WAYNE_QUIET"] = "1"  # Our own modules
...
load_wayne_dotenv(wayne_home=_wayne_home, project_env=_project_env)
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("WAYNE_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.wayne/config.yaml (user config - preferred)
    ...
    """
    # Check user config first ({WAYNE_HOME}/config.yaml)
    user_config_path = _wayne_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("WAYNE_IGNORE_USER_CONFIG") == "1"
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and WayneCLI.__init__ checks "default" first.
    ...
    # UNLESS we're inside a gateway process (detected by _WAYNE_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_WAYNE_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["WAYNE_REDACT_SECRETS"] = str(redact).lower()
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("WAYNE_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("WAYNE_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _arm_exit_watchdog(timeout_s: float | None = None) -> None:
    ...
    Tune with ``WAYNE_EXIT_WATCHDOG_S`` (seconds); ``0`` disables.
    """
    if timeout_s is None:
        try:
            timeout_s = float(os.getenv("WAYNE_EXIT_WATCHDOG_S", "30"))
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    wt_name = f"wayne-{short_id}"
    branch_name = f"wayne/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"wayne pid={os.getpid()}", str(wt_path)],
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    ...
    - ``"dead"``  — locked but the owning pid is gone, or the reason isn't a
                    parseable wayne lock (safe to unlock + reap).
    ...
    for line in result.stdout.splitlines():
        if line.startswith("worktree "):
            ...
            m = re.search(r"wayne pid=(\d+)", reason)
            if not m:
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_wayne_home_maint / "sessions"
                )
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    *dead*-locked one (owning pid gone — a crashed session) is unlocked first
    ...
    Also prunes orphaned ``wayne/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("wayne-"):
            continue
        ...
        # wayne process means the worktree is actively in use — never touch
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    orphaned = [
        ...
        and (b.startswith("wayne/wayne-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. WAYNE_LIGHT / WAYNE_TUI_LIGHT env (true/false) — explicit override
#   2. WAYNE_TUI_THEME=light|dark — explicit theme
#   3. WAYNE_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Wayne assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("WAYNE_LIGHT", "WAYNE_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("WAYNE_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("WAYNE_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    SkinConfig._wayne_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Wayne output it's common to display cron schedules verbatim.
    #
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_wayne_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_wayne_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._wayne_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_wayne_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._wayne_bp_start = now
        ...
        _vt100_mod._wayne_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``WayneCLI._busy_command()``.
        """
        yield self

# ASCII Art - WAYNE-AGENT logo (full width, single line - requires ~95 char terminal)
...
# ASCII Art - Wayne Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS WAYNE - AI Agent Framework"
        tiny_line = "⚕ NOUS WAYNE"
    else:
        agent_name = _skin.get_branding("agent_name", "Wayne Agent") if _skin else "Wayne Agent"
    ...
    if os.environ.get("WAYNE_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Wayne Agent v{_version} ({_release_date})"
    else:
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.wayne/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.wayne/config.yaml on first use)
...
class WayneCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: WAYNE_MAX_TOKENS)
        ...
        self.requested_provider = (
            ...
            or os.getenv("WAYNE_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("WAYNE_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("WAYNE_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        self.system_prompt = (
            os.getenv("WAYNE_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Wayne processes for this WAYNE_HOME.
        ...
        self._history_file = _wayne_home / ".wayne_history"
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - WayneCLI._status_bar_display_width(label)
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("WAYNE_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.wayne/sessions/saved/.
        ...
        saved_dir = get_wayne_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"wayne_conversation_{timestamp}.json"
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Wayne default (chat_completions)
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_wayne_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_wayne_home()}/config.yaml")
            print()
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``WAYNE_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``WAYNE_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors WAYNE_DEV_CREDITS_FIXTURE
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.wayne/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "wayne_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _wayne_home / "interrupt_debug.log"
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Wayne Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Wayne Agent! Type your message or /help for commands."
        ...
        # after an OpenClaw→Wayne migration (especially migrations done by
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Wayne binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from wayne_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Wayne Agent")
        ...
        # Dynamic prompt: shows Wayne symbol when agent is working,
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = WayneCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_wayne_osd_patched", False):
                ...
                _pt_renderer._wayne_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``WAYNE_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("WAYNE_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the wayne process when
...
def _run_kanban_goal_loop_q(cli: "WayneCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``WAYNE_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("WAYNE_KANBAN_TASK") or "").strip()
    if not task_id:
        return
...
def main(
    ...
    os.environ["WAYNE_INTERACTIVE"] = "1"
    ...
    # Default to wayne-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Wayne): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside WayneCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  WAYNE_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("WAYNE_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``wayne chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Wayne" box, no tool-gen
                        ...
                        if os.environ.get("WAYNE_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("WAYNE_KANBAN_TASK") and result.get(
                                "failure_reason"

...
patterns: `skill_view(name="wayne-agent", file_path="references/webhooks.md")`.
...
Full enumeration lives in `toolsets.py` as the `TOOLSETS` dict; `_WAYNE_CORE_TOOLS` is the default bundle most platforms inherit from.
...
`SOUL.md` (in `$WAYNE_HOME`) is independent and always loaded when present — it sets the agent's identity, not project rules.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/wayne_cli/test_auth_toctou_file_modes.py`)

...
metadata:
  wayne:
    tags: [skills, authoring, wayne-agent, conventions, skill-md]
...
- You're committing a reusable workflow that should ship with wayne-agent
- You're editing an existing skill under `/home/bb/wayne-agent/skills/` (use `patch` for small edits, `write_file` for rewrites; `skill_manage` still works for patch on in-repo skills, but not for `create`)

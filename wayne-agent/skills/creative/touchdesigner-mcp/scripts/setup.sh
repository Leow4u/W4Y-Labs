...
WAYNE_HOME_DIR="${WAYNE_HOME:-$HOME/.wayne}"
WAYNE_CFG="${WAYNE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Wayne config has twozero_td MCP entry ──
if [[ ! -f "$WAYNE_CFG" ]]; then
    echo -e " ${FAIL} Wayne config not found at ${WAYNE_CFG}"
    manual_steps+=("Create ${WAYNE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$WAYNE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Wayne config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Wayne config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${WAYNE_CFG} manually"); }
    manual_steps+=("Restart Wayne session to pick up config change")
fi

#!/usr/bin/env bash
# GitHub environment detection helper for Wayne Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _wayne_env="${WAYNE_HOME:-$HOME/.wayne}/.env"; [ -f "$_wayne_env" ] && grep -q "^GITHUB_TOKEN=" "$_wayne_env" 2>/dev/null; then

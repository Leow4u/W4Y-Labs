...
_WAYNE_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on WAYNE_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (WAYNE_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `wayne tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    # Coding posture (base Wayne — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    # ==========================================================================
    # Full Wayne toolsets (CLI + messaging platforms)
    #
    ...
    # the `wayne send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "wayne-cron": {
        # Mirrors wayne-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `wayne tools` filters
    ...
    "wayne-webhook": {
        ...
        "tools": _WAYNE_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "wayne-gateway": {
        ...
        "tools": [],
        "includes": ["wayne-telegram", "wayne-discord", "wayne-whatsapp", "wayne-slack", "wayne-signal", "wayne-bluebubbles", "wayne-homeassistant", "wayne-email", "wayne-sms", "wayne-mattermost", "wayne-matrix", "wayne-dingtalk", "wayne-feishu", "wayne-wecom", "wayne-wecom-callback", "wayne-weixin", "wayne-qqbot", "wayne-webhook", "wayne-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``wayne-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_WAYNE_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``wayne-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (wayne-<name>).
        # Gives them _WAYNE_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("wayne-"):
            platform_name = name[len("wayne-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_WAYNE_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(

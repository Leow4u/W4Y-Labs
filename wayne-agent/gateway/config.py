...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_wayne_home() / "sessions")
    ...
    # gateway behaves exactly as before — single WAYNE_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``wayne config set gateway.multiplex_profiles true``.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.wayne/config.yaml (primary user-facing config)
    3. ~/.wayne/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_wayne_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through wayne_cli.config.load_config, so without this a managed
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``wayne config set gateway.streaming.*``

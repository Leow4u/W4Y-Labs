...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``wayne setup`` display).
    ...
    # built-ins).  Used by ``wayne gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `wayne`
        # invocation -- including plain `wayne chat`, which never touches any
        ...
        # `wayne setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``wayne setup``/``gateway status``, channel

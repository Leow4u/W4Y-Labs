...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        try:
            ...
            return
        env_override = os.environ.get("WAYNE_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban notifier: disabled via WAYNE_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        try:
            cfg = _load_config()
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # WAYNE_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `wayne kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours WAYNE_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        env_override = os.environ.get("WAYNE_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via WAYNE_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Wayne profile
            ...
            PATH, missing venv, credential loss for a real Wayne profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from wayne_cli import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("WAYNE_KANBAN_BOARD")
                try:
                    os.environ["WAYNE_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("WAYNE_KANBAN_BOARD", None)
                    else:
                        os.environ["WAYNE_KANBAN_BOARD"] = prev_env
            return successes

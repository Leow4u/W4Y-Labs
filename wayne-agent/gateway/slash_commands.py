...
Module-level run.py helpers a handler needs (``_wayne_home``,
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Wayne commands.
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            ...
            atomic_json_write(
                _wayne_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _wayne_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Wayne default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_wayne_home())
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _wayne_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _wayne_home, _platform_config_key
        import yaml
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.wayne/pending/skills/``. (Note this is
        ...
        # the real review surface. (Note: `wayne skills diff <name>` is a
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _wayne_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        Gateway includes ONLY the summary report (system info + log tails),
        NOT full log files, to protect conversation privacy.  Users who need
        the full logs should use ``wayne debug share`` from the CLI.

        W4Y fork: remote upload is disabled — ``upload_to_pastebin`` writes
        ...
        # Run blocking I/O (dump capture, log reads, file writes) in a thread.
        def _collect_and_write():
            ...
            urls = {}
            try:
                # W4Y fork: writes to a local file and returns its path.
                urls["Report"] = upload_to_pastebin(report)
            except Exception as exc:
                return t("gateway.debug.upload_failed", error=exc)

            # No-op for local paths — kept so legacy pre-fork pastes recorded
            # in pending.json still get swept.
            _schedule_auto_delete(list(urls.values()))

            # W4Y fork: the stale "uploaded / auto-delete" locale strings are
            # intentionally not used — nothing was uploaded.
            lines = [_GATEWAY_PRIVACY_NOTICE, "", "**Debug report written to:**", ""]
            label_width = max(len(k) for k in urls)
            for label, path in urls.items():
                lines.append(f"`{label:<{label_width}}`  {path}")
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _wayne_home, _resolve_wayne_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if not wayne_cmd:
            return t("gateway.update.wayne_cmd_not_found")
        ...
        # Spawn `wayne update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `wayne update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                wayne_cmd_str = " ".join(shlex.quote(part) for part in wayne_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {wayne_cmd_str} update --gateway"

...
# ``WAYNE_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    """
    raw = os.environ.get("WAYNE_AUTO_CONTINUE_FRESHNESS")
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `wayne tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `wayne tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_wayne_home()}/cron/output/)"
    )
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.wayne/state.db and are shown by `wayne sessions list` and "

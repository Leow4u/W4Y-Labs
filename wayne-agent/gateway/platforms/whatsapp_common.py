...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Wayne Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to WAYNE_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/wayne), this function
    mirrors the bridge source to a writable WAYNE_HOME location and returns that
    ...
    wayne_home_bridge = wayne_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to WAYNE_HOME if needed
    if wayne_home_bridge.exists():
        return wayne_home_bridge

    # Mirror the bridge source to WAYNE_HOME
    try:
        wayne_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            wayne_home_bridge,
            dirs_exist_ok=False,

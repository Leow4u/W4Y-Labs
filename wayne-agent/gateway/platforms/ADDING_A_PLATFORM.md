...
There are two ways to add a platform to the Wayne gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.wayne/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Wayne code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `wayne gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `wayne_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Wayne core.
...
And add it to the `wayne-gateway` composite:

```python
"wayne-gateway": {
    "includes": [..., "wayne-your-platform"]
}
```
...
## 12. Status Display (`wayne_cli/status.py`)
...
## 13. Gateway Setup Wizard (`wayne_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ wayne_cli/ toolsets.py \
  --include="*.py" -l | sort -u

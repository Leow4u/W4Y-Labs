...
# Audio file extensions Wayne recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Wayne' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Wayne-created Telegram private-chat
...
IMAGE_CACHE_DIR = get_wayne_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_wayne_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_wayne_dir(new_subpath, old_name)
...
AUDIO_CACHE_DIR = get_wayne_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_wayne_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_wayne_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_wayne_dir("cache/screenshots", "browser_screenshots")
...
_WAYNE_ROOT = get_default_wayne_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "WAYNE_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "WAYNE_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "WAYNE_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "WAYNE_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _WAYNE_HOME / "image_cache",
    _WAYNE_HOME / "audio_cache",
    _WAYNE_HOME / "video_cache",
    _WAYNE_HOME / "document_cache",
    _WAYNE_HOME / "browser_screenshots",
    ...
    _WAYNE_HOME / "cache" / "images",
    _WAYNE_HOME / "cache" / "audio",
    _WAYNE_HOME / "cache" / "videos",
    _WAYNE_HOME / "cache" / "documents",
    _WAYNE_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Wayne root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* WAYNE_HOME's cache, so a gateway running at
    the root (e.g. ``WAYNE_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when WAYNE_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _WAYNE_ROOT / "profiles"
    try:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Wayne profile and shared Wayne root both contain control
    ...
    # WAYNE_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.wayne
    ...
    for wayne_root in (_WAYNE_HOME, _WAYNE_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(wayne_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Wayne
    secrets (``~/.wayne/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``WAYNE_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Wayne-managed cache, under an operator-allowlisted root
    (``WAYNE_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Wayne root (~/.wayne/.env,
    ...
    # ``MEDIA:~/.wayne/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``WAYNE_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Wayne commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("WAYNE_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "WAYNE_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "WAYNE_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Wayne sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Wayne home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Wayne home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.wayne/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          WAYNE_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          WAYNE_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          WAYNE_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("WAYNE_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("WAYNE_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("WAYNE_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _wayne_run_generation is set on

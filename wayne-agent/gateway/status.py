...
The PID file lives at ``{WAYNE_HOME}/gateway.pid``.  WAYNE_HOME defaults to
``~/.wayne`` but can be overridden via the environment variable.  This means
separate WAYNE_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting WAYNE_HOME."""
    home = get_wayne_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("WAYNE_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "wayne" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Wayne gateway lifecycle subcommand from a command line.
    ...
    (``"C:\\Program Files\\...\\wayne-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Wayne's
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `wayne gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Wayne-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Wayne gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a WAYNE_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.wayne`` or ``$WAYNE_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    explicit ``WAYNE_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"wayne_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit WAYNE_HOME= on the argv. WAYNE_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "wayne_home=" in command_lc and f"wayne_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `wayne status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``WAYNE_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    WAYNE_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different WAYNE_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``wayne.service`` + ``wayne-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different WAYNE_HOME. When two profile gateway
    # services share the same default ~/.wayne (WAYNE_HOME not set
    ...
    # field is absent in markers written by older Wayne versions; treat
    ...
    replacer_home = record.get("replacer_wayne_home")
    if replacer_home is not None and replacer_home != str(get_wayne_home()):
        return False
    ...
    # ``wayne gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_wayne_home": str(get_wayne_home()),

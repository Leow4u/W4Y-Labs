...
Defenses 1 and 2 (the ``_WAYNE_GATEWAY`` guard on ``wayne gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.wayne.gateway")``,
...
State lives in ``<WAYNE_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_wayne_home() / "gateway" / "restart_loop.json"

...
from agent.skill_utils import (
    ...
    get_skill_metadata_namespace as _get_skill_metadata_namespace,
...
# All skills live in ~/.wayne/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from WAYNE_HOME/.env."""
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set WAYNE_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (WAYNE_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("WAYNE_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("WAYNE_SESSION_PLATFORM"))
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.wayne/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``WAYNE_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``WAYNE_PLATFORM`` environment variable
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.wayne/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``wayne skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_wayne_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def skill_view(
    ...
    try:
        ...
        # Check metadata.wayne.* first (agentskills.io convention, with
        # metadata.hermes fallback for upstream ecosystem skills), then top-level
        wayne_meta = _get_skill_metadata_namespace(frontmatter)

        tags = _parse_tags(wayne_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            wayne_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )

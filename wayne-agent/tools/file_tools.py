...
_wayne_config_resolved: str | None = None
_wayne_config_resolved_loaded = False


def _get_wayne_config_resolved() -> str | None:
    """Return the resolved absolute path of the Wayne config file (cached)."""
    global _wayne_config_resolved, _wayne_config_resolved_loaded
    if _wayne_config_resolved_loaded:
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Wayne mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Wayne
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Wayne state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.wayne/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Wayne process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.wayne/…`` path.

    Returns ``None`` when the write is in-scope or outside Wayne scope.
    ...
    # in a session that cd'd into ``~/.wayne/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Wayne internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under WAYNE_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == WAYNE_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Wayne-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Wayne-profile guard for
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with wayne_tools.write_file() for very "
            "large files."
        )

...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("WAYNE_YOLO_MODE", ""))
...
# os.environ["WAYNE_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export WAYNE_INTERACTIVE.
_wayne_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "wayne_interactive",
    default=None,
)


def set_wayne_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["WAYNE_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``WAYNE_INTERACTIVE`` env var for legacy callers.
    """
    return _wayne_interactive_ctx.set("1" if interactive else "")


def reset_wayne_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_wayne_interactive_context`."""
    _wayne_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``WAYNE_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _wayne_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("WAYNE_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("WAYNE_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("WAYNE_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("WAYNE_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set WAYNE_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind WAYNE_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds WAYNE_SESSION_PLATFORM via
    ...
    if env_var_enabled("WAYNE_CRON_SESSION"):
        return False
    if env_var_enabled("WAYNE_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $WAYNE_HOME, or by the resolved absolute
# active profile home path such as /home/wayne/.wayne/config.yaml. The
# resolved-absolute form is folded into the ~/.wayne/ patterns at detection
...
DANGEROUS_PATTERNS = [
    ...
    # that should always require user consent, just like `wayne gateway restart`
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Wayne file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.wayne/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Wayne-managed security file (~/.wayne/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.wayne/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / WAYNE_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Wayne home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\wayne), so folding the user home
    # first would eat the prefix the Wayne-home fold needs.
    command = _rewrite_resolved_wayne_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / WAYNE_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.wayne``); the matched
...
def _rewrite_resolved_wayne_home(command: str) -> str:
    """Rewrite the resolved absolute Wayne home prefix to ``~/.wayne/``.

    Resolves the active ``WAYNE_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.wayne/`` so the static ``_WAYNE_CONFIG_PATH`` / ``_WAYNE_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/wayne/.wayne/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$WAYNE_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.wayne")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["WAYNE_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "WAYNE_SPINNER_PAUSE" in os.environ:
            del os.environ["WAYNE_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Wayne approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``WAYNE_YOLO_MODE`` (frozen at import time
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_gateway or env_var_enabled("WAYNE_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("WAYNE_EXEC_ASK")

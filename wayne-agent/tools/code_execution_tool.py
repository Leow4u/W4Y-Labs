...
Lets the LLM write a Python script that calls Wayne tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `wayne_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `wayne_tools.py` with file-based RPC stubs
...
# available on every platform Wayne itself runs on.
...
# match a safe prefix, the operational WAYNE_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "WAYNE_" prefix was deliberately removed (#27303) — it leaked
# WAYNE_*-named config that lacks a secret substring (e.g. WAYNE_BASE_URL,
# WAYNE_KANBAN_DB, WAYNE_*_WEBHOOK).  The child only needs the few
# location/profile vars in _WAYNE_CHILD_ALLOWED below; WAYNE_RPC_SOCKET /
# WAYNE_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational WAYNE_* vars (_WAYNE_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret WAYNE_* vars dropped by the tightened allowlist (#27303). The
    # broad "WAYNE_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like WAYNE_KANBAN_DB / WAYNE_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _WAYNE_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("WAYNE_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped WAYNE_* var.
            _dropped_wayne.append(k)
    if _dropped_wayne:
        logger.debug(
            "execute_code: dropped %d non-allowlisted WAYNE_* var(s) from the "
...
def generate_wayne_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the wayne_tools.py stub module.
...
def _connect():
    ...
    WAYNE_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["WAYNE_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("WAYNE_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
def _execute_remote(
    ...
    The script and the generated wayne_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/wayne_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_wayne_tools_module(
...
def execute_code(
    ...
    # --- Set up temp directory with wayne_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="wayne_sandbox_")
    ...
    # selector from WAYNE_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"wayne_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated wayne_tools module.
        ...
        tools_src = generate_wayne_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "wayne_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  WAYNE_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # Ensure the wayne-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from wayne_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _wayne_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _wayne_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # WAYNE_TIMEZONE is an internal Wayne setting and must not leak
        # into child processes.
        _tz_name = os.getenv("WAYNE_TIMEZONE", "").strip()
        if _tz_name:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.wayne/.env')).
...
def _load_config() -> dict:
    ...
    ``wayne --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (wayne-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``wayne tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and wayne-agent's own python.
    if mode == "strict":
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Work4You tools programmatically. "
        ...
        f"Available via `from wayne_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into wayne_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from wayne_tools import {import_str}` "

...
-----------------------------------------------------------------------------
    ...
    ~/.wayne/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/wayne/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_wayne_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/wayne"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Wayne convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("WAYNE_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Wayne infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``wayne checkpoints clear-legacy``.
...
# look for ``HEAD``/``info/exclude``/``WAYNE_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at WAYNE_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "WAYNE_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `wayne checkpoints` CLI
# ---------------------------------------------------------------------------

...
This module provides the glue:
    - ``WayneTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Wayne Agent"
"""
...
_USER_SKIPPED_SENTINEL = "__wayne_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses WAYNE_HOME so each profile gets its own OAuth tokens.
...
# ---------------------------------------------------------------------------
# WayneTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class WayneTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        WAYNE_HOME/mcp-tokens/<server_name>.json         -- tokens
        WAYNE_HOME/mcp-tokens/<server_name>.client.json   -- client info
        WAYNE_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Wayne records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Wayne.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Wayne Agent")
    scope = cfg.get("scope")

...
def auth_json_path():
    """Return the Wayne auth store path, respecting WAYNE_HOME overrides."""
    return get_wayne_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`wayne tools`, banner/status paint, provider

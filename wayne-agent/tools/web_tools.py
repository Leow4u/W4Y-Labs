...
Backend is selected during ``wayne tools`` setup (web.backend in config.yaml).
When available, Wayne can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Wayne config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Wayne' config/.env layer (``wayne config set``, ``wayne tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.wayne/config.yaml."""
    try:
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `wayne tools` repaint.
        try:
...
def _ddgs_package_importable() -> bool:
    ...
    Import-only — never prompts or installs during availability probes
    (banner, ``wayne tools``, tool gating). Lazy install runs on first
    ``web_search`` use via the ddgs plugin / ``wayne tools`` post-setup.
    """
    try:
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_wayne_dir("cache/web", "web_cache")

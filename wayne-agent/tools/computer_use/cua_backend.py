...
etc.) surface as specific blocked checks via `wayne computer-use doctor`
...
# fetches the latest release, so a `WAYNE_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `WAYNE_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("WAYNE_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when Wayne should disable cua-driver telemetry for this user.
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or WAYNE_CUA_DRIVER_CMD resolves."""
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Wayne-managed secrets out of the child env.
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        #   - Gets a distinct agent-cursor color per Wayne run, with
        ...
        #     concurrent Wayne runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Wayne run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"wayne-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Wayne' minimal core.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps wayne `capture(mode, app)` → cua-driver `list_windows` +

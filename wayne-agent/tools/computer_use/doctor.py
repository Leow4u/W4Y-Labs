"""
`wayne computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Wayne side — the only
...
def run_doctor(
    ...
    *,
    ...
    Honors `WAYNE_CUA_DRIVER_CMD` via the same `_cua_driver_cmd()` resolver

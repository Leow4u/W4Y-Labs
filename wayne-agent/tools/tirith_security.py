...
it is automatically downloaded from GitHub releases to $WAYNE_HOME/bin/tirith.
...
def _get_wayne_home() -> str:
    """Return the Wayne home directory, respecting WAYNE_HOME env var."""
    return str(get_wayne_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_wayne_home(), ".tirith-install-failed")
...
def _wayne_bin_dir() -> str:
    """Return $WAYNE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_wayne_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $WAYNE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_wayne_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $WAYNE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $WAYNE_HOME/bin/tirith
    ...
    wayne_bin = os.path.join(_wayne_bin_dir(), "tirith")
    if os.path.isfile(wayne_bin) and os.access(wayne_bin, os.X_OK):
        _resolved_path = wayne_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return wayne_bin

...
One ``CDPSupervisor`` runs per Wayne ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Wayne logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "wayne-dialog-bridge.invalid"
...
(() => {
  if (window.__wayneDialogBridgeInstalled) return;
  window.__wayneDialogBridgeInstalled = true;
  const ENDPOINT = "http://wayne-dialog-bridge.invalid/";

...
def _is_wayne_provider_credential(name: str) -> bool:
    """True if ``name`` is a Wayne-managed provider credential (API key,
    token, or similar) per ``_WAYNE_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Wayne API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Wayne credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Wayne-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_wayne_internal_secret(name):
        return True
    return name in _WAYNE_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Wayne-managed provider credentials (from
    ``_WAYNE_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Wayne-managed
    ...
    Non-Wayne third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Wayne-managed provider credentials must not be passed

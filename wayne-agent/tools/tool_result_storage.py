...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/wayne-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/wayne-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/wayne-results"
HEREDOC_MARKER = "WAYNE_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/wayne-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"WAYNE_PERSIST_{uuid.uuid4().hex[:8]}"

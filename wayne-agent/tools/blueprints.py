...
`wayne skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.wayne.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.wayne.blueprint`` (or the upstream
    ``metadata.hermes.blueprint`` spelling used by agentskills.io ecosystem
    skills) is a mapping containing a non-empty ``schedule``. Raises
    BlueprintError if the block exists but is structurally invalid (so a typo
    ...
    from agent.skill_utils import get_skill_metadata_namespace

    wayne = get_skill_metadata_namespace(fm)
    blueprint = wayne.get("blueprint") if isinstance(wayne, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.wayne.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.wayne.blueprint`` block) they can hand
    to ``wayne skills publish`` to share. ``body`` is the plain-language

...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``wayne tools`` repaint, tool-registration scans,
    ...
    2. ``~/.wayne/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Wayne home, then mark it seen."""

#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Wayne Skills Hub.
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _wayne_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        tags = get_skill_metadata_namespace(fm).get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        tags: List[str] = []
        raw_tags = get_skill_metadata_namespace(fm).get("tags", [])
...
# ---------------------------------------------------------------------------
# Wayne centralized index source
# ---------------------------------------------------------------------------

WAYNE_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
WAYNE_INDEX_TTL = 6 * 3600  # 6 hours


def _wayne_index_cache_file() -> Path:
    return _index_cache_dir() / "wayne-index.json"


def _load_wayne_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for WAYNE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    wayne_index_cache_file = _wayne_index_cache_file()
    if wayne_index_cache_file.exists():
        try:
            age = time.time() - wayne_index_cache_file.stat().st_mtime
            if age < WAYNE_INDEX_TTL:
                return json.loads(wayne_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(WAYNE_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Wayne index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Wayne index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        wayne_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        wayne_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class WayneIndexSource(SkillSource):
    """Skill source backed by the centralized Wayne Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "wayne-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "wayne-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        WayneIndexSource(auth=auth), # Centralized index (search + resolved install paths)

...
`mcp_tool.py`, and `wayne_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# WayneMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_wayne_provider_class() -> Optional[type]:
    ...
    class WayneMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._wayne_server_name = server_name
            ...
            self._wayne_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`WayneTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, WayneTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._wayne_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import WayneTokenStorage
                        if isinstance(storage, WayneTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._wayne_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import WayneTokenStorage
            if not isinstance(storage, WayneTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``wayne mcp reauth``.
            ...
            back to ``wayne mcp reauth``.
            """
            try:
                if self._wayne_preregistered:
                    return
    ...
    return WayneMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_WAYNE_PROVIDER_CLS: Optional[type] = _make_wayne_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`WayneMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _WAYNE_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = WayneTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `wayne mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``wayne mcp remove <name>`` and (indirectly) by
        ``wayne mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:

...
provider** — turn on a backend by enabling a plugin (``wayne plugins
enable video_gen/<name>``) and selecting it in ``wayne tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/wayne-agent-dev/references/dynamic-tool-schemas.md.
...
# `wayne tools` or `/skills`, the schema rebuilds automatically.

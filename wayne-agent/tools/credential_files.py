...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.wayne",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``WAYNE_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside WAYNE_HOME so that a malicious
    ...
    host_path = wayne_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape WAYNE_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, wayne_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.wayne/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="wayne-skills-safe-"))
...
def get_cache_directory_mounts(
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_wayne_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.wayne``,
    SSH ``<remote_home>/.wayne``, etc.) and whether translation is wanted.

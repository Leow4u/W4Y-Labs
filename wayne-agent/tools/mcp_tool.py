...
transport, discovers their tools, and registers them into the wayne-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.wayne/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.wayne/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            wayne_home = os.path.expanduser(
                os.getenv(
                    "WAYNE_HOME", os.path.join(os.path.expanduser("~"), ".wayne")
                )
            )
            candidates = [
                os.path.join(wayne_home, "node", "bin", resolved_command),
                ...
                # image (which the Wayne Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Wayne MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Wayne gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Wayne' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (WAYNE_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit WAYNE_SESSION_PLATFORM, so the
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Wayne' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Wayne is fully
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local WAYNE_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_wayne_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_wayne_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local WAYNE_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_wayne_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Wayne config file.
    ...
    try:
        ...
        if _env_enabled("WAYNE_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Wayne' MEDIA: tag convention and out to messaging
            ...
            # Wayne' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Wayne's historical behavior of converting hyphens to
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``wayne tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Wayne registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple wayne

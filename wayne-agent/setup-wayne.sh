...
# 5. Symlinks the 'wayne' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Wayne Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/wayne-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink wayne into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up wayne command..."

WAYNE_BIN="$SCRIPT_DIR/venv/bin/wayne"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$WAYNE_BIN" "$COMMAND_LINK_DIR/wayne"
echo -e "${GREEN}✓${NC} Symlinked wayne → $COMMAND_LINK_DISPLAY_DIR/wayne"
...
# ============================================================================
# Seed bundled skills into ~/.wayne/skills/
# ============================================================================

WAYNE_SKILLS_DIR="${WAYNE_HOME:-$HOME/.wayne}/skills"
mkdir -p "$WAYNE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.wayne/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$WAYNE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  wayne status        # Check configuration"
if is_termux; then
    echo "  wayne gateway       # Run gateway in foreground"
else
    echo "  wayne gateway install # Install gateway service (messaging + cron)"
fi
echo "  wayne cron list     # View scheduled jobs"
echo "  wayne doctor        # Diagnose issues"
echo ""

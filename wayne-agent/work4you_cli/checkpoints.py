"""`wayne checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.wayne/checkpoints/``.  Actions:

    wayne checkpoints               # same as `status`
    wayne checkpoints status        # total size, project count, breakdown
    wayne checkpoints list          # per-project checkpoint counts + workdir
    wayne checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    wayne checkpoints clear [-f]    # nuke the entire base (asks first)
    wayne checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    wayne checkpoints
    wayne checkpoints prune --retention-days 3 --max-size-mb 200
    wayne checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: wayne checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``wayne checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `wayne checkpoints` → status

"""
Wayne Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m wayne_cli.main web          # Start on http://127.0.0.1:9119
    python -m wayne_cli.main web --port 8080
"""
...
from wayne_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from wayne_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
try:
    ...
    # running `wayne dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``wayne gateway run`` — but the
    desktop app spawns a ``wayne dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same WAYNE_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import wayne_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from wayne_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire wayne_cli.gateway import into a background thread so the event
    # loop is not blocked and WAYNE_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (WAYNE_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `wayne
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("WAYNE_DESKTOP") == "1":
...
# WAYNE_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("WAYNE_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Wayne-Session-Token"
...
#
# This list is defined in ``wayne_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from wayne_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Wayne-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``wayne-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    from wayne_cli.platform_tenant import (
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from wayne_cli.dashboard_auth.token_auth import token_auth_middleware
...
class ModelAssignment(BaseModel):
    ...
    # ``wayne model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Wayne provider slug — fine.
    ...
    1. Vendor-name → Wayne-provider mapping: when the provider string is
       not a known Wayne provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from wayne_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from wayne_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "WAYNE_DASHBOARD_FILES_ROOT"
...
def _default_wayne_root_is_opt_data() -> bool:
    raw = os.environ.get("WAYNE_HOME", "").strip()
    if not raw:
        return False
    try:
        from wayne_constants import get_default_wayne_root

        root = get_default_wayne_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``wayne update``.
    ...
    a container — e.g. the wayne-webui image sharing the Wayne source tree),
    the dashboard's ``wayne update`` button is the correct update path and
    ...
    """
    if _default_wayne_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Wayne root is actually /opt/data
    # (the container/hosted layout) or when WAYNE_DASHBOARD_FILES_ROOT is set.
    if _default_wayne_root_is_opt_data():
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``wayne:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.wayne-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``wayne_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from wayne_cli import web_git as _web_git  # noqa: E402
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_wayne_home() at call time
    ...
    try:
        ...
        # Windows install the first import of wayne_cli.gateway blocks the
        ...
        # and which providers are registered so ``wayne status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from wayne_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_wayne": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the wayne home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_wayne_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_wayne_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from wayne_cli.auth import get_nous_auth_status
...
async def run_prompt_size():
    try:
        proc = _spawn_wayne_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_wayne_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_wayne_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
class DebugShareRequest(BaseModel):
    ...
    # out of log content before it lands in the bundle files (which may later
    # be shared by hand). The toggle exists so an operator who knows the logs
    # are clean can opt out for fuller fidelity.
    redact: bool = True
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    """Write a redacted debug report + full logs locally and return the paths.

    W4Y fork: remote upload is disabled — ``build_debug_share`` writes the
    bundle under ``~/.wayne/debug-shares/`` and ``urls`` carries local file
    paths instead of paste URLs (the field name is kept for dashboard
    compatibility). Unlike the other diagnostics actions (doctor, dump,
    prompt-size) this is *synchronous*: we run the write in a worker thread
    and return the structured ``{urls, failures, redacted, ...}`` payload
    directly.
    """
    from wayne_cli.debug import build_debug_share
    ...
    try:
        ...
        # Required summary-report write failed (disk full / permissions).
        raise HTTPException(status_code=502, detail=f"Debug bundle write failed: {exc}")
    except Exception as exc:
...
# streamed to a per-action log file under ``~/.wayne/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_wayne_home() / "logs"
...
def _spawn_wayne_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``wayne <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``wayne_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "wayne_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "WAYNE_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["wayne", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``wayne gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``wayne gateway restart`` children race each other on the
...
async def gateway_drain(request: Request):
    ...
    active (no ``WAYNE_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/wayne/update")
async def update_wayne():
    ...
    if _dashboard_local_update_managed_externally():
        ...
        _record_completed_action("wayne-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "wayne-update",
    ...
    try:
        proc = _spawn_wayne_action(["update"], "wayne-update")
    except Exception as exc:
        _log.exception("Failed to spawn wayne update")
...
@app.get("/api/wayne/update/check")
async def check_wayne_update(force: bool = False):
    """Report whether a Wayne update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/wayne/update`` actually runs ``wayne update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Wayne version string
    ...
    try:
        ...
        if force:
            try:
                (get_wayne_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def get_sessions(
    ...
    try:
        ...
        from wayne_cli.platform_tenant import TenantScopeRequiredError
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Wayne surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Wayne supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_wayne_home() / provider.name / "config.json"
...
# in wayne_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `wayne model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, request: Request, profile: Optional[str] = None):
    ...
    try:
        ...
        if model:
            ...
            if cookie:
                from wayne_cli.plan_model_gating import (
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (wayne_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from wayne_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``wayne model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from wayne_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``wayne model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from wayne_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from wayne_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `wayne model` provider — would be
        ...
        # Vertex — a `wayne model` provider — would be invisible in the desktop
...
_TELEGRAM_ONBOARDING_USER_AGENT = f"WayneDashboard/{__version__}"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Wayne appears
...
def _get_messaging_platforms_sync(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    # all resolve against the requested profile's WAYNE_HOME.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.wayne/.anthropic_oauth.json`` — Wayne-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_wayne_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_wayne_oauth_credentials,
            _WAYNE_OAUTH_FILE,
        )
    except ImportError:
        read_wayne_oauth_credentials = None  # type: ignore
        _WAYNE_OAUTH_FILE = None  # type: ignore

    wayne_creds = None
    if read_wayne_oauth_credentials:
        try:
            wayne_creds = read_wayne_oauth_credentials()
        except Exception:
            wayne_creds = None
    if wayne_creds and wayne_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "wayne_pkce",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Wayne even
    when they also have a separate Wayne-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Wayne never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from wayne_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Wayne, so the disconnect
    ...
    user sees exactly what executes, and Wayne then stops resolving the token.
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``wayne model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `wayne model` universe (zero-edit for new plugins).
    try:
        from wayne_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("wayne_pkce", "claude_code", ...)
    ...
    sync with the `wayne model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Wayne-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _WAYNE_OAUTH_FILE
                if _WAYNE_OAUTH_FILE.exists():
                    _WAYNE_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from wayne_cli.auth import clear_provider_auth
        ...
        try:
            from wayne_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.wayne/.anthropic_oauth.json AND credential pool
...
# Guarded so wayne web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Wayne file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``wayne auth add anthropic``.
    """
    from agent.anthropic_adapter import _WAYNE_OAUTH_FILE
    payload = {
    ...
    _WAYNE_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _WAYNE_OAUTH_FILE.with_name(
        f"{_WAYNE_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _WAYNE_OAUTH_FILE)
        try:
            _WAYNE_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "wayne-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("WAYNE_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from wayne_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from wayne_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``wayne auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``wayne model`` re-login path in _login_xai_oauth).
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from wayne_cli.auth import _save_codex_tokens
...
def _session_latest_descendant(session_id: str):
    ...
    """
    from wayne_cli.platform_tenant import open_session_db
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `wayne sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `wayne sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_wayne_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from wayne_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_wayne_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from wayne_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, WAYNE_HOME)."""
    from wayne_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process WAYNE_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (wayne_cli.mcp_config), so
# servers managed here show up under `wayne mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from wayne_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from wayne_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from wayne_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from wayne_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from wayne_cli.mcp_config import (
        _get_mcp_servers,
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the WAYNE_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _run():
        from tools.mcp_oauth import WayneTokenStorage, force_interactive_oauth
        ...
        # resolve WAYNE_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = WayneTokenStorage(name)
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds WAYNE_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (wayne_cli.webhook); the webhook
...
async def list_webhooks():
    import wayne_cli.webhook as wh
...
# spawn the real `wayne gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_wayne_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_wayne_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from wayne_cli.auth import read_credential_pool
...
async def run_doctor():
    try:
        proc = _spawn_wayne_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_wayne_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_wayne_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"wayne-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_wayne_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `wayne import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_wayne_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    try:
        from wayne_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <wayne_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_wayne_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_wayne_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``wayne`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_wayne_action`` tracks one process/log per name, so a shared
...
# Human-readable labels for each hub source id (matches `wayne skills search`
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from wayne_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`wayne -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the WAYNE_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_wayne_home()
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local WAYNE_HOME override so the write lands in the target
    ...
    token = set_wayne_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local WAYNE_HOME override (same mechanism as
    ...
    """
    from wayne_constants import set_wayne_home_override, reset_wayne_home_override
    from wayne_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the WAYNE_HOME override. Returns the
    number of skills newly disabled.
    """
    from wayne_constants import set_wayne_home_override, reset_wayne_home_override
    from wayne_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's WAYNE_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``wayne profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from WAYNE_HOME).
    """
    from wayne_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``wayne profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the WAYNE_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``wayne profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from wayne_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_wayne_home()`` at call
       time — the context-local override from ``set_wayne_home_override``
    ...
    retargeted to the *current* ``get_wayne_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a WAYNE_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_wayne_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_wayne_home()`` at call time
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``wayne tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``wayne tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `wayne tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``wayne tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.wayne/.env`` via ``save_env_value`` —
    the same store ``wayne tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``wayne tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``wayne -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same WAYNE_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Wayne,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``wayne computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``wayne --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from wayne_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from wayne_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str, Optional[dict]]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from wayne_cli.dashboard_auth.audit import AuditEvent, audit_log
        from wayne_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``wayne --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``WAYNE_TUI_RESUME`` env var —
    matching what ``wayne_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``WAYNE_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``WAYNE_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``WAYNE_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``WAYNE_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_wayne_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``wayne -p <name>`` performs. The in-process
    ``WAYNE_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from wayne_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if sidecar_url:
        env["WAYNE_TUI_SIDECAR_URL"] = sidecar_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from wayne_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="wayne-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Wayne Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Wayne CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="wayne-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_wayne_root_is_opt_data() else "local"
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from wayne_cli.console_engine import WayneConsoleEngine

        engine = WayneConsoleEngine(
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if ticket_info:
        ...
        if tenant_id:
            from wayne_cli.platform_tenant import ensure_tenant_home, tenant_home_path

            ensure_tenant_home(tenant_id)
            tenant_home = str(tenant_home_path(tenant_id))

    from wayne_cli.platform_tenant import shared_motor_enabled
...
# WAYNE_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`wayne_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from wayne_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/wayne/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /wayne`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__WAYNE_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/wayne``)
        ...
        ``__WAYNE_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__WAYNE_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__WAYNE_BASE_PATH__="{prefix}";'
                f"window.__WAYNE_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__WAYNE_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__WAYNE_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__WAYNE_BASE_PATH__="{prefix}";'
                f"window.__WAYNE_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/wayne`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Wayne backend. Intercept CSS asset
...
# W4Y 2026-07-04: theme set reduced to four (white is the platform default).
# Must stay in sync with web/src/themes/presets.ts BUILTIN_THEMES.
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "white",     "label": "White",     "description": "Modo claro — branco limpo e neutro (padrão)"},
    {"name": "mono",      "label": "Black",     "description": "Preto minimalista — grayscale escuro e focado"},
    {"name": "cyberpunk", "label": "Cyberpunk", "description": "Verde neon no preto — terminal matrix"},
    {"name": "rose",      "label": "Rosé",      "description": "Rosa suave e marfim quente — leve para os olhos"},
]
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.wayne/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.wayne/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_wayne_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.wayne/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as wayne_cli.plugins):
    1. User plugins:    ~/.wayne/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.wayne/plugins/  (only if WAYNE_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from wayne_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_wayne_home() / "plugins", "user"),
    ...
    # ``wayne_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("WAYNE_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".wayne" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from wayne_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_wayne_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"wayne auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from wayne_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from wayne_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from wayne_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.wayne/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"wayne_dashboard_plugin_{plugin['name']}"
...
from wayne_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``WAYNE_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("WAYNE_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    # the wayne-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing WAYNE_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Wayne Agent exposes a
    ...
    # WAYNE_DASHBOARD_READY, open the browser, *then* serve.
    #

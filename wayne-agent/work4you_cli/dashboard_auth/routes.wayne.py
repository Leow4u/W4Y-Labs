...
from wayne_cli.dashboard_auth.base import (
    InvalidCodeError,
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``WAYNE_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /wayne`` (Mission Control deploys) — we
    ...
    from wayne_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``wayne_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("wayne_session_pkce", "")
...
async def auth_platform_sso(request: Request, ticket: str = ""):
    ...
    from wayne_cli.dashboard_auth.platform_sso import verify_platform_sso_claims
    from wayne_cli.platform_tenant import ensure_tenant_home, shared_motor_enabled
...
async def api_auth_ws_ticket(request: Request):
    ...
    from wayne_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket

...
from wayne_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /wayne``, the
    ``login_url`` is prefixed (``/wayne/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    # Password-capable providers (e.g. the bundled ``basic`` provider) have no
    # OAuth ``start_login`` redirect — auto-SSO would bounce to /auth/login and
    # 500 with NotImplementedError. Fall through to the ``/login`` form page,
    # which renders the username/password form for them. (W4Y fork fix.)
    ...
    from wayne_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["wayne.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``wayne_session_rt``. If we bailed on ``not at`` here we'd
...
def _attach_refreshed_session_cookies(
    ...
    from wayne_cli.dashboard_auth.cookies import detect_https, set_session_cookies
    from wayne_cli.dashboard_auth.prefix import prefix_from_request

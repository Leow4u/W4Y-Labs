...
Three cookies in play:
  ...
  - wayne_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/wayne) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/wayne`` covers
...
setter wrote ``__Secure-wayne_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "wayne_session_at"
SESSION_RT_COOKIE = "wayne_session_rt"
PKCE_COOKIE = "wayne_session_pkce"
...
SSO_ATTEMPT_COOKIE = "wayne_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /wayne`` we want ``Path=/wayne`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (wayne #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/wayne``)
...
def clear_session_cookies(response: Response, *, prefix: str = "", use_https: bool = True) -> None:
    ...
    """
    path = _cookie_path(prefix)
    attrs = dict(path=path, httponly=True, samesite="lax")
    if use_https:
        attrs["secure"] = True
    for variant in _NAME_VARIANTS:
        name_at = f"{variant}{SESSION_AT_COOKIE}"
        name_rt = f"{variant}{SESSION_RT_COOKIE}"
        needs_secure = variant != "" or use_https
        del_attrs = {**attrs, **({"secure": True} if needs_secure else {})}
        response.set_cookie(name_at, "", max_age=0, **del_attrs)
        response.set_cookie(name_rt, "", max_age=0, **del_attrs)

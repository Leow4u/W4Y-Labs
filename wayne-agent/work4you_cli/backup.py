"""
Backup and import commands for wayne CLI.

`wayne backup` creates a zip archive of the entire ~/.wayne/ directory
(excluding the wayne-agent repo and transient files).

`wayne import` restores from a backup zip, overlaying onto the current
WAYNE_HOME root.
"""
...
from work4you_constants import get_default_wayne_root, get_wayne_home, display_wayne_home
...
# ``wayne-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/wayne-agent/``
...
# WAYNE_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under WAYNE_HOME) —
...
# File names that ``wayne import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE WAYNE_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside WAYNE_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to wayne root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``wayne-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/wayne-agent/`` — must be preserved.
        if part == "wayne-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside WAYNE_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Wayne home directory."""
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"wayne-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"wayne-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_wayne_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(wayne_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(wayne_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``wayne-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "wayne-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(wayne_root)
    ...
    # outside WAYNE_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_wayne_home()}."
        )
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Wayne backup.
    ...
    # Look for telltale files that a wayne home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.wayne/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a wayne dir name
        if prefix in {".wayne", "wayne"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_wayne_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (wayne_root / "config.yaml").exists()
        has_env = (wayne_root / ".env").exists()
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under WAYNE_HOME.
            ...
            target = wayne_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(wayne_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_wayne_home()}."
            )
        ...
        profiles_dir = wayne_root / "profiles"
        restored_profiles = []
...
# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and wayne backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to WAYNE_HOME).
...
# — `wayne update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $WAYNE_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside WAYNE_HOME and the entry is silently
...
def _quick_snapshot_root(wayne_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(wayne_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside WAYNE_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``wayne update``.
    ...
    Args:
        ...
        wayne_home: Override for the Wayne home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(wayne_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for wayne backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_wayne_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, wayne_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``wayne_root`` to ``out_path``.
...
def _pre_update_backup_dir(wayne_home: Optional[Path] = None) -> Path:
    home = wayne_home or get_wayne_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of WAYNE_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<WAYNE_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``wayne update``) should continue even if the backup fails.
    """
    wayne_root = wayne_home or get_default_wayne_root()
    if not wayne_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(wayne_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, wayne_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `wayne claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of WAYNE_HOME under ``backups/`` before a
    ``wayne claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``wayne import <archive>``.  Writes to
    ``<WAYNE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `wayne import` and the

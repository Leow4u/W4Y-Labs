...
from work4you_cli import restart_jobs
...
from work4you_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _warm_gateway_module() -> None:
    try:
        import work4you_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire work4you_cli.gateway import into a background thread so the event
...
def _shared_motor_home() -> Optional[Path]:
    """Context-local tenant home when serving a shared-motor request.

    ``platform_tenant_request_scope`` pins this per request from
    ``session.org_id``. Returns None off the shared motor, where one process
    serves exactly one tenant and the process-wide home IS the tenant's.
    """
    try:
    ...
    if not shared_motor_enabled():
        return None
    override = get_wayne_home_override()
    return Path(override) if override else None


def _shared_motor_tenant() -> str:
    """Tenant id of the request in flight, or "" when not on a shared motor.

    Derived from the pinned home rather than from ``FLY_APP_NAME`` so it cannot
    disagree with the storage this same request reads and writes —
    ``_connector_tenant_id`` returns the APP name, which is identical for every
    tenant once one app serves them all.
    """
    home = _shared_motor_home()
    if home is None:
        return ""
    return re.sub(r"[^a-z0-9_-]+", "-", home.name.strip().lower())


def _connector_base_home() -> Path:
    """WAYNE_HOME that owns this request's connector state.

    On a shared motor the tenant's own home holds its ``.env`` and
    ``config.yaml``. Reading the process-wide root there would hand every
    tenant the same key and write the ``mcp_servers`` entry into a config the
    tenant's agent never loads — the agent runs with the tenant home pinned
    (see ``gateway_ws`` / ``_resolve_chat_argv``), so the connector would show
    as attached while exposing no tools.

    Off the shared motor this stays the pre-profile root, which is what the
    "global" scope has always meant.
    """
    home = _shared_motor_home()
    if home is not None:
        return home
...
def _connector_event_scope(uid: str) -> Optional[Tuple[str, str]]:
    """Mapeia o ``user_id`` de um evento para ``(tenant, escopo)``.

    Devolve ``None`` se o uid não é de uma das nossas sessões — uids estranhos
    (e-mails, formatos multi-tenant crus da Composio) não devem gerar tasks.

    ``tenant`` vem vazio quando o uid é nu ("global" / "<profile>"), que é o
    formato de um projeto dedicado e do desktop. Quando vem prefixado
    ("<tenant>:<escopo>" — o formato do shared motor, ver
    ``_connector_user_id``) é o prefixo que diz A QUEM o evento pertence, já
    que esta rota bypassa o gate de auth e portanto não tem sessão.
    """
    uid = (uid or "").strip()
    if not uid:
        return None
    tenant = ""
    if ":" in uid:
        prefix, rest = uid.split(":", 1)
        # Legado da opção B: o prefixo era o nome do app, não o id do tenant.
        # Trata-se igual — o que importa é isolar o escopo do sufixo.
        if re.fullmatch(r"[a-z0-9_-]{1,64}", prefix or ""):
            tenant, uid = prefix, rest
        else:
            return None
...
@contextmanager
def _connector_event_home(tenant: str):
    """Pin the owning tenant's home while an inbound event is handled.

    The events route bypasses the dashboard auth gate on purpose (authenticity
    is the webhook HMAC), so no request scope was ever established. On a shared
    motor that leaves the kanban task and the cron lookup pointing at the
    process-wide home instead of the tenant's — the event would land in the
    wrong place, or in nobody's. The tenant comes from the ``user_id`` prefix.

    No-op off the shared motor, where the process serves exactly one tenant.
    """
    from work4you_cli.platform_tenant import shared_motor_enabled

    if not tenant or not shared_motor_enabled():
        yield
        return

    from work4you_cli.platform_tenant import activate_platform_tenant_scope
    from work4you_constants import reset_wayne_home_override

    token = activate_platform_tenant_scope(tenant)
    try:
        yield
    finally:
...
def _connector_homes(scope: str) -> List[Path]:
    ...
    # Shared motor: o home do tenant é a única config que lhe pertence. O
    # fan-out por profiles não se aplica — `list_profiles` é ancorado no HOME do
    # container, logo listaria profiles que não são deste tenant.
    tenant_home = _shared_motor_home()
    if tenant_home is not None:
        return [tenant_home]

    if scope == "global":
        homes = [_connector_base_home()]
        try:
...
def _write_connector_entry(homes: List[Path], entry_name: str, url: str) -> int:
    ...
    from work4you_cli.config import load_config as _load_cfg, save_config as _save_cfg
...
async def connectors_attach(body: ConnectorAttach):
    ...
    def _rediscover() -> None:
        try:
            from work4you_cli.mcp_startup import restart_mcp_discovery
...
async def device_connector_bootstrap():
    ...
    * ``user_id`` — "global", so the device sees the same connected accounts.
    ...
    def _run():
        ...
        # The device must ask Composio with the SAME user_id this tenant uses,
        # or it sees an empty account list (prefixed on a shared motor).
        return {
            "composio_key": key,
            "mcp_url": url,
            "user_id": _connector_user_id("global"),
        }
...
async def connector_events_receiver(token: str, request: Request):
    ...
    def _kanban_task(title: str, body_lines: List[str], assignee: Optional[str], event_id: str):
        try:
            from work4you_cli import kanban_db  # type: ignore
        except Exception:
    ...
    def _run():
        ...
        if etype == "composio.connected_account.expired":
            ...
            if acc_id:
                try:
                    ...
                    uid = ""
            resolved = _connector_event_scope(uid)
            if resolved is None:
            ...
            with _connector_event_home(tenant):
                return _kanban_task(
        ...
        # The cron store and the kanban board both resolve from WAYNE_HOME, so
        # everything below has to run inside the owning tenant's home.
        with _connector_event_home(tenant):
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            from work4you_cli.dashboard_auth import list_providers as _list_providers
...
def _restart_home_for(profile: Optional[str]) -> Optional[Path]:
    ...
    try:
        from work4you_cli.profiles import _get_default_wayne_home as _default_home
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nous":
            try:
                from work4you_cli.nous_subscription import apply_nous_managed_defaults
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        try:
            from work4you_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from work4you_cli.auth import persist_nous_credentials
...
# ---------------------------------------------------------------------------
# Projects (first-class, multi-folder) — REST bridge over work4you_cli.projects_db
#
...
def get_profiles_pulse():
    from work4you_cli import profiles as profiles_mod
    from work4you_cli import budget as budget_mod
...
async def knowledge_list(profile: Optional[str] = None):
    from work4you_cli import knowledge as knowledge_mod
...
def _resolve_chat_argv(
    ...
    """
    from work4you_cli.main import PROJECT_ROOT, _make_tui_argv
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from work4you_cli.console_engine import WayneConsoleEngine

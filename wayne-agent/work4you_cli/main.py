...
from work4you_cli.subcommands.postinstall import build_postinstall_parser
...
def _has_any_provider_configured() -> bool:
    ...
    from work4you_cli.config import get_env_path, get_wayne_home, load_config
...
def cmd_chat(args):
    ...
    # work4you_cli/plugins.py and tools/mcp_tool.py).
...
def cmd_whatsapp(args):
    ...
    from work4you_constants import find_node_executable, with_wayne_node_path
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``wayne`` launcher present but ``work4you_cli`` not importable.
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # work4you_cli.config.get_wayne_home to simulate setup failure.
        from work4you_cli.config import get_wayne_home as _get_wayne_home
...
def _run_pre_update_backup(args) -> None:
    ...
    # ``work4you_cli/config.py``). Fall back to false if the key is missing
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_wayne_home added to work4you_constants).
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "work4you_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                ...
                if tail:
                    print(tail)
                from work4you_constants import display_wayne_home as _dhh
        ...
        # modules in this process.  Reload work4you_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_wayne_home() added since the last release.
        try:
            import importlib
            import work4you_constants as _hc
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from work4you_constants import get_wayne_home as _hh
        ...
        from work4you_cli.profiles import rename_profile
...
def main():
    ...
    try:
        from work4you_cli.home_migration import maybe_migrate_home
    ...
    # =========================================================================
    # postinstall command  (parser built in work4you_cli/subcommands/postinstall.py)
    # =========================================================================

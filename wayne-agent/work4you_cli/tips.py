...
TIPS = [
    ...
    "Checkpoints use shadow repos in {home}/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in {home}/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at {home}/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to {home}/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from {home}/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at {home}/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads {home}/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under {home}/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a {home}/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    'Drop a YAML in {home}/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in {home}/dashboard-plugins/ — no npm build required.',
    ...
    'WORK4YOU_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to {home}/cache/piper-voices/.',
    ...
    '/reload-skills re-scans {home}/skills/ so drop-in skills appear without restarting the session.',
    ...
    'WORK4YOU_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'WORK4YOU_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.wayne/plugins/ — trust-gated by design.',
    ...
    'WORK4YOU_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into {home}/dashboard-plugins/.',
]
...
def get_random_tip(exclude_recent: int = 0) -> str:
    ...
    ``{home}`` in the corpus is resolved at render time to the *real* home
    directory (profile-aware, and post brand migration that is no longer
    ``~/.wayne``) — never hard-code a spelling in the tip text.
    ...
    return random.choice(TIPS).replace("{home}", display_wayne_home())

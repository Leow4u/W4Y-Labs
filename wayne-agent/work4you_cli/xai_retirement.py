...
Pure logic: walks a Wayne config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `wayne doctor` and a future `wayne migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Wayne config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Wayne config and return retirement issues.

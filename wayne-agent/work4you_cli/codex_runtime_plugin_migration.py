"""Migrate Wayne' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Wayne' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Wayne mcp_servers.<n>.command/args/env  → codex stdio transport
  Wayne mcp_servers.<n>.url/headers       → codex streamable_http transport
  Wayne mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Wayne mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Wayne-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Wayne' own
...
MIGRATION_MARKER = (
    "# managed by wayne-agent — `wayne codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end wayne-agent managed section"
)
...
# Wayne keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Wayne' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, wayne_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Wayne MCP server config to the codex inline-table dict
    ...
    if not isinstance(wayne_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(wayne_cfg.get("command"))
    has_url = bool(wayne_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (WAYNE_HOME, PYTHONPATH) could
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Wayne' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Wayne' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Wayne
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="wayne-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a WAYNE_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed wayne-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.wayne`` symlink that happens to live under ``/private/var/folders``
...
def _build_wayne_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Wayne' own
    ...
    so a wayne installed under /opt/, /usr/local/, or a venv all work.
    WAYNE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # WAYNE_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_wayne_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever WAYNE_HOME its launcher sets
    ...
    # — that would override the launcher's WAYNE_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("WAYNE_HOME", tmp_path) would
    ...
    wayne_home = os.environ.get("WAYNE_HOME") or ""
    if wayne_home and _looks_like_test_tempdir(wayne_home):
        wayne_home = ""
    if wayne_home:
        env["WAYNE_HOME"] = wayne_home
    # PYTHONPATH passes through so a worktree-launched wayne finds the
    ...
    env["WAYNE_QUIET"] = "1"
    env["WAYNE_REDACT_SECRETS"] = env.get("WAYNE_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.wayne_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_wayne_tools: bool = True,
) -> MigrationReport:
    """Translate Wayne mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        wayne_config: full ~/.wayne/config.yaml dict
        ...
        expose_wayne_tools: when True (default), register Wayne' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Wayne for tools
    ...
    for name, cfg in wayne_servers.items():
    ...
    # Inject Wayne' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Wayne for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/wayne_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_wayne_tools:
        translated["wayne-tools"] = _build_wayne_tools_mcp_entry()
        if "wayne-tools" not in report.migrated:
            report.migrated.append("wayne-tools")

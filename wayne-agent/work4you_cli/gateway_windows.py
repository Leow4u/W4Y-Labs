...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m work4you_cli.main [--profile X] gateway run``
    ...
    prog_args = [pythonw_path, "-m", "work4you_cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    ``work4you_cli`` package without the venv launcher's site config.
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m work4you_cli.main gateway run``

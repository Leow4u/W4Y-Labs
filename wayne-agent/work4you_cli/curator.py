"""CLI subcommand: `wayne curator <subcommand>`.
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at {display_wayne_home()}/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    print(
        f"\nThis will replace the current {display_wayne_home()}/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help=f"Take a manual tar.gz snapshot of {display_wayne_home()}/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help=f"Restore {display_wayne_home()}/skills/ from a curator snapshot "

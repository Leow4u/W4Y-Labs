...
load_wayne_dotenv(
    wayne_home=_wayne_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.wayne/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_wayne_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("WAYNE_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("WAYNE_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("WAYNE_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent WayneCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str):
        ...
        self.proc = subprocess.Popen(
            ...
            bufsize=1,
            cwd=os.getcwd(),
            # slash_worker runs the Wayne agent → needs provider credentials.
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `wayne --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("WAYNE_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `wayne --tui`, so it must NOT count as dead here (doing
...
# ``profile`` on those calls; we open that profile's db and bind its WAYNE_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_wayne_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s WAYNE_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_wayne_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_wayne_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `wayne` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # WAYNE_HOME so config/skills/model resolve to it, and hand the
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _wayne_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_wayne_home_override()
        home = override if isinstance(override, str) and override else _wayne_home
...
def _resolve_model() -> str:
    env = (
        os.environ.get("WAYNE_MODEL", "")
        or os.environ.get("WAYNE_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over WAYNE_MODEL / WAYNE_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set WAYNE_INFERENCE_MODEL in the
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("WAYNE_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        current_provider = (
            ...
            or os.environ.get("WAYNE_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("WAYNE_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("WAYNE_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Wayne): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `wayne --tui` both land here. See
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _WAYNE_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off wayne-cli's tool universe — can't
...
def _apply_model_switch(
    ...
    *,
    ...
    #
    # We deliberately do NOT write process-global env vars (WAYNE_MODEL /
    # WAYNE_INFERENCE_MODEL / WAYNE_TUI_PROVIDER / WAYNE_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # WAYNE_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("WAYNE_DEV_CREDITS")):
        try:
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("WAYNE_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("WAYNE_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``WayneCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("WAYNE_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("WAYNE_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("WAYNE_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("WAYNE_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind WAYNE_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `WAYNE_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_wayne_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_wayne_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_wayne_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds WAYNE_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``wayne --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``wayne gateway`` process, whose
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Wayne has to support both the legacy 8-row petdex atlas and the current
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("WAYNE_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Wayne profile home
    # (~/.wayne/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $WAYNE_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn WAYNE_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_wayne_home_override(_profile_home_str)
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".wayne" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.wayne/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.wayne/desktop-attachments/``.
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("WAYNE_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["WAYNE_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("WAYNE_YOLO_MODE", None)
                    nv = "0"
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under WAYNE_HOME (~/.wayne by default) — config/sessions/skills,
    ...
    wayne_home = os.path.realpath(str(get_wayne_home()))

    return real == home or real == wayne_home or real.startswith(wayne_home + os.sep)
...
def _discover_repos_payload(db, *, conn=None, backfill: bool = True) -> list[dict]:
    ...
    repos even with zero wayne sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (wayne home subtree + bare
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("WAYNE_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("WAYNE_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["WAYNE_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["WAYNE_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("WAYNE_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("WAYNE_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_wayne_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``wayne journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``wayne plugins`` / the dashboard, so the three surfaces

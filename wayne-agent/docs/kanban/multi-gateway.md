...
Wayne supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.wayne/config.yaml`:
...
Or set the env var: `WAYNE_KANBAN_DISPATCH_IN_GATEWAY=false`

...
When running Wayne inside Docker, the default `network_mode: host` gives the
...
The Wayne [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   WAYNE_UID=$(id -u) WAYNE_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=wayne,wayne-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://wayne-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Wayne trust model and vulnerability reporting

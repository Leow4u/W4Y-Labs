...
Wayne middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Wayne'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Wayne calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Wayne retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Wayne guardrails,
...
- `telemetry_schema_version`: currently `wayne.observer.v1`
- `middleware_schema_version`: currently `wayne.middleware.v1`
...
Wayne stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Wayne runs
them as a nested chain in registration order. Middleware failures are fail-open:
Wayne logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Wayne applies middleware in this order:
...
For each tool call, Wayne applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Wayne pre-execution path against the effective arguments:
...
```bash
wayne plugins enable <plugin-name>
```

For isolated local testing, use one `WAYNE_HOME` for plugin enablement and the
agent run:

```bash
export WAYNE_HOME=/tmp/wayne-middleware-test
mkdir -p "$WAYNE_HOME"
wayne plugins enable <plugin-name>
wayne chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run wayne plugins enable <plugin-name>
uv run wayne chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["wayne_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/wayne-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Wayne expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Wayne treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Wayne preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Wayne does not convert downstream

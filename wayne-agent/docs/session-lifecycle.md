...
The canonical transcript store is SQLite via `SessionDB` (from `wayne_state`). The
...
Counts consecutive restarts via a JSON file (`{WAYNE_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `wayne update`, `wayne gateway restart`,
or `/restart`.

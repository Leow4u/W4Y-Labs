...
This document is the formal interface between the **Wayne gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `wayne gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`wayne gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `wayne` CLI.

...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with wayne-agent)
2. User-installed providers: ``$WAYNE_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_wayne_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_wayne_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_wayne_user_memory'`` — the
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$WAYNE_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _wayne_user_memory.<name>.cli, usually

...
```bash
wayne memory setup    # select "holographic"
```

Or manually:
```bash
wayne config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.wayne-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$WAYNE_HOME/memory_store.db` | SQLite database path |

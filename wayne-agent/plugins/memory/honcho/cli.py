...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "wayne" else ""
...
def _local_config_path() -> Path:
    ...
    Always returns $WAYNE_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_wayne_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        wayne_host.get("pinUserPeer"),
        wayne_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in wayne_host:
        aliases_src = wayne_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in wayne_host:
        prefix_src = wayne_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(wayne_host: dict) -> None:
...
def _apply_runtime_prefix(
    wayne_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        wayne_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(wayne_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = wayne_host.get("userPeerAliases")
    prefix = wayne_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(wayne_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(wayne_host)
    wayne_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        wayne_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        wayne_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = wayne_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            wayne_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(wayne_host)
        ...
        if use_oauth:
            ...
            if cred.consent_peer_name:
                wayne_host["peerName"] = cred.consent_peer_name
    ...
    # --- 3. Identity ---
    current_peer = wayne_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        wayne_host["peerName"] = new_peer

    current_ai = wayne_host.get("aiPeer") or cfg.get("aiPeer", "wayne")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        wayne_host["aiPeer"] = new_ai

    current_workspace = wayne_host.get("workspace") or cfg.get("workspace", "wayne")
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, wayne_host)
    ...
    if run_mapping:
        peer_target = wayne_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(wayne_host)
            wayne_host["pinUserPeer"] = False
            if prior_aliases:
                wayne_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                wayne_host, current_prefix, prefix_from_root,
            ...
            if merged:
                wayne_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                wayne_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                wayne_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = wayne_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        wayne_host["observationMode"] = new_obs
    else:
        wayne_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(wayne_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        wayne_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        wayne_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = wayne_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        wayne_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = wayne_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        wayne_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                wayne_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(wayne_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            wayne_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        wayne_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        wayne_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        wayne_host["dialecticReasoningLevel"] = new_reasoning
    else:
        wayne_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = wayne_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        wayne_host["sessionStrategy"] = new_strat

    wayne_host["enabled"] = True
    wayne_host.setdefault("saveMessages", True)
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        wayne = hosts.get(_host_key(), {})
        user = wayne.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = wayne.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = wayne.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = wayne.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = wayne.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = wayne.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = wayne.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()

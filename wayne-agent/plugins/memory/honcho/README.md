...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/wayne>
...
```bash
wayne memory setup honcho   # configure Honcho directly (works on a fresh install)
wayne memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
wayne config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.wayne/.env
```

> `wayne honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `wayne memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$WAYNE_HOME/honcho.json` | Profile-local (isolated Wayne instances) |
| 2 | `~/.wayne/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Wayne profile: `wayne` (default) or `wayne_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `wayne honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Wayne session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`wayne-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`wayne`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-wayne-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening wayne in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Wayne session ID (timestamp + hex). Every `wayne` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Wayne profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`wayne`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Wayne profile: `wayne` (default) or `wayne_<profile>` (e.g. `wayne -p coder` -> host key `wayne_coder`). Older `wayne.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `WAYNE_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `wayne-agent`) |
...
|---------|-------------|
| `wayne memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `wayne honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `wayne memory setup`) |
| `wayne honcho status` | Show resolved config for active profile |
| `wayne honcho enable` / `disable` | Toggle Honcho for active profile |
| `wayne honcho mode <mode>` | Change recall or observation mode |
| `wayne honcho peer --user <name>` | Update user peer name |
| `wayne honcho peer --ai <name>` | Update AI peer name |
| `wayne honcho tokens --context <N>` | Set context token budget |
| `wayne honcho tokens --dialectic <N>` | Set dialectic max chars |
| `wayne honcho map <name>` | Map current directory to a session name |
| `wayne honcho sync` | Create host blocks for all Wayne profiles |

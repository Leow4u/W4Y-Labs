...
```bash
wayne memory setup    # select "byterover"
```

Or manually:
```bash
wayne config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.wayne/.env
```
...
Working directory: `$WAYNE_HOME/byterover/` (profile-scoped).

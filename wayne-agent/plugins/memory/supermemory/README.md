...
- Supermemory API key from [app.supermemory.ai/integrations?connect=wayne](http://app.supermemory.ai/integrations?connect=wayne)

## Setup

```bash
wayne memory setup    # select "supermemory"
```

Or manually:

```bash
wayne config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.wayne/.env
```

## Config

Config file: `$WAYNE_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `wayne` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `wayne-{identity}` → `wayne-coder`). |
...
All Supermemory API calls send `x-sm-source: wayne`, and document writes stamp
`metadata.sm_source: wayne`. This is a **functional routing key, not telemetry**:
it groups Wayne-written memories into a dedicated "Wayne" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Wayne profile:

```json
{
  "container_tag": "wayne-{identity}"
}
```

For a profile named `coder`, this resolves to `wayne-coder`. The default profile resolves to `wayne-default`. Without `{identity}`, all profiles share the same container.

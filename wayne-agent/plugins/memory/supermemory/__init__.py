...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=wayne"
...
def _load_supermemory_config(wayne_home: str) -> dict:
    config = _default_config()
    config_path = Path(wayne_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, wayne_home: str) -> None:
    config_path = Path(wayne_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "wayne"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "wayne", **(metadata or {})}
...
def _resolve_container_tag_for_setup(wayne_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(wayne_home)
...
def _probe_supermemory_connection(api_key: str, wayne_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(wayne_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(wayne_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `wayne memory setup`.
        # All other options are documented for $WAYNE_HOME/supermemory.json
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, wayne_home)
    ...
    def post_setup(self, wayne_home: str, config: dict) -> None:
        ...
        if env_writes:
            _write_env_vars(Path(wayne_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._config = _load_supermemory_config(self._wayne_home)

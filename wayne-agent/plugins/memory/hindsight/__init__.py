...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: wayne)
...
Or via $WAYNE_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Wayne can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $WAYNE_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_wayne_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "wayne": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "wayne"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Wayne config."""
    profile = config.get("profile", "wayne")
    return str(profile or "wayne")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Wayne profile name (from agent_identity)
      {workspace} — Wayne workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``wayne-{user}`` with no user becomes ``wayne``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Wayne Agent and the User"
    ...
    def save_config(self, values, wayne_home):
        """Write config to $WAYNE_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(wayne_home) / "hindsight"
    ...
    def post_setup(self, wayne_home: str, config: dict) -> None:
        ...
        provider_config.setdefault("bank_id", "wayne")
        ...
        self.save_config(provider_config, wayne_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(wayne_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(wayne_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "wayne"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: wayne-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Wayne Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "wayne"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "wayne"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "wayne", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "wayne")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Wayne Agent and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                ...
                # Wayne get sluggish. (issue #13125)
                try:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Wayne created/used that client on the shared async loop,

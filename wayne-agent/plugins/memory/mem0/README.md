...
```bash
wayne memory setup    # select "mem0"
```

Or manually:
```bash
wayne config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.wayne/.env
```

## Config

Behavioral settings live in `$WAYNE_HOME/mem0.json` (set them via `wayne memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.wayne/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `wayne-user` | User identifier on Mem0 |
| `agent_id` | `wayne` | Agent identifier |
...
```bash
wayne memory setup mem0 --mode oss \
...
```bash
wayne memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$WAYNE_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.wayne/mem0_qdrant"}}
  }
}
```
...
```bash
wayne memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
wayne memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$WAYNE_HOME/mem0.json`).

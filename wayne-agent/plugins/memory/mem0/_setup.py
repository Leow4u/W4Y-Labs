...
def _save_mem0_json(wayne_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(wayne_home) / "mem0.json"
    existing = {}
...
def _setup_platform(wayne_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "wayne-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "wayne"},
    ...
    provider.save_config(provider_config, wayne_home)

    if env_writes:
        _write_env(Path(wayne_home) / ".env", env_writes)
...
def _setup_oss(wayne_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(wayne_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "wayne-user")
    ...
    if env_writes:
        _write_env(Path(wayne_home) / ".env", env_writes)
    _save_mem0_json(wayne_home, {"mode": "oss", "user_id": user_id, "agent_id": "wayne", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, wayne_home: str) -> str:
...
_PGVECTOR_CONTAINER = "wayne-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "wayne"
...
def _setup_oss_interactive(wayne_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], wayne_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], wayne_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'wayne-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "wayne-user")

    agent_id = input("  Agent ID [wayne]: ").strip()
    agent_id = agent_id or "wayne"
    ...
    if env_writes:
        _write_env(Path(wayne_home) / ".env", env_writes)
    _save_mem0_json(wayne_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(wayne_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(wayne_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(wayne_home, config, flags)
        return

/**
 * Wayne Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.wayne/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__WAYNE_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__WAYNE_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "wayne-kanban-dot-triage",
    todo: "wayne-kanban-dot-todo",
    ready: "wayne-kanban-dot-ready",
    running: "wayne-kanban-dot-running",
    blocked: "wayne-kanban-dot-blocked",
    done: "wayne-kanban-dot-done",
    archived: "wayne-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-wayne-task";
  ...
  const LS_BOARD_KEY = "wayne.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="wayne-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])wayne-kanban-md-code\1/i.test(attrs)) {
      return ' class="wayne-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "wayne-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "wayne-kanban-md",
  ...
  // standard `drop` event and our `wayne-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("wayne-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("wayne-kanban-column--drop");
          if (target) target.classList.add("wayne-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("wayne-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("wayne-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("wayne-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__WAYNE_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "wayne-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "wayne-kanban-attention",
        "wayne-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "wayne-kanban-attention-bar" },
        h("span", { className: "wayne-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "wayne-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "wayne-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "wayne-kanban-attention-row",
                  "wayne-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "wayne-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "wayne-kanban-attention-row-id" }, task.id),
                h("span", { className: "wayne-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "wayne-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "wayne-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "wayne-kanban-diag-action-btn",
      action.suggested ? "wayne-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " wayne-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".wayne-kanban-drawer-comment-row input, .wayne-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "wayne-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("wayne-kanban-diag", sevClass) },
      h("div", { className: "wayne-kanban-diag-header" },
        h("span", { className: "wayne-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "wayne-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "wayne-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "wayne-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "wayne-kanban-diag-data-row" },
                  h("span", { className: "wayne-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "wayne-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "wayne-kanban-diag-data-row" },
                h("span", { className: "wayne-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "wayne-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "wayne-kanban-diag-reassign-row" },
            h("span", { className: "wayne-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "wayne-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "wayne-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "wayne-kanban-diag-msg",
              msg.ok ? "wayne-kanban-diag-msg--ok" : "wayne-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "wayne-kanban-section" },
      h("div", { className: "wayne-kanban-section-head-row" },
        h("span", { className: "wayne-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "wayne-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "wayne-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "wayne-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "wayne-kanban-docs-link",
      title: "Open Wayne Kanban docs in a new tab",
      "aria-label": "Wayne Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `wayne kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "wayne-kanban-msg-ok" : "wayne-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "wayne-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "wayne-kanban-boardswitcher" },
      h("div", { className: "wayne-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "wayne-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "wayne-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "wayne-kanban-dialog-title" },
        ...
        h("div", { className: "wayne-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Wayne profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "wayne-kanban-bulk" },
      h("span", { className: "wayne-kanban-bulk-count" },
      ...
      h("div", { className: "wayne-kanban-bulk-priority",
      ...
      h("div", { className: "wayne-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Wayne profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "wayne-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("wayne-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("wayne-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "wayne-kanban-trash",
        dragOver ? "wayne-kanban-trash--drop" : "",
        props.draggingTaskId ? "wayne-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "wayne-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "wayne-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".wayne-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "wayne-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("wayne-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("wayne-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "wayne-kanban-column",
        dragOver ? "wayne-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "wayne-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "wayne-kanban-col-check",
        ...
        h("span", { className: cn("wayne-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "wayne-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "wayne-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "wayne-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "wayne-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "wayne-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "wayne-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "wayne-kanban-lane" },
                  h("div", { className: "wayne-kanban-lane-head" },
                    h("span", { className: "wayne-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "wayne-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "wayne-kanban-card--stale-red";
    if (age >= tier.amber) return "wayne-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".wayne-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "wayne-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "wayne-kanban-card",
        props.selected ? "wayne-kanban-card--selected" : "",
        props.failed ? "wayne-kanban-card--failed" : "",
        props.draggingSource ? "wayne-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "wayne-kanban-card-content" },
          h("div", { className: "wayne-kanban-card-row" },
            h("label", {
              className: "wayne-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "wayne-kanban-card-check",
            ...
            h("span", { className: "wayne-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or wayne kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "wayne-kanban-warning-badge",
                    "wayne-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "wayne-kanban-needs-assignee",
          ...
          h("div", { className: "wayne-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "wayne-kanban-card-row wayne-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "wayne-kanban-assignee",
                            title: `Assigned to Wayne profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "wayne-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "wayne-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "wayne-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "wayne-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Wayne profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Wayne profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "wayne-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "wayne-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "wayne-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "wayne-kanban-drawer-close",
        ...
        data ? h("div", { className: "wayne-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "wayne-kanban-section" },
      h("div", { className: "wayne-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "wayne-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "wayne-kanban-drawer-body" },
      h("div", { className: "wayne-kanban-drawer-title" },
        h("span", { className: cn("wayne-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "wayne-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "wayne-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "wayne-kanban-section" },
        h("div", { className: "wayne-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "wayne-kanban-section" },
        h("div", { className: "wayne-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "wayne-kanban-comment" },
                h("div", { className: "wayne-kanban-comment-head" },
                  h("span", { className: "wayne-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "wayne-kanban-comment-ago" },
      ...
      h("div", { className: "wayne-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "wayne-kanban-event",
              isDiag ? "wayne-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "wayne-kanban-event-header" },
                  h("span", { className: "wayne-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "wayne-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "wayne-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "wayne-kanban-event-header-plain" },
                  h("span", { className: "wayne-kanban-event-kind" }, e.kind),
                  h("span", { className: "wayne-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "wayne-kanban-event-phantom-row" },
                  h("span", { className: "wayne-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "wayne-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "wayne-kanban-event-payload" },
    ...
    return h("div", { className: "wayne-kanban-section" },
      h("div", { className: "wayne-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "wayne-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `wayne-kanban-run--${r.outcome || r.status || "ended"}`
          : "wayne-kanban-run--active";
        return h("div", { key: r.id, className: cn("wayne-kanban-run", outcomeClass) },
          h("div", { className: "wayne-kanban-run-head" },
            h("span", { className: "wayne-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "wayne-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "wayne-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "wayne-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "wayne-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "wayne-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "wayne-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "wayne-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "wayne-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "wayne-kanban-pre wayne-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "wayne-kanban-meta-row" },
      h("span", { className: "wayne-kanban-meta-label" }, props.label),
      h("span", { className: "wayne-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "wayne-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "wayne-kanban-meta-row" },
        h("span", { className: "wayne-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "wayne-kanban-meta-value wayne-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "wayne-kanban-meta-row" },
        h("span", { className: "wayne-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "wayne-kanban-meta-value wayne-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "wayne-kanban-section" },
      h("div", { className: "wayne-kanban-section-head-row" },
        h("span", { className: "wayne-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "wayne-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "wayne-kanban-section" },
      h("div", { className: "wayne-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "wayne-kanban-deps-row" },
        h("span", { className: "wayne-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "wayne-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "wayne-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "wayne-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "wayne-kanban-dep-chip-x",
      ...
      h("div", { className: "wayne-kanban-deps-row" },
        h("span", { className: "wayne-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "wayne-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "wayne-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "wayne-kanban-section" },
      ...
      h("div", { className: "wayne-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "wayne-kanban-home-sub wayne-kanban-home-sub--on"
              : "wayne-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__WAYNE_PLUGINS__ && typeof window.__WAYNE_PLUGINS__.register === "function") {
    window.__WAYNE_PLUGINS__.register("kanban", KanbanPage);
  }
})();

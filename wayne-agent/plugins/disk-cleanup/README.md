...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `WAYNE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under WAYNE_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `WAYNE_HOME` or `/tmp/wayne-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$WAYNE_HOME/disk-cleanup/` is itself excluded
- `$WAYNE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

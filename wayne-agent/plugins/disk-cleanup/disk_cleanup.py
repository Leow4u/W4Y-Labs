"""disk_cleanup — ephemeral file cleanup for Wayne Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under WAYNE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly WAYNE_HOME and /tmp/wayne-*
Never touches: ~/.wayne/logs/ or any system directory.
"""
...
try:
    ...
    def get_wayne_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("WAYNE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".wayne").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$WAYNE_HOME/logs/``."""
    return get_wayne_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$WAYNE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under WAYNE_HOME or ``/tmp/wayne-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    wayne_home = get_wayne_home()
    try:
        path.resolve().relative_to(wayne_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/wayne-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("wayne-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "wayne-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $WAYNE_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so WAYNE_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        wayne_home = get_wayne_home()
        for parent in ("cron", "cronjobs"):
            base = wayne_home / parent
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under WAYNE_HOME, but never recurse into known
    # durable state trees.  Some installs place the Wayne checkout, venv,
    # and desktop build under WAYNE_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    wayne_home = get_wayne_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in wayne_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    wayne_home = get_wayne_home()
    try:
        rel = path.resolve().relative_to(wayne_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "wayne-agent",
        }:
            return None
        ...
        # Path isn't under WAYNE_HOME (e.g. /tmp/wayne-*) — fall through.
        pass

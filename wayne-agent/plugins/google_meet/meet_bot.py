...
``$WAYNE_HOME/workspace/meetings/<meeting-id>/``. The main wayne process
...
# Filenames the bot reads/writes in ``WAYNE_MEET_OUT_DIR``.
...
# and exposes ``window.__wayneMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__wayneMeetInstalled) return;
  window.__wayneMeetInstalled = true;
  window.__wayneMeetQueue = [];
  ...
  window.__wayneMeetDrain = () => {
    const out = window.__wayneMeetQueue.slice();
    window.__wayneMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "wayne_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("WAYNE_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__wayneMeetDrain && window.__wayneMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__wayneMeetInstalled) {

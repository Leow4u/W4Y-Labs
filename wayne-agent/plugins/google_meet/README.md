...
Let the wayne agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where wayne runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `wayne meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `wayne meet` CLI |
...
| `cli.py` | `wayne meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$WAYNE_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `wayne meet node {run,list,approve,remove,status,ping}` |
...
wayne meet install                                      # pip + Chromium
wayne meet setup                                        # preflight
wayne meet auth                                         # optional
wayne meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
wayne meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.wayne/.env
wayne meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
wayne meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
wayne meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.wayne/.env
wayne meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, wayne will **not** switch your system audio input automatically — the
...
wayne plugins enable google_meet
wayne meet node run --display-name my-mac --host 0.0.0.0 --port 18789

...
session-scoped state file under ``$WAYNE_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_wayne_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this wayne install, leave it first —
    ...
    env = os.environ.copy()
    env["WAYNE_MEET_URL"] = url
    env["WAYNE_MEET_OUT_DIR"] = str(out)
    env["WAYNE_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["WAYNE_MEET_HEADED"] = "1"
    if auth_state:
        env["WAYNE_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["WAYNE_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if WAYNE_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["WAYNE_MEET_MODE"] = mode
    if realtime_model:
        env["WAYNE_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["WAYNE_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["WAYNE_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["WAYNE_MEET_REALTIME_KEY"] = realtime_api_key

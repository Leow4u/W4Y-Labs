...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "wayne-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Wayne Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Wayne Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: wayne_, wayne_1, wayne_2...
...
def check_requirements() -> bool:
    ...
    # wayne setup / requirements check path.
...
def interactive_setup() -> None:
    ...
    print_info("Connect Wayne to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #wayne — comma-separate for multiple)",
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "wayne-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "wayne-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Wayne Agent (cron)")

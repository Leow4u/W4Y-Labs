...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        wayne_action = data.get("wayne_action", "")
        session_key = data.get("session_key", "")

        if not wayne_action or not session_key:
        ...
        choice = choice_map.get(wayne_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="wayne_approve",
                    data={**btn_data_base, "wayne_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="wayne_approve",
                    data={**btn_data_base, "wayne_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="wayne_approve",
                    data={**btn_data_base, "wayne_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="wayne_approve",
                    data={**btn_data_base, "wayne_action": "deny"},
                    style="destructive",
                ),
            ])
        )

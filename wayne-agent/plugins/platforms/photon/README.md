...
TypeScript-only, Wayne runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Wayne-Sidecar-Token`.

## First-time setup

```bash
# One-shot setup: device login (opens browser) + project + user + sidecar deps
wayne photon setup --phone +15551234567

# Start the gateway
wayne gateway start
```

`wayne photon setup` does, in order:
...
There is no separate `login` command; like every other Wayne channel,
...
Run `wayne photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.wayne/.env` (the same place every other
...
Management metadata lives in `~/.wayne/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `wayne photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `wayne photon status`, a DM and a group roundtrip,

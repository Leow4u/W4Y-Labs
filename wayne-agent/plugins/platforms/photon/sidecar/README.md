...
Small Node helper that bridges Wayne Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Wayne is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Wayne-Sidecar-Token`)
...
The Wayne plugin's `wayne photon setup` command runs `npm install`
here automatically.

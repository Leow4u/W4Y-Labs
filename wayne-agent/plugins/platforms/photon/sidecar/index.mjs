...
//
// Inbound  (gRPC -> Wayne): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Wayne -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Wayne-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `wayne photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Wayne can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Wayne'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Wayne allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Wayne, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-wayne-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Wayne supplied them so a known-good

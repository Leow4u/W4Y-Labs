...
Credential storage mirrors every other Wayne channel:

    * runtime SDK creds  -> ``~/.wayne/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.wayne/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a wayne dependency
...
# registers Wayne as its own client_id.
...
# Default name of the project Wayne provisions for the operator.
DEFAULT_PROJECT_NAME = "Wayne Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of wayne-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.wayne/auth.json`` honouring the active Wayne profile."""
    try:
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.wayne/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.wayne/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.wayne/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: wayne_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `wayne photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `wayne photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `wayne photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `wayne photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `wayne photon setup`)"

...
there is no standalone ``login`` verb (matching how every other Wayne
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Wayne Agent')")
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Wayne Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.wayne/.env,
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.wayne/.env.

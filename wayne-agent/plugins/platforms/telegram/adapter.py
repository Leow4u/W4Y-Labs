...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("WAYNE_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "WAYNE_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "WAYNE_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Wayne-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on Wayne' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Wayne must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `wayne update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Wayne or OpenClaw instance is running "
            "with this token, then restart the gateway with 'wayne gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # payload size limit (~4KB total).  Wayne defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the Wayne process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("WAYNE_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("WAYNE_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("WAYNE_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("WAYNE_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("WAYNE_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }
            ...
            disable_fallback = (os.getenv("WAYNE_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            _max_connect = 8
            _init_timeout = _env_float("WAYNE_TELEGRAM_INIT_TIMEOUT", 30.0)
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``wayne model`` picker via the shared ``group_providers`` fold,
    ...
    # Scripts live in ~/.wayne/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".wayne" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@wayne_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Wayne bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the Wayne inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the Wayne inbox (#52363).
        if self._is_own_message(message):
            return False
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("WAYNE_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def register(ctx) -> None:
    """Plugin entry point — called by the Wayne plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'wayne-agent[telegram]'",

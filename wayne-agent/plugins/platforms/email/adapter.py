"""
Email platform adapter for the Wayne gateway.

Allows users to interact with Wayne by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "wayne-agent" "version" "{_wayne_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `wayne config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `wayne gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Wayne Agent")
        ...
        msg_id = f"<wayne-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# wayne_cli/gateway.py, and the _send_email dispatch in

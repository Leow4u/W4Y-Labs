"""ntfy platform adapter (Wayne plugin).
...
``plugins/platforms/ntfy/``. The Wayne plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "wayne-in"              # subscribe topic (incoming)
          publish_topic: "wayne-out"     # optional — defaults to topic
...
_ECHO_TAG = "wayne-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``wayne cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Wayne dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `wayne gateway status` without

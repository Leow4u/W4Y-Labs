...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.wayne/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("WAYNE_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("WAYNE_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = env_float(
            "WAYNE_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "WAYNE_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Wayne gateway
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``WAYNE_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_wayne_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Wayne manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        # wayne_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from wayne_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from wayne_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from wayne_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Wayne session in the thread
...
def _component_check_auth(
    ...
    the pairing store (``wayne pairing approve``).
    ...
    # so users approved via ``wayne pairing approve`` can interact with
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("WAYNE_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["WAYNE_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("WAYNE_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["WAYNE_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'wayne-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # wayne_cli/setup.py::_setup_discord function.  Same shape as Teams.

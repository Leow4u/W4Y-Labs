...
- Per-user tokens (keyed by sender email):
    ``${WAYNE_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${WAYNE_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${WAYNE_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${WAYNE_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${WAYNE_HOME}/google_chat_user_client_secret.json``
"""
...
def _wayne_home() -> Path:
    """Resolve WAYNE_HOME at call time (NOT module import).

    Tests and ``WAYNE_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.wayne/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _wayne_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _wayne_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _wayne_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _wayne_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _wayne_home() / "google_chat_user_client_secret.json"
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into WAYNE_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_wayne_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_wayne_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Wayne (native attachment delivery)"
    )

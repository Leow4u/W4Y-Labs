...
# ``wayne_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Wayne is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a wayne MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${WAYNE_HOME}/google_chat_thread_counts.json so
        ...
        self._thread_count_store = _ThreadCountStore(
            _wayne_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Wayne is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("WAYNE_HOME", str(_Path.home() / ".wayne"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${WAYNE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a wayne MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Wayne is thinking…' marker message.
...
def interactive_setup() -> None:
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. wayne-chat-events) and a Pull subscription.")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Wayne plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'wayne-agent[google_chat]'",
        ...
        # the "Wayne is thinking..." marker patches and edit overhead.

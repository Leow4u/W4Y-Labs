"""SimpleX Chat platform adapter (Wayne plugin).
...
This adapter ships as a Wayne platform plugin under
``plugins/platforms/simplex/``. The Wayne plugin loader scans the
...
Optional environment variables:
    ...
    WAYNE_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``wayne setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Wayne — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("WAYNE_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``wayne setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.wayne/.env`` via
    ``wayne_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "wayne_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.wayne/.env"
        )
        return

...
for-secrets-only rule), provisioned by NAS at deploy time (Phase 3):
    ...
    WAYNE_DASHBOARD_DRAIN_SECRET   # the per-agent shared secret (>=43 url-safe-b64 chars)
...
When ``WAYNE_DASHBOARD_DRAIN_SECRET`` is unset, the plugin is a no-op (records
...
def register(ctx) -> None:
    ...
    No-op (records a skip reason) when ``WAYNE_DASHBOARD_DRAIN_SECRET`` is
    ...
    try:
        from wayne_cli.dashboard_auth.token_auth import register_token_route

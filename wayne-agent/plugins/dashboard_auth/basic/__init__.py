...
mirroring the Nous provider's precedence convention:
  ...
  Environment overrides::
      ...
      WAYNE_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      WAYNE_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      WAYNE_DASHBOARD_BASIC_AUTH_SECRET
      WAYNE_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "WAYNE_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "WAYNE_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "WAYNE_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "WAYNE_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "WAYNE_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )
    ...
    # WAYNE_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "WAYNE_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()

...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      WAYNE_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      WAYNE_DASHBOARD_PORTAL_URL       — defaults to
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``wayne-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``wayne_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set WAYNE_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via WAYNE_DASHBOARD_PORTAL_URL for
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"wayne_session_pkce\"]``
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between WAYNE_DASHBOARD_PORTAL_URL / WAYNE_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``WAYNE_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("WAYNE_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``WAYNE_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("WAYNE_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``WAYNE_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set WAYNE_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``wayne_cli.web_server.should_require_auth`` +
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"WAYNE_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "

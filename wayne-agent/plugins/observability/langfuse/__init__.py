"""langfuse — Wayne plugin for Langfuse observability.
...
Optional env vars:
  WAYNE_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  WAYNE_LANGFUSE_RELEASE     - release/version tag
  WAYNE_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  WAYNE_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  WAYNE_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("WAYNE_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Wayne plugin system —
    ...
    public_key = _env("WAYNE_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("WAYNE_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("WAYNE_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("WAYNE_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset WAYNE_LANGFUSE_PUBLIC_KEY / WAYNE_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("WAYNE_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("WAYNE_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("WAYNE_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("WAYNE_LANGFUSE_SAMPLE_RATE")
...
def _trace_key(
    ...
    *,
    ...
    Older Wayne paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("WAYNE_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Wayne branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Wayne also has a turn-scoped
    ...
    # messages is a list only for legacy Wayne branches that fired
    # pre_llm_call with API messages directly. Current Wayne fires
...
def register(ctx) -> None:
    ...
    # Wayne versions.  pre_api_request / post_api_request fire per API

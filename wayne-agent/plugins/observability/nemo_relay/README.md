...
Optional Wayne observability plugin that maps Wayne observer hooks to
...
```bash
wayne plugins enable observability/nemo_relay
```

The `WAYNE_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `WAYNE_HOME` that the
agent run will use:

```bash
env WAYNE_HOME=/tmp/wayne-nemo-relay-test \
  wayne plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`WAYNE_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`WAYNE_HOME` is the Wayne profile/config home used by both
`wayne plugins enable ...` and the later `wayne chat ...` run. If unset,
Wayne uses the user's default home, usually `~/.wayne`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export WAYNE_HOME=/tmp/wayne-nemo-relay-test
wayne plugins enable observability/nemo_relay
wayne chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `wayne` command you run is built from the
...
uv run wayne plugins enable observability/nemo_relay
uv run wayne chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/wayne_agent-*.whl
...
The plugin can configure exporters directly from `WAYNE_NEMO_RELAY_*`
...
```bash
export WAYNE_NEMO_RELAY_ATOF_ENABLED=1
export WAYNE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export WAYNE_NEMO_RELAY_ATIF_ENABLED=1
export WAYNE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `WAYNE_NEMO_RELAY_ATOF_FILENAME`
- `WAYNE_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `WAYNE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `WAYNE_NEMO_RELAY_ATIF_AGENT_NAME`
- `WAYNE_NEMO_RELAY_ATIF_AGENT_VERSION`
- `WAYNE_NEMO_RELAY_ATIF_MODEL_NAME`
- `WAYNE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Wayne at it:

```bash
export WAYNE_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `WAYNE_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`WAYNE_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`WAYNE_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Wayne does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Wayne keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Wayne routes LLM and tool
...
For the full generic Wayne middleware contract, see
...
export WAYNE_HOME=/tmp/wayne-nemo-relay-docs/wayne-home
mkdir -p "$WAYNE_HOME"

cat > "$WAYNE_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Wayne session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export WAYNE_NEMO_RELAY_ATOF_ENABLED=1
export WAYNE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/wayne-nemo-relay-docs/subagent/atof
export WAYNE_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export WAYNE_NEMO_RELAY_ATOF_MODE=overwrite
export WAYNE_NEMO_RELAY_ATIF_ENABLED=1
...
{"kind":"mark","name":"wayne.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
message. Wayne dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/wayne-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/wayne-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/wayne-nemo-relay-docs/workdir/beta.txt
cd /tmp/wayne-nemo-relay-docs/workdir

export WAYNE_NEMO_RELAY_ATOF_ENABLED=1
export WAYNE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/wayne-nemo-relay-docs/parallel/atof
export WAYNE_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export WAYNE_NEMO_RELAY_ATOF_MODE=overwrite
export WAYNE_NEMO_RELAY_ATIF_ENABLED=1
...
- Wayne sessions map to `agent` scopes.
- Wayne API request hooks map to `llm` scope start/end events.
- Wayne tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Wayne execution middleware to hand
...
```bash
export WAYNE_NEMO_RELAY_PLUGINS_TOML=/tmp/wayne-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Wayne routes execution
...
Wayne provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Wayne provider adapter next_call(...)

Wayne tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Wayne tool dispatcher next_call(...)
```
...
middleware for a local Wayne run. This path requires a NeMo Relay runtime that
...
```bash
export WAYNE_HOME=/tmp/wayne-middleware-test/wayne-home
mkdir -p "$WAYNE_HOME" /tmp/wayne-middleware-test/nemo-relay
...
cat > /tmp/wayne-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/wayne-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/wayne-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Wayne Middleware E2E"
agent_version = "local"
...
export WAYNE_NEMO_RELAY_PLUGINS_TOML=/tmp/wayne-middleware-test/nemo-relay/plugins.toml

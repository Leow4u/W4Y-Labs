# Wayne Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: wayne-achievements plugin
...
### `GET /api/plugins/wayne-achievements/achievements`
...
### `POST /api/plugins/wayne-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/wayne-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/wayne-achievements/overview`
...
- Persist plugin data under:
  - `~/.wayne/plugins/wayne-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.wayne/plugins/wayne-achievements/scan_checkpoint.json`
...
- `~/.wayne/plugins/wayne-achievements/`

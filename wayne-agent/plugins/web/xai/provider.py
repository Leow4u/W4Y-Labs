...
``{title, url, description, position}`` rows every other Wayne web
provider produces.
...
prefers Wayne-managed xAI Grok OAuth (via ``wayne auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.wayne/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``wayne tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `wayne auth` to sign in with "
